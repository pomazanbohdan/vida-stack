...
pub(crate) fn fsync_directory<P: AsRef<Path>>(path: P) -> std::io::Result<()> {
	...
	// ERROR_ACCESS_DENIED because FlushFileBuffers requires access that
	// directory handles do not provide. Newer SurrealKV skips directory fsync
	// on Windows; keep this local patch until the SurrealDB dependency can move
	// past surrealkv 0.20.x.
	#[cfg(not(target_os = "windows"))]
	{
