...
proc cmdLease*(args: seq[string]): int =
  if args.len == 0:
    echo """Usage:
  vida-legacy lease acquire <resource_type> <resource_id> <holder> [--ttl-seconds N]
  vida-legacy lease renew <resource_type> <resource_id> <holder> [--ttl-seconds N]
  vida-legacy lease release <resource_type> <resource_id> <holder>
  vida-legacy lease list"""
    return 1

  case args[0]
  of "acquire":
    if args.len < 4:
      echo "Usage: vida-legacy lease acquire <resource_type> <resource_id> <holder>"
      return 1
    var ttl = 3600
  ...
  of "renew":
    if args.len < 4:
      echo "Usage: vida-legacy lease renew <resource_type> <resource_id> <holder>"
      return 1
    var ttl = 3600
    if args.len > 4 and args[4] == "--ttl-seconds" and args.len > 5:
      ttl = parseInt(args[5])
  ...
  of "release":
    if args.len < 4:
      echo "Usage: vida-legacy lease release <resource_type> <resource_id> <holder>"
      return 1
