...
proc cmdRoute*(args: seq[string]): int =
  if args.len == 0:
    echo """Usage:
  vida-legacy route snapshot <task_class> [task_id]
  vida-legacy route receipt <task_id> <task_class>
  vida-legacy route hash <task_id> <task_class>
  vida-legacy route framework-snapshot
  vida-legacy route project-snapshot"""
    return 1

  case args[0]
  of "snapshot":
    if args.len < 2: echo "Usage: vida-legacy route snapshot <task_class> [task_id]"; return 1
  ...
  of "receipt":
    if args.len < 3: echo "Usage: vida-legacy route receipt <task_id> <task_class>"; return 1
    let (_, route) = routeSnapshot(args[2], args[1])
    let path = writeRouteReceipt(args[1], args[2], route)
    echo path; return 0

  of "hash":
    if args.len < 3: echo "Usage: vida-legacy route hash <task_id> <task_class>"; return 1
    let (_, route) = routeSnapshot(args[2], args[1])
