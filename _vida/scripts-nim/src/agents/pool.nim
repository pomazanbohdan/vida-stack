...
proc cmdPool*(args: seq[string]): int =
  if args.len == 0:
    echo """Usage:
  vida-legacy pool borrow <task_class> <holder> [--ttl-seconds N]
  vida-legacy pool release <subagent> <holder>
  vida-legacy pool status"""
    return 1

  case args[0]
  of "borrow":
    if args.len < 3:
      echo "Usage: vida-legacy pool borrow <task_class> <holder> [--ttl-seconds N]"
      return 1
    var ttl = 1800
  ...
  of "release":
    if args.len < 3:
      echo "Usage: vida-legacy pool release <subagent> <holder>"
      return 1
