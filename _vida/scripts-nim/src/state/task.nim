## VIDA Legacy task surface — DB-backed replacement for basic br reads.
...
proc cmdTask*(args: seq[string]): int =
  if args.len == 0:
    echo """Usage:
  vida-legacy task create <task_id> <title> [--type <issue_type>] [--status <status>] [--priority <n>] [--display-id <display_id>] [--parent-id <task_id>] [--parent-display-id <display_id>] [--auto-display-from <parent_display_id>] [--description <description>] [--labels <label>] [--json]
  vida-legacy task update <task_id> [--status <status>] [--notes <notes>] [--description <description>] [--add-label <label>] [--remove-label <label>] [--set-labels <label>] [--json]
  vida-legacy task close <task_id> --reason <reason> [--json]
  vida-legacy task import-jsonl <path> [--json]
  vida-legacy task export-jsonl <path> [--json]
  vida-legacy task list [--status <status>] [--all] [--json]
  vida-legacy task show <task_id> [--json|--jsonl]
  vida-legacy task next-display-id <parent_display_id> [--json]
  vida-legacy task ready [--json]"""
    return 1

  case args[0]
  of "create":
    if args.len < 3:
      echo "Usage: vida-legacy task create <task_id> <title> [--type <issue_type>] [--status <status>] [--priority <n>] [--display-id <display_id>] [--parent-id <task_id>] [--parent-display-id <display_id>] [--auto-display-from <parent_display_id>] [--description <description>] [--labels <label>] [--json]"
      return 1
    var issueType = "task"
    var status = "open"
    var priority = 2
  ...
  of "update":
    if args.len < 2:
      echo "Usage: vida-legacy task update <task_id> [--status <status>] [--notes <notes>] [--description <description>] [--add-label <label>] [--remove-label <label>] [--set-labels <label>] [--json]"
      return 1
    var status = ""
    var notes = ""
    var description = ""
  ...
  of "close":
    if args.len < 4 or args[2] != "--reason":
      echo "Usage: vida-legacy task close <task_id> --reason <reason> [--json]"
      return 1
  ...
  of "import-jsonl":
    if args.len < 2:
      echo "Usage: vida-legacy task import-jsonl <path> [--json]"
      return 1
  ...
  of "export-jsonl":
    if args.len < 2:
      echo "Usage: vida-legacy task export-jsonl <path> [--json]"
      return 1
  ...
  of "show":
    if args.len < 2:
      echo "Usage: vida-legacy task show <task_id> [--json|--jsonl]"
      return 1
  ...
  of "next-display-id":
    if args.len < 2:
      echo "Usage: vida-legacy task next-display-id <parent_display_id> [--json]"
      return 1
...
proc cmdBrCompat*(args: seq[string]): int =
  if args.len == 0:
    echo """Usage:
  vida-legacy br import [path] [--json]
  vida-legacy br export [path] [--json]
...
Defaults:
  ...
  of "export":
    ...
    echo "Usage: vida-legacy br <import|export> [path] [--json]"
    return 1
