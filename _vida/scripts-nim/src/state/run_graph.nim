...
proc cmdRunGraph*(args: seq[string]): int =
  ...
  of "init":
    if args.len < 3:
      echo "Usage: vida-legacy run-graph init <task_id> <task_class> [route_task_class]"
      return 2
  ...
  of "update":
    if args.len < 5:
      echo "Usage: vida-legacy run-graph update <task_id> <task_class> <node> <status> [route_task_class] [meta_json]"
      return 2
    let taskClass = args[2]
    let node = args[3]
    let status = args[4]
