...
proc cmdVerificationPrompt*(args: seq[string]): int =
  if args.len < 5:
    echo "Usage: vida-legacy verification-prompt <original_prompt_file> <task_class> <verification_task_class> <merge_summary_json> <post_arbitration_summary_json> [results_json]"
    return 1
