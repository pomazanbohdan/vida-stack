...
const SubagentEntryDoc* = "_vida/docs/SUBAGENT-ENTRY.MD"
const SubagentThinkingDoc* = "_vida/docs/SUBAGENT-THINKING.MD"
...
proc cmdWorkerPacket*(args: seq[string]): int =
  if args.len < 2:
    echo """Usage:
  vida-legacy worker check <prompt_file|->
  vida-legacy worker check-output <prompt_file|-> <output_file|->"""
    return 1
  ...
  of "check-output":
    if args.len < 3:
      echo "Usage: vida-legacy worker check-output <prompt_file> <output_file>"; return 1
