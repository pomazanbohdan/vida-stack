...
proc cmdAuthGate*(args: seq[string]): int =
  if args.len == 0:
    echo """Usage:
  vida-legacy auth check <task_id> <task_class> [--local-write] [--block-id <id>]
  vida-legacy auth authorize-local <task_id> <task_class> <reason> <scope> <notes> [evidence] [actor]
  vida-legacy auth authorize-internal <task_id> <task_class> <reason> <scope> <notes> [evidence] [actor]
  vida-legacy auth authorize-skip <task_id> <task_class> <reason> <notes> [evidence] [actor]"""
    return 1

  case args[0]
  of "check":
    if args.len < 3:
      echo "Usage: vida-legacy auth check <task_id> <task_class>"; return 1
