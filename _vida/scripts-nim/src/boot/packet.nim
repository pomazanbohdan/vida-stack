...
proc profileReads*(profile: string, nonDev: bool, agentSystemActive: bool,
  ...
  if nonDev:
    result.add("_vida/docs/spec-contract-protocol.md")
  if fileExists(rootDir / "vida.config.yaml"):
    result.add("vida.config.yaml")
  if agentSystemActive:
    result.add("_vida/docs/subagent-system-protocol.md")
...
proc cmdBootPacket*(args: seq[string], rootDir: string): int =
  if args.len == 0:
    echo "Usage: vida-legacy boot <lean|standard|full|read-contract|summary> [args]"
    return 1
  ...
  of "read-contract":
    if args.len < 2:
      echo "Usage: vida-legacy boot read-contract <lean|standard|full> [--non-dev]"
      return 1
  ...
  of "summary":
    if args.len < 2:
      echo "Usage: vida-legacy boot summary <subject>"
      return 1
