...
| `dev-pack` | start/continue implementation | active `br` task, target files | code/test changes + verification |
...
Generic orchestration lifecycle is owned by `_vida/docs/orchestration-protocol.md`.
This file owns pack taxonomy, pack-selection intent, and pack-specific step maps.

1. Detect pack via `bash _vida/scripts/vida-pack-helper.sh detect "<request>"`.
2. Start the selected pack session:
   `bash _vida/scripts/vida-pack-helper.sh start <task_id> <pack_id> "<goal>" [constraints]`.
3. Scaffold the pack-specific TODO plan (optional but recommended):
...
5. End the selected pack session:
   `bash _vida/scripts/vida-pack-helper.sh end <task_id> <pack_id> <done|partial|failed> "<summary>" [next]`.

Shortcut for standard non-dev pack initialization:

```bash
bash _vida/scripts/nondev-pack-init.sh <task_id> <research-pack|spec-pack|work-pool-pack|bug-pool-pack|reflection-pack> "<goal>" [constraints]
```
...
`_vida/docs/spec-contract-protocol.md` is mandatory for all non-dev packs:
...
1. For all non-dev packs, run `_vida/docs/web-validation-protocol.md` when triggers fire.
...
Use `_vida/docs/form-task-protocol.md` as canonical flow:
...
4. `FT04`: Build/Update `br` task pool
...
Use `_vida/docs/implement-execution-protocol.md` as canonical flow:
...
Use unified bug-fix protocol from `_vida/docs/bug-fix-protocol.md`:
