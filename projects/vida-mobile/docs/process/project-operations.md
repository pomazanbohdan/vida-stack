...
This file is the extracted project-level command map for `vida-mobile` (historic project id: `mobile-odoo`).

This copy is kept outside `vida/config/instructions/**` on purpose so framework cleanup can continue without deleting project-owned operational knowledge.
