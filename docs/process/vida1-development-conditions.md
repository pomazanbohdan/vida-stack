...
157. the first `v0.2.2` TaskFlow protocol-binding bridge is now launcher-owned and DB-first over the authoritative state store instead of detached file logs: `vida taskflow protocol-binding sync [--json]` materializes the bounded Wave-1 runtime-bearing subset (`bridge.instruction-activation-protocol`, `work.taskflow-protocol`, `runtime.task-state-telemetry-protocol`, `work.execution-health-check-protocol`, `work.task-state-reconciliation-protocol`) into persisted `protocol_binding_state` and `protocol_binding_receipt` rows, `status`/`check` query that same runtime truth, and root `vida status --json` plus `vida doctor --json` now expose a `protocol_binding` summary surface; the bounded proof is green through `cargo test -p vida`, plus a real booted CLI cycle for `cargo run -p vida -- boot`, `cargo run -p vida -- taskflow protocol-binding sync --json`, `cargo run -p vida -- taskflow protocol-binding status --json`, and `cargo run -p vida -- taskflow protocol-binding check --json`
...
1. `bash dist/vida-install.sh install --archive dist/vida-stack-v0.2.1.tar.gz --root <tmp-root> --bin-dir <tmp-bin> --force`
...
4. passes `bash install/install.sh doctor --root <tmp-root> --bin-dir <tmp-bin>`
...
10. `bash scripts/build-release.sh` currently produces release artifacts that include `bin/taskflow-v0`, `AGENTS.sidecar.md`, and the launcher-owned task-bridge helpers under `taskflow-v0/helpers/turso_task_store.py` and `taskflow-v0/helpers/toon_render.py`; this is proven by inspecting the built `dist/vida-stack-v0.2.1.tar.gz` contents and `dist/vida-stack-v0.2.1.manifest.json`
