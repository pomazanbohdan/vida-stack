...
13. `vida.config.yaml -> agent_system.subagents.internal_subagents.model_profiles`
   - internal Codex App/host-subagent execution profiles for low, medium, high, and xhigh GPT-5.5 lanes plus GPT-5.4 compatibility fallback profiles.
...
4. the active carrier tiers prefer `gpt-5.5` with low, medium, high, and xhigh reasoning bands when the detected Codex CLI satisfies the profile's `readiness.codex_cli.min_version`,
...
8. `agent_system.subagents.internal_subagents` carries GPT-5.5 low, medium, high, and xhigh profiles for Codex App/host-subagent selection, plus GPT-5.4 compatibility fallbacks selected when the Codex CLI preflight rejects GPT-5.5,
