...
27. For Windows local framework development, use the current system tools directly: PowerShell Core from WindowsApps or `VIDA_PWSH`, Rust tools from `rustup`/Cargo, ripgrep from `winget` or an explicit `RG` path, and install the release `vida.exe` only when installed-runtime validation or release admission is the active proof target.
28. Use the Windows proof ladder in cost order: script/docs-only proof through `scripts\vida-dev-gate.cmd -Mode script-check`, cheap source proof through `scripts\vida-dev-gate.cmd -Mode quick`, focused regression proof through `scripts\vida-dev-gate.cmd -Mode focused-nextest -TestFilter <filter>`, package proof through `scripts\vida-dev-gate.cmd -Mode package-nextest`, local workspace proof through `scripts\vida-dev-gate.cmd -Mode workspace-nextest` when the coherent batch is assembled, doc-test proof through `scripts\vida-dev-gate.cmd -Mode doc-test`, debug build proof through `scripts\vida-dev-gate.cmd -Mode build-debug`, debug runtime smoke through `scripts\vida-dev-gate.cmd -Mode runtime-smoke` if current state-store compatibility is needed, release archive proof through `scripts\vida-dev-gate.cmd -Mode release-package`, then installed runtime validation (`vida status` or the exact operator command), and only then `vida release install` for installed launcher or release-admission proof. Treat Windows Application Control failures for generated integration-test binaries under `target\debug\deps\*.exe` or nextest-launched test binaries as host policy blockers unless the policy is changed.
29. After Windows release install, smoke a disposable state root with `vida boot --state-dir <temp-dir>` and `vida status --state-dir <temp-dir> --summary` so SurrealKV filesystem compatibility is proven outside the long-lived project state.
30. For repeatable local gate timing, prefer `scripts\vida-dev-gate.cmd -Mode <mode>` on Windows over ad hoc command chains. Add `-Json` only when a caller explicitly needs machine-readable proof. The current modes are discoverable with `scripts\vida-dev-gate.cmd -Help` and include `script-check`, `quick`, `focused-nextest`, `package-nextest`, `workspace-nextest`, `doc-test`, `build-debug`, `runtime-smoke`, `release-package`, `release-install`, `target-dir-policy`, and `cargo-env-check`.
