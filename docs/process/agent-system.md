...
- host-tool API restrictions that require explicit subagent/delegation permission remain a separate host/user approval boundary; spawn-capable host adapters must fail closed unless the current user instruction or host approval surface explicitly authorizes that host subagent/delegation path for the bounded work
