...
16. For bounded read-only diagnosis, report triage, duplicate-risk review, and second-opinion review, dispatch `vibe_cli` or another configured zero-cost read-only carrier when available; treat the result as advisory evidence until the orchestrator validates it.
