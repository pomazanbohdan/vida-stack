...
15. When root-session token pressure is active, prefer compact runtime surfaces, refreshed TaskFlow snapshots, and targeted `vida task show/children` calls before broad `status --json`, `orchestrator-init --full --json`, or large log reads.
16. For bounded read-only diagnosis, report triage, duplicate-risk review, and second-opinion review, dispatch `vibe_cli` first when available; dispatch `jcode_nim_cli` with `mistralai/mistral-medium-3.5-128b` when an independent NIM-backed advisory pass can reduce root-session analysis calls or when `vibe_cli` is unavailable. Treat both results as advisory evidence until the orchestrator validates them.
17. During task research, evaluate both external delegation modes before root implementation when useful: `external_readonly_complete` for analysis/spec/review/proof-diagnosis reports, and `external_patch_proposal` for patch plans/diffs that root will apply and verify. Record which mode ran, which mode was skipped, and why; high-ambiguity or high-risk tasks may run both in parallel before root synthesis.
18. For write-producing development, try internal low first when the packet owns one clear scope and proof command; move to internal medium for test authoring, ambiguous bounded implementation, or coach gating; reserve high/xhigh for architecture, safety, release, or repeated low/medium failure evidence.
19. After each coherent work pool, record whether root-token usage could have been reduced by earlier advisory prefetch, compact command output, task snapshot refresh, or batching TaskFlow mutations; create or update a TaskFlow optimization item when the answer is yes.
...
1. `script_or_doc_proof`: default for process, docs, and script-only edits that do not need Cargo. Use `scripts\vida-dev-gate.cmd -Mode script-check` on Windows, or equivalent `git diff --check` plus script parser checks on Unix-like hosts.
2. `debug_source_proof`: default for active Rust repair loops. Use `scripts\vida-dev-gate.cmd -Mode quick` on Windows, or the equivalent direct `cargo fmt -p vida -- --check`, `cargo check --locked -p vida`, and `git diff --check` on Unix-like hosts. This is the normal cheap compile-aware proof class for code correctness while a batch is still being assembled. Use `scripts\vida-dev-gate.cmd -Mode focused-nextest -TestFilter <filter>` or direct `cargo nextest run --locked -p vida --profile default <filter>` only when the bounded slice needs a regression test proof.
3. `debug_runtime_smoke`: use `target/debug/vida ...` only after the debug binary proves it can open the current project state with an authoritative read such as `target/debug/vida status`. If the debug binary cannot open the state store, classify that as `debug_runtime_incompatible` and do not use it for runtime closure.
4. `installed_runtime_validation`: use the environment-resolved `vida ...` when the acceptance target is specifically the operator-facing launcher, installed binary path, state compatibility, command timing, downstream project behavior through the user's normal PATH, or an output/help/runtime-state change whose accepted behavior must be visible through the installed CLI.
5. `release_install_gate`: run `vida release install` only for installed-runtime acceptance, output/help/runtime-state surface closure that changes the installed operator CLI, release admission, packaging/installer proof, explicit user order, closure of a TaskFlow wave parent, or when debug runtime smoke is invalid and the current closure must validate the installed launcher. It is not a per-micro-edit proof. Runtime commit/release publication proof must use `vida release install --require-clean-worktree` from the intended clean commit/worktree; dirty status fails closed with `dirty_worktree_release_proof_required`.
...
When a release install is considered, first record why `debug_source_proof` and, when applicable, `debug_runtime_smoke` are insufficient. A closed wave parent is sufficient reason because the operator-facing system binary must advance to the closed wave state. Output, default/JSON/help, or runtime-state changes are sufficient only when the task acceptance claims operator-visible installed CLI behavior; the proof must include the installed `vida --version` path check plus a bounded smoke through the changed command or `vida status` on the normal PATH. If the reason is only "the code changed" inside an unfinished task or micro-edit, use the debug proof class instead.
...
10. For VIDA runtime recovery diagnostics, prefer the fastest authoritative inspection surface that exposes the needed evidence. If a timeout/recovery path only needs task metadata or current owned scope, use `vida task show <task-id>` before heavier lane or run-graph projections.
...
12. Prefer `scripts/vida-dev-gate.ps1` for local Windows proof loops that need consistent timing records and deterministic Cargo cache behavior. Use `-Mode script-check` for no-Cargo diff/script proof, `-Mode quick` for cheap source proof through diff check, fmt, and `cargo check`, `-Mode focused-nextest -TestFilter <filter>` for bounded regression proof, `-Mode package-nextest` for the full `vida` package nextest proof, `-Mode workspace-nextest` for a log-backed local workspace test gate, `-Mode doc-test` for Rust doc tests, `-Mode build-debug` for debug runtime entrypoint builds, `-Mode runtime-smoke` for debug-runtime state compatibility, `-Mode release-package -SkipBuild -Windows -ReleaseBinDir <dir>` for native PowerShell release archive packaging from existing Windows release binaries, and `-Mode release-install` only when installed runtime validation is the bounded acceptance target. Use `-Mode target-dir-policy` as the cheap policy probe before running Cargo from a new linked worktree. Pass `-Jobs <n>` only when the local machine or task class needs an explicit nextest concurrency cap. Add `-Json` only when the caller explicitly needs machine-readable timing records; with `-Json`, every operation record must include `target_dir_policy` and `effective_cargo_target_dir`.
13. For Windows `cargo llvm-cov` or other proof gates that invoke MSVC C/C++ compilation from an agent shell, first restore the full Windows process environment. At minimum set `SystemRoot=C:\Windows`, `windir=C:\Windows`, `ComSpec=C:\Windows\System32\cmd.exe`, a writable `TEMP`/`TMP`, MSVC `bin\Hostx64\x64` on `PATH`, Windows SDK `bin\<version>\x64` on `PATH`, `C:\Program Files\PowerShell\7`, `C:\Users\pomaz\AppData\Local\Microsoft\WindowsApps`, `C:\Windows\System32\WindowsPowerShell\v1.0`, and explicit `LIB`/`INCLUDE` for the selected MSVC and SDK versions. Use the absolute Cargo path (`C:\Users\pomaz\.cargo\bin\cargo.exe`) to avoid shell wrappers shadowing `cargo`, and verify `where.exe pwsh` in the same shell when tests can spawn PowerShell. If `cl.exe` reports `D8037`, first inspect `%TMP%` for `_CL_*.ss` files as Microsoft documents, then verify these Windows environment variables are present in the current process before reinstalling Build Tools.
...
1. `vida orchestrator-init` observed around `22000 ms` through the local command wrapper during re-entry.
2. `vida task next-lawful` observed around `17000 ms`.
...
6. `vida release install` took about 84 seconds in the 2026-06-01 bridge-adapter repair loop. The release install was useful only for installed-runtime validation; subsequent debug runtime smoke showed `target/debug/vida status` can be a faster validation step when the debug binary is state-store compatible.
...
9. A cold local `scripts/vida-dev-gate.ps1 -Mode quick` run spent about `79758 ms` in `cargo check --locked -p vida`; this is acceptable only as compile-aware source proof. Docs/script-only edits must use `-Mode script-check` first so local proof does not pay a Cargo compile cost unnecessarily.
...
19. The 2026-06-18 cargo CRAP baseline proof showed that `cargo llvm-cov --workspace --lcov --output-path .vida/tmp/cargo-crap-workspace.lcov` can fail in stripped Windows agent shells before reaching Rust tests. Initial failures were `link.exe` missing from `PATH`, then MSVC `cl.exe` D8037 (`cannot create temporary il file`) even with clean temp directories. The successful environment repair was not a blind reinstall: restore `SystemRoot`, `windir`, `ComSpec`, `TEMP`, `TMP`, MSVC `PATH`, Windows SDK `PATH`, PowerShell/WindowsApps PATH entries, `LIB`, and `INCLUDE`, then invoke absolute Cargo. On this host, `C:\Users\pomaz\AppData\Local\Microsoft\WindowsApps` is required because `pwsh.exe` can resolve through the WindowsApps app alias. With that environment, the gate compiled and progressed to the current `vida` test failure: `target\llvm-cov-target\debug\deps\vida-ce48d76d1bf4dd0e.exe` exited `0xc0000409, STATUS_STACK_BUFFER_OVERRUN`, so subsequent CRAP baseline work should start from that Rust/test failure rather than the older MSVC setup error.
