...
17. During task research, choose one external delegation mode before root implementation when useful: `external_readonly_complete` for analysis/spec/review/proof-diagnosis reports, or `external_patch_proposal` for patch plans/diffs that root will apply and verify.
