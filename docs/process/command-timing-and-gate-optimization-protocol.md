...
19. For bounded read-only diagnosis, report triage, duplicate-risk review, and second-opinion review, use a config-selected read-only/medium capability by default; use a separately configured advisory carrier only when host authorization and runtime admission select it. Treat advisory results as evidence until the orchestrator validates them.
...
17. For runtime fixes, preserve focused regression tests → compatible debug build → live debug `status`/reconcile/`doctor` proof → one final release-install when state-compatible and safe; do not install per defect.
18. Start long build/release/install gates log-backed, recording PID, cwd, target-dir, and stdout/stderr artifact refs before waiting; inspect the original child before retrying and never blind-rerun a live process.
19. Use count/status/blocker selectors for row-list commands by default; require an explicit details/full mode for unbounded rows.
20. PowerShell helpers must use descriptive names and never shadow automatic variables such as `$Args`.
21. Treat mutation success as unproven until a selector-bounded read-back verifies the intended rows/fields.
