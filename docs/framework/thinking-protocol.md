...
2. Boot/read activation is owned by `docs/framework/instruction-activation-protocol.md` and `docs/framework/ORCHESTRATOR-ENTRY.MD`.
3. Web/internet validation is owned by `docs/framework/web-validation-protocol.md`.
...
STEPS:
  ...
  6. If external facts affect the decision, delegate web validation to `docs/framework/web-validation-protocol.md`
...
```yaml
PREREQ: docs/framework/thinking-protocol.md#section-algorithm-selector  # Understand when STC applies (Score ≤12)
```
...
```yaml
PREREQ: docs/framework/thinking-protocol.md#section-algorithm-selector  # Understand when PR-CoT applies (Score 13-22)
```
...
```yaml
PREREQ: [docs/framework/thinking-protocol.md#section-algorithm-selector, docs/framework/thinking-protocol.md#section-pr-cot]  # MAR builds on PR-CoT
```
...
ACTION: Select 2-3 modules from `docs/framework/thinking-protocol.md#section-reasoning-modules` → inform categories
MAPPING:
...
```yaml
PREREQ: [docs/framework/thinking-protocol.md#section-algorithm-selector, docs/framework/thinking-protocol.md#section-reasoning-modules]  # META composes from the block registry and domain packets
```
...
> **⛔ MANDATORY.** Canonical web/internet validation rules live in `docs/framework/web-validation-protocol.md`.
...
ANALYSIS:
  ...
  root_cause:
    ...
    vida_mapping: `docs/framework/thinking-protocol.md#section-bug-reasoning`, MAR Round 1
```
