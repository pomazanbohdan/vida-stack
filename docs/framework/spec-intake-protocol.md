...
1. Canonical intake layer before `docs/framework/spec-contract-protocol.md`.
2. Canonical intake layer before `docs/framework/issue-contract-protocol.md` when the incoming issue/release signal is still ambiguous or scope-bearing.
...
1. `ready_for_scp` -> `docs/framework/spec-contract-protocol.md`
2. `ready_for_issue_contract` -> `docs/framework/issue-contract-protocol.md`
3. `needs_user_negotiation` -> stay in spec-intake / SCP discovery until open decisions are resolved
4. `needs_spec_delta` -> `docs/framework/spec-delta-protocol.md`
