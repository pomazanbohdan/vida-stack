...
3. Used by `docs/framework/thinking-protocol.md` and `docs/framework/spec-contract-protocol.md` as single source.
...
When a WVP trigger fired, include a compact evidence block in TODO logs (`block-end` evidence or `reflect` evidence):
...
1. `docs/framework/thinking-protocol.md#section-web-search`: router-level trigger map and algorithm integration.
2. `docs/framework/spec-contract-protocol.md`: SCP gates and weighted readiness model.
3. `docs/framework/implement-execution-protocol.md`: execution-time validation before code decisions.
