...
6. hand off execution only to `docs/framework/implement-execution-protocol.md`.
...
1.1. Equivalent bugfix paths may use approved `issue_contract` input from `docs/framework/issue-contract-protocol.md` instead of a longer SCP artifact when the scope is already bounded.
...
6. WVP evidence for external assumptions (`docs/framework/web-validation-protocol.md`).
...
1. `/vida-implement` must run by `docs/framework/implement-execution-protocol.md` only.
