...
import std/json
import ./[agent_inventory, assignment_engine, instruction_engine, kernel_config, projection_engine, transition_engine, toon, utils]

proc printHelp() =
  echo """
Usage:
  vida-legacy kernel summary
  vida-legacy kernel machine <name> [--json]
  vida-legacy kernel route <name> [--json]
  vida-legacy kernel agents [--json]
  vida-legacy kernel instruction <role> [machine] [route] [--json]
  vida-legacy kernel policy <name> [--json]
  vida-legacy kernel assign lane <lane> [context_json] [--json]
  vida-legacy kernel assign task-class <task_class> [context_json] [--json]
  vida-legacy kernel transition <machine> <current_state> <command> [context_json]
  vida-legacy kernel project transition <machine> <current_state> <command> [context_json] [--json]
  vida-legacy kernel project route <task_class> [task_id] [--json]
"""
...
proc cmdKernel*(args: seq[string]): int =
  ...
  of "machine":
    if args.len < 2:
      echo "Usage: vida-legacy kernel machine <name> [--json]"
      return 1
  ...
  of "route":
    if args.len < 2:
      echo "Usage: vida-legacy kernel route <name> [--json]"
      return 1
  ...
  of "policy":
    if args.len < 2:
      echo "Usage: vida-legacy kernel policy <name> [--json]"
      return 1
  ...
  of "instruction":
    if args.len < 2:
      echo "Usage: vida-legacy kernel instruction <role> [machine] [route] [--json]"
      return 1
    let machineName =
  ...
  of "assign":
    if args.len < 3:
      echo "Usage: vida-legacy kernel assign <lane|task-class> <name> [context_json] [--json]"
      return 1
    let ctx =
  ...
  of "transition":
    if args.len < 4:
      echo "Usage: vida-legacy kernel transition <machine> <current_state> <command> [context_json]"
      return 1
  ...
  of "project":
    if args.len < 2:
      echo "Usage: vida-legacy kernel project <transition|route> ..."
      return 1
    case args[1]
    of "transition":
      if args.len < 5:
        echo "Usage: vida-legacy kernel project transition <machine> <current_state> <command> [context_json] [--json]"
        return 1
    ...
    of "route":
      if args.len < 3:
        echo "Usage: vida-legacy kernel project route <task_class> [task_id] [--json]"
        return 1
      let taskId =
        if args.len >= 4 and args[3] != "--json": args[3]
        else: ""
