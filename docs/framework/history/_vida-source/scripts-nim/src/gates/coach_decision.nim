...
proc cmdCoachDecision*(args: seq[string]): int =
  if args.len == 0:
    echo """Usage:
  vida-legacy coach-decision parse <output_file|->
  vida-legacy coach-decision merge <required_results> <merge_policy> <decisions_json_file|->"""
    return 1

  case args[0]
  of "parse":
    if args.len < 2:
      echo "Usage: vida-legacy coach-decision parse <output_file|->"
      return 1
  ...
  of "merge":
    if args.len < 4:
      echo "Usage: vida-legacy coach-decision merge <required_results> <merge_policy> <decisions_json_file|->"
      return 1
