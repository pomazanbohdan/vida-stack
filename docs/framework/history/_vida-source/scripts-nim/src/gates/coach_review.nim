...
proc cmdCoachGate*(args: seq[string]): int =
  if args.len == 0:
    echo """Usage:
  vida-legacy coach check <task_id>
  vida-legacy coach authorize-skip <task_id> <reason> <notes> [evidence] [actor]"""
    return 1

  case args[0]
  of "check":
    if args.len < 2: echo "Usage: vida-legacy coach check <task_id>"; return 1
  ...
  of "authorize-skip":
    if args.len < 4:
      echo "Usage: vida-legacy coach authorize-skip <task_id> <reason> <notes>"; return 1
