...
proc buildCapsulePayload(taskId, doneText, nextStep, risks, acceptanceSlice, constraints, taskRole: string): JsonNode =
  ...
  %*{
    ...
    "done": doneText,
    "next": nextStep,
    "constraints": (if constraints.len > 0: %*[constraints] else: %*["follow-L0-invariants", "legacy-zero", "vida-legacy-task-store"]),
...
proc cmdContextCapsule*(args: seq[string]): int =
  if args.len == 0:
    echo """Usage:
  vida-legacy context-capsule write <task_id> <done> <next> [risks] [acceptance_slice] [constraints] [task_role]
  vida-legacy context-capsule read <task_id> [--json]
  vida-legacy context-capsule hydrate <task_id> [--json]"""
    return 1

  case args[0]
  of "write":
    if args.len < 4:
      echo "Usage: vida-legacy context-capsule write <task_id> <done> <next> [risks] [acceptance_slice] [constraints] [task_role]"
      return 1
    let payload = writeCapsule(
      args[1],
      args[2],
  ...
  of "read":
    if args.len < 2:
      echo "Usage: vida-legacy context-capsule read <task_id> [--json]"
      return 1
  ...
  of "hydrate":
    if args.len < 2:
      echo "Usage: vida-legacy context-capsule hydrate <task_id> [--json]"
      return 1
