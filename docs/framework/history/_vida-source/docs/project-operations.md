...
Framework/runtime policy stays in `_vida/docs/*`. App-specific operational commands stay here and in `scripts/`.
...
2. Follow the project-specific preflight and execution order documented here; keep project sequencing rules out of `_vida/*`.
...
2. If a command enforces VIDA runtime protocol, keep it in `_vida/scripts/` and reference it from `_vida/docs/`.
