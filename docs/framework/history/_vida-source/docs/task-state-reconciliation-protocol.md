...
2. TODO remains SSOT for execution telemetry.
...
2. `todo-tool.sh ui-json <task_id>`
...
1. `active`
   - task has an active TODO block or resumable run-graph node.
2. `blocked`
   - task has a blocked TODO block.
3. `done_ready_to_close`
   - task is still `in_progress`, but TODO has no active backlog and verification evidence is already valid.
4. `stale_in_progress`
...
5. `open_but_satisfied`
   - task remains `open`, but TODO/verification evidence indicates the scoped work is already satisfied.
6. `drift_detected`
   - `br`, TODO, and verification artifacts disagree in a way that requires explicit reconciliation.
7. `invalid_state`
   - task is structurally contradictory, for example `closed` with active TODO backlog.
8. `closed`
...
```bash
python3 _vida/scripts/task-state-reconcile.py status <task_id>
```
