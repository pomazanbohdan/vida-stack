...
Purpose: define the canonical runtime contract for agent definitions in VIDA.
...
4. `Prompt Template Configuration` is the rendering/configuration layer only.
5. Provider-specific prompt/config data must not silently become the source of truth for role behavior.

## Canonical Artifact Set

Required runtime artifacts:

1. `_vida/docs/agent-definition-protocol.md`
2. `_vida/templates/instruction-contract.yaml`
3. `_vida/templates/prompt-template-config.yaml`
4. `_vida/tests/test_agent_definition_contract.py`

Upstream supporting artifacts:

1. `_vida/docs/research/2026-03-08-agentic-agent-definition-system.md`
2. `_vida/docs/research/2026-03-08-agentic-terminology-glossary.md`
3. `_vida/docs/role-profile-protocol.md` when role profiles are implemented
...
3. `Prompt Template Configuration` renders the contract for a runtime/provider.
4. `Role Profile` remains upstream identity/stance input, not the full runtime behavior object by itself.
...
4. output contract and proof requirements remain inspectable after rendering

Compact formula:

`agent_definition = role_profile + instruction_contract + prompt_template_configuration + permission policy + output/proof contract`

## Validation Expectations

At minimum, the framework must be able to validate:

1. required fields exist in the instruction contract template
2. required fields exist in the prompt template configuration template
3. terminology hierarchy is not inverted
4. deterministic sections are present
...
4. add conformance checks
5. then allow the role into routed execution surfaces
...
1. instruction contract template exists
2. prompt template configuration template exists
