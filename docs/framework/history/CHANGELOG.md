...
1. Recalibrated the canonical selector bands in [thinking-protocol.md](/home/unnamed/project/vida-stack/docs/framework/history/_vida-source/docs/thinking-protocol.md) to `STC <=12`, `PR-CoT 13-22`, `MAR 23-32`, `5-SOL 33-42`, and `META >42`, so medium/high-governance work escalates earlier without making `META` the default for simple execution.
2. Added explicit `META` routing overrides in [thinking-protocol.md](/home/unnamed/project/vida-stack/docs/framework/history/_vida-source/docs/thinking-protocol.md) for framework-owned behavior changes, protocol conflicts, execution gate mismatch, fail-closed law risk, and tracked writer `no_eligible_*` routing gaps.
3. Added canonical `RISK_ESCALATORS` and `RETROSPECTIVE_ESCALATION` rules in [thinking-protocol.md](/home/unnamed/project/vida-stack/docs/framework/history/_vida-source/docs/thinking-protocol.md), including confirmed `STC` misfire handling so the same task class does not silently repeat under too-weak routing.
4. Synced the operational summary in [algorithms-quick-reference.md](/home/unnamed/project/vida-stack/docs/framework/history/_vida-source/docs/algorithms-quick-reference.md) to the new selector bands and escalation rules.
...
6. Added focused regression coverage in [test_execution_auth_gate.py](/home/unnamed/project/vida-stack/docs/framework/history/_vida-source/tests/test_execution_auth_gate.py) and clarified the fail-closed exception in [implement-execution-protocol.md](/home/unnamed/project/vida-stack/docs/framework/history/_vida-source/docs/implement-execution-protocol.md).
7. Tightened `execution-auth-gate.py` so structured execution-auth overrides are fail-fast and limited to framework-labeled `no_eligible_analysis_lane` cases, and synced the compact matrix in [algorithms-one-screen.md](/home/unnamed/project/vida-stack/docs/framework/history/_vida-source/docs/algorithms-one-screen.md).

## 2026-03-08 02:11

Fixed:

1. [quality-health-check.sh](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/quality-health-check.sh) now runs execution-auth validation only for real `dev-pack` implementation context, not for reflection/docs tasks that merely share writer-like block ids or retain stray implementation receipts.
2. Added regression coverage in [test_beads_runtime.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/tests/test_beads_runtime.py) so this health-check classification path stays pinned to execution context instead of block-id or receipt-only heuristics.
...
1. Added [instruction-activation-protocol.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/instruction-activation-protocol.md) as the canonical framework surface for phase-based instruction loading, trigger-only protocol activation, naming guidance, and instruction-layer decomposition rules.
...
2. [ORCHESTRATOR-ENTRY.MD](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/ORCHESTRATOR-ENTRY.MD) now treats instruction loading as a triggered decision bound to the new activation protocol instead of broadening the boot read-set by implication.
3. [protocol-index.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/protocol-index.md) now indexes instruction activation/decomposition as a canonical framework domain.
...
2. Added regression coverage in [test_beads_runtime.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/tests/test_beads_runtime.py) for the pending-capable bootstrap hydration contract.
...
4. Added task-state reconciliation via [task-state-reconciliation-protocol.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/task-state-reconciliation-protocol.md) and [task-state-reconcile.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/task-state-reconcile.py), and surfaced the derived classification in health/operator/boot flows.

## 2026-03-07 21:34

Added:

1. Added local trace-grading protocol and helper via [trace-eval-protocol.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/trace-eval-protocol.md) and [trace-eval.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/trace-eval.py).
2. Added focused regression coverage for trace grading and dataset export in [test_trace_eval.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/tests/test_trace_eval.py).
3. Added typed capability registry protocol and helper via [capability-registry-protocol.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/capability-registry-protocol.md) and [capability-registry.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/capability-registry.py).
...
1. Added durable run-graph protocol and helper via [run-graph-protocol.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/run-graph-protocol.md) and [run-graph.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/run-graph.py) so routed execution now has a canonical node-level resumability ledger under `.vida/state/run-graphs/`.
...
1. Added [spec-intake-protocol.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/spec-intake-protocol.md) and [spec-intake.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/spec-intake.py) as the new compact normalization layer for mixed research, issue/release signals, and user-scope negotiation before SCP/ICP/FTP.
2. Added focused tests for the new intake helper in [test_spec_intake.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/tests/test_spec_intake.py).
3. Added [spec-delta-protocol.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/spec-delta-protocol.md), [spec-delta.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/spec-delta.py), and focused tests in [test_spec_delta.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/tests/test_spec_delta.py) for explicit non-equivalent contract reconciliation.
...
5. Added [draft-execution-spec.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/draft-execution-spec.py) and focused tests in [test_draft_execution_spec.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/tests/test_draft_execution_spec.py) so the draft execution-spec contract has a canonical helper surface instead of living only in prose.

## 2026-03-07 20:42

Added:

1. Added [future.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/future.md) to separate out-of-environment platform roadmap items from locally remaining non-Rust VIDA work.
...
1. Added first-class human approval receipt helper via [human-approval-gate.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/human-approval-gate.py).
2. Added canonical approval governance protocol via [human-approval-protocol.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/human-approval-protocol.md).
3. Added framework memory ledger via [framework-memory.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/framework-memory.py) and [framework-memory-protocol.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/framework-memory-protocol.md).
4. Added document lifecycle ledger and validator via [doc-lifecycle.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/doc-lifecycle.py) and [document-lifecycle-protocol.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/document-lifecycle-protocol.md).
5. Added aggregated operator visibility surface via [framework-operator-status.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/framework-operator-status.py).
...
1. Added bounded `problem-party` protocol and helper for optional multi-role conflict discussion via [problem-party-protocol.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/problem-party-protocol.md) and [problem-party.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/problem-party.py).
...
3. Added declarative framework wave task-state reconciliation through [framework-task-sync.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/framework-task-sync.py).
...
1. Added queue-backed single-writer task-state mutation path through [br-mutation-queue.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/br-mutation-queue.py) and wired mutating `br`/`beads_mutate` calls through the canonical serialized runtime path.
2. Added silent VIDA framework self-diagnosis mode with root overlay support, boot visibility, deferred framework bug capture, and session reflection via [vida-silent-diagnosis.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/vida-silent-diagnosis.py).
3. Added reusable leased subagent pool helper in [subagent-pool.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/subagent-pool.py).
4. Added reusable product/framework proving-pack templates in [proving-pack.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/proving-pack.py).
...
1. `docs/framework/history/_vida-source/scripts/vida-boot-snapshot.py` as a compact dev-boot status surface that renders top-level active work, `ready_head`, `decision_required`, and open/in-progress `parent-child` subtask trees.
...
1. `docs/framework/history/_vida-source/docs/ORCHESTRATOR-ENTRY.MD` as the canonical L0 orchestrator contract replacing the old monolithic `AGENTS.md` body.
...
4. `vida.config.yaml` and `docs/framework/templates/vida.config.yaml.template` now include per-cli-subagent timeout profiles aligned with observed runtime behavior, including a longer profile for `gemini_cli`.
...
1. `docs/framework/templates/vida.config.yaml.template` now mirrors the canonical VIDA provider stack instead of a generic single-provider example.
...
1. `docs/framework/history/_vida-source/docs/SUBAGENT-THINKING.MD` as the worker-lane thinking subset limited to `STC`, `PR-CoT`, and `MAR`.

Changed:

1. Worker-lane prompts now inject both entry and thinking contracts through `docs/framework/history/_vida-source/scripts/render-subagent-prompt.sh`.
...
1. `docs/framework/history/CHANGELOG.md` as the canonical framework change log.

Changed:

1. `docs/framework/templates/vida.config.yaml.template` to reflect the real agent-system shape:
   `senior_internal`, `external_free`, `cost_priority`, `dispatch.env`, runtime budget fields, and fanout metadata examples.
2. `docs/framework/history/_vida-source/docs/protocol-index.md` to link the framework change log.

## 2026-03-06 23:55

Added:

1. `docs/framework/history/_vida-source/docs/SUBAGENT-ENTRY.MD` as the worker-lane entry contract.

Changed:

1. `docs/framework/history/_vida-source/docs/subagents.md` to separate orchestrator entry from worker entry.
2. `docs/framework/history/_vida-source/docs/subagent-prompt-templates.md` so external workers receive bounded worker semantics instead of orchestrator identity.
3. `docs/framework/history/_vida-source/scripts/render-subagent-prompt.sh` to inject `Worker Entry Contract` into canonical rendered prompts.
...
1. Fixed [quality-health-check.sh](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/quality-health-check.sh) so execution-auth validation runs only for real `dev-pack` implementation context, not for reflection/docs tasks that merely share block ids or stray implementation receipts.
1. Expanded [framework-operator-status.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/framework-operator-status.py) to surface route rationale, estimated route cost classes, silent-diagnosis backlog, recent framework reflections, anomaly clusters, and suspicious run-graph artifacts.
2. Updated [framework-memory-protocol.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/framework-memory-protocol.md), [context-governance-protocol.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/context-governance-protocol.md), and [run-graph-protocol.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/run-graph-protocol.md) to align canonical ledgers with the richer operator surface.
3. Added env-driven run-graph state-dir resolution in [run-graph.py](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/run-graph.py) and switched focused tests to explicit isolated overrides so run-graph tests no longer pollute real `.vida/state/run-graphs`.
4. Hardened [framework-wave-start.sh](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/framework-wave-start.sh) and [quality-health-check.sh](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/quality-health-check.sh) with JSONL-safe task label fallback when `br show` is degraded by malformed SQLite state.
5. Hardened [beads-workflow.sh](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/scripts/beads-workflow.sh) with named optional tail flags for `block-end` and `block-finish`, plus a `parse-block-tail` diagnostic command, so telemetry fields no longer depend on brittle positional shifting.
6. Tightened subagent-first orchestration law in [AGENTS.md](/home/unnamed/project/mobile-odoo/AGENTS.md), [ORCHESTRATOR-ENTRY.MD](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/ORCHESTRATOR-ENTRY.MD), and [subagent-system-protocol.md](/home/unnamed/project/mobile-odoo/docs/framework/history/_vida-source/docs/subagent-system-protocol.md): agent/thread saturation now requires `reuse existing eligible agents first` before local-only continuation.
