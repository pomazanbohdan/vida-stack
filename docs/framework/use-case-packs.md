...
Generic orchestration lifecycle is owned by `docs/framework/orchestration-protocol.md`.
...
`docs/framework/spec-contract-protocol.md` is mandatory for all non-dev packs:
...
1. For all non-dev packs, run `docs/framework/web-validation-protocol.md` when triggers fire.
...
Use `docs/framework/form-task-protocol.md` as canonical flow:
...
Use `docs/framework/implement-execution-protocol.md` as canonical flow:
...
Use unified bug-fix protocol from `docs/framework/bug-fix-protocol.md`:
...
4. Explicit VIDA/framework diagnosis requests route through `reflection-pack` and `docs/framework/framework-self-analysis-protocol.md`.
...
1. `vida-pack-helper` and `nondev-pack-init` are explicitly migration-only wrapper surfaces per `docs/framework/runtime-transition-map.md`.
