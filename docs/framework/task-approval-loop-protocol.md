...
3. While suspended, next-task analysis must still happen under `docs/framework/autonomous-execution-protocol.md`; the suspension removes the wait, not the analysis discipline.
...
5. When suspended, `docs/framework/autonomous-execution-protocol.md` owns between-task continuation behavior.
...
7. This protocol must not suppress the default boundary analysis/report/update behavior owned by `docs/framework/autonomous-execution-protocol.md`; it may only add or remove the approval wait.
