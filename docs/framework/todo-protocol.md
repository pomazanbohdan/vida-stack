...
1. When task lifecycle state and execution telemetry appear out of sync, use `docs/framework/task-state-reconciliation-protocol.md` as the canonical reconciliation layer before closing, reopening, or declaring the task stale.
...
3. wrapper retirement is tracked by `docs/framework/runtime-transition-map.md`.
...
bash docs/framework/history/_vida-source/scripts/todo-tool.sh compact <task_id> [limit]
bash docs/framework/history/_vida-source/scripts/todo-tool.sh list <task_id>
bash docs/framework/history/_vida-source/scripts/todo-tool.sh current <task_id>
bash docs/framework/history/_vida-source/scripts/todo-tool.sh next <task_id>
bash docs/framework/history/_vida-source/scripts/todo-tool.sh ui-json <task_id>
bash docs/framework/history/_vida-source/scripts/todo-sync-plan.sh <task_id>
bash docs/framework/history/_vida-source/scripts/todo-sync-plan.sh <task_id> --mode compact --max-items 3
bash docs/framework/history/_vida-source/scripts/todo-sync-plan.sh <task_id> --mode delta
bash docs/framework/history/_vida-source/scripts/todo-sync-plan.sh <task_id> --mode json-only --quiet
bash docs/framework/history/_vida-source/scripts/todo-overhead-report.sh <task_id>
...
11. When more than one next task/block appears lawful, apply `docs/framework/execution-priority-protocol.md` before choosing or redirecting.
...
1. TODO UI reads from `docs/framework/history/_vida-source/scripts/todo-tool.sh ui-json <task_id>`.
...
4. Use `docs/framework/history/_vida-source/scripts/todo-sync-plan.sh <task_id>` to generate a deterministic checklist snapshot for UI mirroring.
5. `docs/framework/history/_vida-source/scripts/beads-workflow.sh` auto-runs `todo-sync-plan.sh` in `json-only` mode according to `TODO_AUTO_SYNC_LEVEL`.
...
12. Use `python3 docs/framework/history/_vida-source/scripts/task-state-reconcile.py status <task_id>` when a task looks done-but-open, stale-in-progress, or otherwise drifted between `br` and TODO.
...
0.2. If a task enters bounded conflict-discussion mode via `docs/framework/problem-party-protocol.md`, record the board artifact path in block evidence before resuming normal execution.
1. Step gate: `block-end` requires evidence or artifacts.
1.1. If WVP trigger fired, `block-end`/`reflect` evidence must include WVP markers per `docs/framework/web-validation-protocol.md`.
