...
4. Overlay is project-owned data; framework-owned behavior stays in `AGENTS.md`, `docs/framework/*`, and `vida-v0/*`.

No-overlay execution rule:

1. If `vida.config.yaml` is absent, VIDA must use only framework-owned canonical commands and wrappers declared in `AGENTS.md`, `docs/framework/protocol-index.md`, and `vida-v0/*`.
...
4. project docs referenced from `project_bootstrap` remain project-owned runbooks; framework runtime law must stay in `AGENTS.md`, `docs/framework/*`, and `vida-v0/*`, not be synchronized into project docs by drift.
5. framework-owned document lifecycle/freshness metadata belongs in `docs/framework/document-lifecycle-protocol.md` and `.vida/state/doc-lifecycle.json`, not inside project-owned docs.

## Language Policy

Framework-owned rule:

1. `AGENTS.md`, `docs/framework/*`, and `vida-v0/*` use English as the framework language.
...
4. Approval gating still belongs to `docs/framework/task-approval-loop-protocol.md`.
