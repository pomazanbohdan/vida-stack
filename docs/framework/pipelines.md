...
3. If project operational guidance changes, update `docs/process/*` and `scripts/*`, not `docs/framework/*`.
...
3. If analyzer/build/test behavior depends on project environment preparation, the canonical sequence must be documented in project runbooks, not `docs/framework/*`.
...
4. Full MCP search guide: `docs/framework/tooling.md`.
...
```bash
docs/framework/script-runtime-architecture.md
```
...
```bash
docs/framework/self-reflection-protocol.md
```
...
1. `vida-pack-helper.sh` remains a migration-only helper per `docs/framework/runtime-transition-map.md`.
