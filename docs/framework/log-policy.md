...
1. `AGENTS.md`
2. `docs/framework/*.md` protocol documents
...
1. TODO runtime scripts must minimize non-essential console chatter by default.
2. Keep machine-readable output stable on `stdout` (`todo-tool ui-json`, `list`, `current`, `next`).
3. Keep user-facing diagnostics in `quality-health-check.sh` and explicit status commands.
4. Use explicit verbose mode (`VIDA_TODO_VERBOSE=1`) only for debugging.
