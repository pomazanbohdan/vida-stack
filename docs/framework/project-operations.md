...
Framework/runtime policy stays in `docs/framework/*`. App-specific operational commands stay here and in `scripts/`.
...
2. If a command enforces VIDA runtime protocol, keep it in `docs/framework/history/_vida-source/scripts/` and reference it from `docs/framework/`.
