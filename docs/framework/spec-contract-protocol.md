...
3. Raw issue text for bugfix execution is normalized by `docs/framework/issue-contract-protocol.md` instead of running full SCP by default.
4. Raw research findings, release signals, and user clarification should first be normalized by `docs/framework/spec-intake-protocol.md` when they are still ambiguous, mixed, or scope-bearing.
...
0. `SCP-0 Pre-Intake Normalization`
   - when raw inputs are still ambiguous or mixed, normalize them through `docs/framework/spec-intake-protocol.md`,
...
4. `SCP-3 External Reality Validation`
   - Execute `docs/framework/web-validation-protocol.md` (WVP) as canonical validation flow.
...
Canonical rules are defined in `docs/framework/web-validation-protocol.md`.
...
Artifact templates: `docs/framework/spec-contract-artifacts.md`
