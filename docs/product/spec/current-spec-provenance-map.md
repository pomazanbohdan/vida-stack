...
3. [development-flow-catalog-schema-design.md](development-flow-catalog-schema-design.md)
   Sources: `vida.config.yaml`, `docs/framework/templates/vida.config.yaml.template`, `docs/process/agent-extensions/flows.yaml`, `docs/product/spec/hook-templates.yaml`, `docs/product/spec/host-agent-bridge-adapter-contract-design.md`, current flow projection code in `crates/vida/src/{taskflow_consume_bundle,agent_dispatch_surface,development_flow_orchestration}.rs`, OpenAI Codex CLI repository, Vibe Kanban agent/profile docs, Claude Code subagent docs, Pi sub-agent package/docs, and OpenCode agent docs
4. [add-workflow-policy-loader-service-orchestrated-design.md](add-workflow-policy-loader-service-orchestrated-design.md)
   Sources: active TaskFlow host-bridge request for `vida-symphony-workflow-md-policy-loader`, internal `multi_agent_v1` specification evidence returned on 2026-06-02, `vida.config.yaml`, `docs/process/agent-extensions/flows.yaml`, `docs/product/spec/development-flow-catalog-schema-design.md`, `docs/product/spec/host-agent-bridge-adapter-contract-design.md`, and current runtime/TUI requirement to derive service-orchestrated flow behavior from configuration rather than hardcoded command sequences
...
22. [codex-host-agent-boundary-and-cli-bridge-design.md](codex-host-agent-boundary-and-cli-bridge-design.md)
    Sources: `vida.config.yaml`, `docs/framework/templates/vida.config.yaml.template`, `docs/process/agent-system.md`, `docs/process/codex-agent-configuration-guide.md`, `docs/product/spec/hybrid-host-executor-semantics-host-environment-design.md`, current runtime dispatch/status code in `crates/vida/src/{runtime_dispatch_execution,runtime_dispatch_state,status_surface_host_agents}.rs`, and the active architecture finding that parent-session Codex host tools are not equivalent to child-process `codex exec`
23. [host-agent-bridge-adapter-contract-design.md](host-agent-bridge-adapter-contract-design.md)
    Sources: `vida.config.yaml`, `docs/framework/templates/vida.config.yaml.template`, `docs/process/agent-system.md`, `docs/process/project-orchestrator-operating-protocol.md`, `docs/product/spec/codex-host-agent-boundary-and-cli-bridge-design.md`, `docs/product/spec/hybrid-host-executor-semantics-host-environment-design.md`, current runtime dispatch/lane code in `crates/vida/src/{runtime_dispatch_execution,lane_surface}.rs`, OpenAI Codex CLI repository, Vibe Kanban supported-agent/profile docs, Claude Code subagent docs, Pi sub-agent package/docs, and OpenCode agent docs
24. [vida-coder-service-mode-executor-design.md](vida-coder-service-mode-executor-design.md)
    Sources: user-requested design discussion on 2026-06-03 for a Rig-backed VIDA-aware coder as a first-class `vida coder` subcommand and service-capable executor; existing VIDA runtime/carrier law in `docs/process/agent-system.md`, `docs/process/team-development-and-orchestration-protocol.md`, `docs/process/external-cli-carrier-operator-procedure.md`, `docs/product/spec/pi-primary-environment-and-agent-carrier-design.md`, `docs/product/spec/taskflow-execution-semantics-and-scheduler-design.md`, `docs/product/spec/multi-orchestrator-session-ownership-and-claims-design.md`, `vida.config.yaml`, and current adapter/runtime precedents in `crates/vida-pi-agent`, `crates/vida/src/{runtime_dispatch_execution,status_surface,carrier_runtime_projection,agent_dispatch_surface}.rs`; external references include Rig core/AgentBuilder docs and MCP authorization/security model references
24. [hybrid-host-executor-semantics-host-environment-design.md](hybrid-host-executor-semantics-host-environment-design.md)
    Sources: `vida.config.yaml`, `docs/process/agent-system.md`, `docs/product/spec/host-agent-bridge-adapter-contract-design.md`, `docs/product/spec/codex-host-agent-boundary-and-cli-bridge-design.md`, and current hybrid dispatch/status code paths that distinguish host posture from selected executor backend
25. [specification-lane-scope-hardening-design.md](specification-lane-scope-hardening-design.md)
...
27. [vida-service-tui-wizard-execution-spec.md](vida-service-tui-wizard-execution-spec.md)
...
30. [repair-task-close-closure-truth-exception-design.md](repair-task-close-closure-truth-exception-design.md)
   Sources: `docs/product/spec/lawful-closure-continuation-rebinding-design.md`, `docs/product/spec/lane-supersede-and-shared-truth-envelope-design.md`, current runtime/taskflow code in `crates/vida/src/{runtime_dispatch_state,taskflow_consume_resume,release1_contracts}.rs`, the live 2026-04-21 task/run evidence for `feature-repair-task-close-exception-reconcile-closure-truth`, and the current need to prevent downstream closure receipts from inheriting upstream exception-path lineage
31. [reconcile-qwen-cli-carrier-drift-design.md](reconcile-qwen-cli-carrier-drift-design.md)
   Sources: user policy fixed on 2026-04-21 that `qwen_cli` remains template/reference-only, source-tree runtime evidence from `cargo run -p vida -- {status,orchestrator-init} --json`, active docs/specs under `docs/process/**` and `docs/product/spec/**` that still present qwen as current, current Rust test-fixture drift in `crates/vida/src/{init_surfaces,runtime_dispatch_execution,runtime_dispatch_state,runtime_lane_summary,status_surface,status_surface_external_cli,status_surface_host_agents,taskflow_consume,taskflow_consume_resume,taskflow_routing,taskflow_run_graph}.rs`, and the current need to reconcile surrounding parity debt without re-adding qwen to the active carrier catalog
32. [repair-explicit-continuation-bind-preservation-after-qwen-rebind-design.md](repair-explicit-continuation-bind-preservation-after-qwen-rebind-design.md)
   Sources: fresh 2026-04-21 source-tree runtime evidence where explicit qwen-task binding survives in `orchestrator-init` but `consume continue` rewrites truth back to stale run/task lineage while `agent-init` remains activation-view-only with no `packet_activation_evidence`; earlier closure-truth repair notes in `docs/product/spec/repair-task-close-closure-truth-exception-design.md`; continuation law in `docs/product/spec/{continuation-binding-fail-closed-hardening-design,continuation-and-seeded-dispatch-bridge-design,lawful-closure-continuation-rebinding-design}.md`; and current owner code in `crates/vida/src/{taskflow_consume_resume,taskflow_continuation,init_surfaces,taskflow_run_graph}.rs`
33. [repair-design-backed-reseed-canonicalization-does-not-deadlock-qwen-design.md](repair-design-backed-reseed-canonicalization-does-not-deadlock-qwen-design.md)
   Sources: fresh 2026-04-21 source-tree runtime evidence where `taskflow run-graph dispatch-init` lawfully reseeds the explicit qwen task but shapes it as `pbi_discussion/pm/work-pool-pack`, while the rendered packet canonicalizes to `dispatch_target=specification`, `handoff_runtime_role=pm`, and `selected_backend=null`; prior qwen design docs at `docs/product/spec/{reconcile-qwen-cli-carrier-drift-design,repair-explicit-continuation-bind-preservation-after-qwen-rebind-design}.md`; closed routing work in `docs/product/spec/existing-design-implementation-routing-blocked-design.md`; and current owner code in `crates/vida/src/{taskflow_run_graph,taskflow_consume,taskflow_routing,runtime_dispatch_state,runtime_lane_summary,runtime_assignment_builder}.rs`
34. [coach-retry-reuses-same-blocked-hermes-packet-without-fallback-design.md](coach-retry-reuses-same-blocked-hermes-packet-without-fallback-design.md)
   Sources: fresh 2026-04-21 persisted coach-lane artifacts for `feature-reconcile-qwen-cli-carrier-drift-across-config-code`, including repeated blocked results and the unchanged packet path `...19-33-27.253874589Z.json`; adjacent qwen/coaching docs at `docs/product/spec/{reconcile-qwen-cli-carrier-drift-design,coach-lane-inherits-internal-fallback-over-explicit-review-route-design,external-coach-timeout-truth-does-not-return-cleanly-design,blocked-external-coach-artifact-truth-not-reconciled-design}.md`; and current retry/rewrite owner code in `crates/vida/src/{taskflow_consume_resume,taskflow_routing,runtime_dispatch_state}.rs`
35. [analysis-lane-can-close-implementation-without-write-evidence-design.md](analysis-lane-can-close-implementation-without-write-evidence-design.md)
   Sources: fresh 2026-04-21 live run evidence for `feature-fix-bug-coach-retry-reuses-same-blocked-hermes-packet-without-fallback`, where a read-only `analysis` dispatch executed through `internal_cli:codex` yet `vida taskflow run-graph status --json` reconciled the run to `status=completed`, `lifecycle_stage=implementation_complete`, and downstream `closure` packet ready; existing closure-candidate regression in `crates/vida/src/state_store.rs`; summary reconciliation owner code in `crates/vida/src/state_store_run_graph_summary.rs`; implementation completion transitions in `crates/vida/src/taskflow_run_graph.rs`; and adjacent continuation/runtime canon in `docs/product/spec/{coach-retry-reuses-same-blocked-hermes-packet-without-fallback-design,repair-explicit-continuation-bind-preservation-after-qwen-rebind-design,continuation-binding-fail-closed-hardening-design}.md`
36. [explicit-implementation-seed-drops-design-backed-owned-paths-design.md](explicit-implementation-seed-drops-design-backed-owned-paths-design.md)
