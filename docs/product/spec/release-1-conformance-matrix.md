...
Status: active Release-1 implementation law (runtime checkpoint refreshed `2026-04-08`)
...
|---|---|---|---|---|---|
| `compiled-autonomous-delivery-runtime-architecture.md` | workspace boundary between `crates/vida/**`, `crates/taskflow-*/**`, and `crates/docflow-*/**` | runtime-family split exists, but launcher still owns large parts of execution, activation, bundle, closure glue, and carrier selection; neutral `carrier_runtime` / `runtime_assignment` contracts exist, canonical write paths no longer mirror `codex_*`, and the remaining debt is codex-era materialization/proof residue plus launcher-owned bridge logic | architecture review, `vida status`, `vida doctor`, `vida taskflow consume` | `spec_complete / code_bridge_heavy / proof_partial` | thin the shell, burn down codex-era residue, and move execution/closure truth into family-owned modules |
...
| `compiled-runtime-bundle-contract.md` + `taskflow-protocol-runtime-binding-model.md` | `crates/vida/src/taskflow_runtime_bundle.rs`, `crates/vida/src/taskflow_protocol_binding.rs` | bundle-check and protocol-binding checks exist, neutral runtime identity already emits `carrier_runtime` plus `runtime_assignment`, compiled bundles no longer emit `codex_multi_agent`, and launcher routing no longer reads `codex_runtime_assignment`; the remaining debt is codex-heavy fixture/materialization residue plus moving bundle/protocol-binding truth below the launcher | bundle check, protocol-binding sync/check, init surfaces | `spec_complete / code_partial / proof_partial` | keep burning down codex-heavy residue, then move bundle/protocol-binding truth below the launcher |
...
| `release-1-operator-surface-contract.md` | CLI rendering layer | `status`, `doctor`, root `vida consume`, and root `vida recovery` now route as canonical top-level surfaces while preserving TaskFlow family ownership underneath; root `vida lane` and `vida approval` now exist as explicit fail-closed reserved surfaces with the shared envelope, but still lack family-owned implementation and canonical payload builders | `status`, `doctor`, `consume`, `lane`, `approval`, `recovery` | `spec_complete / code_partial / proof_partial` | keep the shared envelope canonical across existing surfaces, then close or explicitly defer the remaining `lane`/`approval` family-owned implementation work |
...
updated_at: 2026-04-08T07:36:14.852064279Z
