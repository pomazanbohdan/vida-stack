...
Status: active Release-1 implementation law (runtime checkpoint refreshed `2026-04-06`)
...
| `release-1-closure-contract.md` | TaskFlow closure admission domain | closure admission exists in `main.rs`, `taskflow_consume.rs`, `taskflow_consume_bundle.rs`, and `taskflow_consume_resume.rs`; `status` and `doctor` now expose proof-backed trace, tool-contract, retrieval-trust, approval/delegation, and failure-control posture, but DocFlow verdicts are still shell-assembled and seam proof still requires explicit readiness/proof receipts | closure receipts, seam proof, persisted-final-snapshot recovery proofs | `spec_complete / code_partial / proof_partial` | refresh the closure matrices from the current proof state, keep the seam receipt requirements fail-closed, and emit the final admission verdict from current build/proof evidence |
...
| `release-1-control-metrics-and-gates.md` | evaluation and metrics services | fail-closed gate checks now exist across `status`, `doctor`, and consume flows for canonical tool-contract, trace, retrieval-trust, approval/delegation, and failure-control evidence, but explicit metric artifacts and threshold views are still mostly absent | status/doctor/eval dashboards, control-track smoke proofs | `spec_complete / code_partial / proof_partial` | materialize threshold/metric artifacts and bind them into the final admission evidence bundle |
...
| `release-1-proof-scenario-catalog.md` | tests/eval fixtures | smoke coverage in `crates/vida/tests/**` is substantial, the full `cargo test -p vida` suite is green, and persisted-final-snapshot recovery proofs now cover failure-control evidence, but some proofs still assert legacy bundle names, backend literals, and exact `agent-init` surfaces | proofcheck, full package test suite, targeted recovery/failure-control proofs | `spec_complete / code_partial / proof_partial` | continue burning down legacy fixture assumptions while keeping the current suite green as the admission baseline |
...
| `release-1-operator-surface-contract.md` | CLI rendering layer | `status`, `doctor`, and nested `vida taskflow consume` now emit canonical `compatibility_class` and shared-envelope placeholders for `trace_id`, `workflow_class`, and `risk_tier`, but root `consume`, `lane`, `approval`, and `recovery` contracts are still not promoted as stable top-level root surfaces | `status`, `doctor`, `consume`, `lane`, `approval`, `recovery` | `spec_complete / code_partial / proof_partial` | keep the shared envelope canonical across existing surfaces, then close or explicitly defer the remaining root-surface promotion work |
...
updated_at: 2026-04-06T14:21:10.327265184Z
