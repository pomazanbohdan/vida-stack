...
|---|---|---|---|---|---|
| `compiled-autonomous-delivery-runtime-architecture.md` | workspace boundary between `crates/vida/**`, `crates/taskflow-*/**`, and `crates/docflow-*/**` | runtime-family split exists, but launcher still owns large parts of execution, activation, bundle, closure glue, and carrier selection through `codex_multi_agent` / `codex_runtime_assignment` | architecture review, `vida status`, `vida doctor`, `vida taskflow consume` | `spec_complete / code_bridge_heavy / proof_partial` | thin the shell, neutralize carrier/runtime contracts, and move execution/closure truth into family-owned modules |
...
| `compiled-runtime-bundle-contract.md` + `taskflow-protocol-runtime-binding-model.md` | `crates/vida/src/taskflow_runtime_bundle.rs`, `crates/vida/src/taskflow_protocol_binding.rs` | bundle-check and protocol-binding checks exist, but compiled runtime identity is still emitted as `codex_multi_agent` and `codex_runtime_assignment` rather than carrier-neutral contracts | bundle check, protocol-binding sync/check, init surfaces | `spec_complete / code_partial / proof_partial` | replace codex-era bundle fields with neutral carrier/runtime-assignment contracts and move bundle/protocol-binding truth below the launcher |
...
| `release-1-closure-contract.md` | TaskFlow closure admission domain | closure admission exists in `main.rs`, `taskflow_consume.rs`, and `taskflow_consume_bundle.rs`, but DocFlow verdicts are shell-assembled and seam “receipt-backed” proof currently means protocol-binding receipt only | closure receipts, seam proof | `spec_complete / code_partial / proof_partial` | move closure admission below shell and require DocFlow readiness/proof receipts in the seam |
| `release-1-workflow-classification-and-risk-matrix.md` | shared runtime contract module plus TaskFlow enforcement | no shared Rust `workflow_class` or `risk_tier` contract layer is present today | workflow classification outputs, operator risk surfaces | `spec_complete / code_sparse / proof_absent` | introduce canonical workflow/risk enums and emit them in operator artifacts |
| `release-1-control-metrics-and-gates.md` | evaluation and metrics services | some fail-closed gate checks exist in `status`, `doctor`, and consume flows, but metric artifacts and threshold views are mostly absent | status/doctor/eval dashboards | `spec_complete / code_partial / proof_sparse` | materialize metric artifacts, gate views, and evidence windows |
...
| `release-1-decision-tables.md` | policy and gate services | fail-closed gating exists across consume, protocol-binding, status, and doctor paths, but logic is duplicated and shell-local | policy, approval, tool, and retrieval execution paths | `spec_complete / code_partial / proof_partial` | convert prose decisions into one family-owned admission/policy layer |
...
| `release-1-error-and-exception-taxonomy.md` | contracts/state/runtime modules | some blocker codes are canonicalized, but many blockers still arise as ad hoc shell strings | operator errors and blocker outputs | `spec_complete / code_partial / proof_partial` | replace ad hoc error strings with one canonical blocker registry |
...
| `release-1-proof-scenario-catalog.md` | tests/eval fixtures | smoke coverage in `crates/vida/tests/**` is substantial, but many proofs still assert codex-era bundle names, backend literals, and exact `agent-init` surfaces | proofcheck, test suites, benchmark runs | `spec_complete / code_partial / proof_partial` | promote the checked-in smoke scenarios into carrier-neutral fixture/golden assets |
...
| `release-1-runtime-enum-and-code-contracts.md` | shared enums/types | `crates/vida/src/release1_contracts.rs` only canonicalizes `lane_status`, codex-shaped `compatibility_class`, and a narrow blocker subset; shared `workflow_class`, `risk_tier`, `approval_status`, and `gate_level` contracts are missing | machine-readable outputs | `spec_complete / code_partial / proof_partial` | centralize the full canonical enum/value set in shared contracts |
| `release-1-operator-surface-contract.md` | CLI rendering layer | `status`, `doctor`, nested `vida taskflow consume`, and nested `vida taskflow recovery` exist, but root `consume`, `lane`, `approval`, and `recovery` contracts are not present and some workflow-bound fields are missing | `status`, `doctor`, `consume`, `lane`, `approval`, `recovery` | `spec_complete / code_partial / proof_partial` | stabilize the shared envelope and close or explicitly defer the missing root surfaces |
...
| `release-1-risk-acceptance-register.md` | release governance surface | the register exists as canon, but active runtime risks are not yet populated there as the live closure surface | risk report / closure bundle | `spec_started / process_only / proof_absent` | populate actual open risks and bind them to closure evidence |
