...
3. Explicit VIDA/framework self-diagnosis requests are a special execution-in-chat path: execute the diagnosis now, but keep it outside TaskFlow unless the user explicitly asks for tracked TaskFlow.
...
TaskFlow is mandatory only when at least one is true:
...
1. when the request already satisfies this gate at session start, attach to or create/select the lawful `TaskFlow task before implementation,
2. start tracked execution through the canonical entrypoints from `vida/config/instructions/runtime-instructions.taskflow-protocol.md` and `vida/config/instructions/runtime-instructions.beads-protocol.md`,
3. pre-register planned blocks before non-trivial execution and keep subsequent work inside active TaskFlow block lifecycle.

TaskFlow is forbidden by default when all are true:
...
TaskFlow is also forbidden for explicit VIDA/framework self-diagnosis by default when all are true:
...
3. durable runtime state (`run-graph`, TaskFlow evidence, context governance, framework memory),
...
| TaskFlow/pack engagement, moderate cross-protocol impact, or uncertainty after Lean | `Standard` |
...
7. Read `vida/config/instructions/runtime-instructions.web-validation-protocol.md`.
8. Read `vida/config/instructions/runtime-instructions.project-overlay-protocol.md`.
9. Read `vida.config.yaml` when present.
10. If `protocol_activation.agent_system=true`, read `vida/config/instructions/instruction-contracts.agent-system-protocol.md`.
11. Read `vida/config/instructions/runtime-instructions.beads-protocol.md` only if the request is not `answer_only` and the compact snapshot is insufficient.
12. If `framework_self_diagnosis.enabled=true`, read `vida/config/instructions/diagnostic-instructions.silent-framework-diagnosis-protocol.md` and keep the silent diagnosis contract active for the session.
13. Apply `vida/config/instructions/instruction-contracts.instruction-activation-protocol.md` when deciding whether additional instruction/protocol files should be loaded beyond the lean set.
...
1. Execute Lean Boot.
2. Read `vida/config/instructions/runtime-instructions.taskflow-protocol.md` only if TaskFlow engagement is required.
3. Read `vida/config/instructions/command-instructions.use-case-packs.md` only if a pack path is required.
4. Read `vida/config/instructions/command-instructions.implement-execution-protocol.md` only if implementation flow is in scope.
5. Do not expand beyond route-triggered protocol surfaces; use `vida/config/instructions/instruction-contracts.instruction-activation-protocol.md` when in doubt.
...
2. Read `vida/config/instructions/instruction-contracts.orchestration-protocol.md`.
3. Read `vida/config/instructions/command-instructions.pipelines.md`.

## Execution Rules

1. For `answer_only`, stay in chat/report mode and avoid task/TaskFlow machinery.
...
3. `vida/config/instructions/instruction-contracts.worker-dispatch-protocol.md`
4. `vida/config/instructions/command-instructions.pipelines.md`
