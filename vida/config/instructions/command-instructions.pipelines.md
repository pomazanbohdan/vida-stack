...
2. `--mode quick` is for intermediate checks and skips strict TaskFlow snapshot gates.
...
TaskFlow decomposition and parallel execution policy:
...
3. If project operational guidance changes, update `docs/process/*` and `scripts/*`, not `vida/config/instructions/*`.
...
3. If analyzer/build/test behavior depends on project environment preparation, the canonical sequence must be documented in project runbooks, not `vida/config/instructions/*`.
...
3. Stateful commands include:
   - task-state mutation (`br`, `beads-workflow`, TaskFlow sync/index writes),
...
4. Full MCP search guide: `vida/config/instructions/system-maps.tooling.md`.
...
1. Do not run implementation or research steps outside an active TaskFlow block.
...
```bash
vida/config/instructions/diagnostic-instructions.self-reflection-protocol.md
```
...
1. `vida-pack-helper.sh` remains a migration-only helper per `vida/config/instructions/system-maps.runtime-transition-map.md`.
