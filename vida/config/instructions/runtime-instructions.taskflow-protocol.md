...
1. When task lifecycle state and execution telemetry appear out of sync, use `vida/config/instructions/runtime-instructions.task-state-reconciliation-protocol.md` as the canonical reconciliation layer before closing, reopening, or declaring the task stale.
...
8. When the same technical error repeats twice inside an active block, the block must record an escalation event and switch to `vida/config/instructions/diagnostic-instructions.debug-escalation-protocol.md` before the next substantive fix attempt.
...
When root `vida.config.yaml` enables silent framework diagnosis, TaskFlow must treat framework friction as a tracked deferred follow-up, not as an ad hoc side path.
...
3. Continue the current task with the lightest safe workaround when possible and record that workaround in TaskFlow evidence.
...
7. Framework follow-up work still uses normal TaskFlow, web/WVP validation when architecture claims depend on external reality, and delegated verification before closure.
...
11. When more than one next task/block appears lawful, apply `vida/config/instructions/runtime-instructions.execution-priority-protocol.md` before choosing or redirecting.
...
0.2. If a task enters bounded conflict-discussion mode via `vida/config/instructions/runtime-instructions.problem-party-protocol.md`, record the board artifact path in block evidence before resuming normal execution.
1. Step gate: `block-end` requires evidence or artifacts.
1.1. If WVP trigger fired, `block-end`/`reflect` evidence must include WVP markers per `vida/config/instructions/runtime-instructions.web-validation-protocol.md`.
