...
2. code, configuration, documentation, or process change candidate selection:
   - `overlay.solution-minimality-protocol.md`
3. request-intent classification, worker-first coordination, or lawful-next selection beyond `answer_only`:
   - `core.orchestration-runtime-capsule.md`
   - owner file `core.orchestration-protocol.md` for edge cases
4. worker-first routing, mode posture, backend fallback, or saturation-recovery posture:
   - `core.agent-system-runtime-capsule.md`
   - owner file `core.agent-system-protocol.md` for edge cases
5. external facts can change the decision:
   - `work.web-validation-protocol.md`
6. visible skill catalog or skill-bound work:
   - `core.skill-activation-protocol.md`
7. packet shaping / leaf-depth selection:
   - `core.packet-decomposition-protocol.md`
8. worker packet / handoff / next-agent prompt work:
   - `lane.agent-handoff-context-protocol.md`
9. tracked execution with multiple lawful next items:
   - `work.execution-priority-protocol.md`
10. silent diagnosis enabled:
   - `analysis.silent-framework-diagnosis-protocol.md`
11. restart / checkpoint / replay / resumability:
   - `recovery.checkpoint-replay-recovery-protocol.md`
   - `core.run-graph-protocol.md`
12. settled plan/spec/task pool should continue to completion:
   - `overlay.autonomous-execution-runtime-capsule.md`
   - owner file `overlay.autonomous-execution-protocol.md` for edge cases
13. separated authorship, verifier independence, or closure proof semantics are active:
