...
2. If worker-lane confirmation is absent or ambiguous, fall back to `vida/config/instructions/agent-definitions.orchestrator-entry.md`.
...
2. You do not own task routing, fallback policy, merge policy, TaskFlow state, `br` state, or final synthesis.
...
7. Use the worker-safe thinking subset from `vida/config/instructions/instruction-contracts.worker-thinking.md`.
