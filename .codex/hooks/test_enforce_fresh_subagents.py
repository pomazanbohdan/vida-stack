...
class EnforceFreshSubagentsTests(unittest.TestCase):
    def test_spoofed_spawn_agent_names_receive_no_permission_decision(self) -> None:
        for tool_name in (
            ...
            "despawn_agent",
            "x.spawn-agent",
            "evil.multi_agent_v1.spawn_agent",
            "multi_agent_v1.spawn_agent.extra",
        ):
    ...
    def test_trusted_spawn_agent_rejects_unknown_input_fields(self) -> None:
        for tool_name in ("multi_agent_v1.spawn_agent", "spawn_agent"):
            with self.subTest(tool_name=tool_name):
                output = run_hook(
                    {
                        "tool_name": tool_name,
                ...
                self.assertIn(
                    "unsupported tool_input fields: cmd",
