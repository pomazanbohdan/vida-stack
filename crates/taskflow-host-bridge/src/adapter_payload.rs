...
mod tests {
    ...
    fn payload_for(request: &Value) -> Value {
        ...
        build_host_bridge_adapter_payload(HostBridgeAdapterPayloadInput {
            ...
            completion_command: format!("vida lane complete run-1 --receipt-id run-1-{dispatch_target}-host-bridge-receipt --host-bridge-request request.json --host-agent-id <host-agent-id> --decision pass --verdict implemented --allowed-next-node coach --blocker-codes [] --host-bridge-summary completed"),
    ...
    fn host_bridge_adapter_payload_pass_renders_host_tool_contract() {
        ...
        assert_eq!(
            ...
            "vida lane complete run-1 --receipt-id run-1-implementer-host-bridge-receipt --host-bridge-request request.json --host-agent-id <host-agent-id> --decision pass --verdict implemented --allowed-next-node coach --blocker-codes [] --host-bridge-summary completed"
        );
        assert_eq!(
