...
fn host_bridge_registry_missing_for_internal_backend(
    ...
    selected_cli_entry
        ...
        .is_none()
        && backend_is_internal_host_bridge(role_selection, overlay, backend_id)
}
...
fn selected_internal_host_carrier(
    ...
    let config_matched_carrier = carriers.iter().find(|carrier| {
        let runtime_role_matches = requested_runtime_role
            .as_deref()
...
fn dispatch_packet_identity_fields(dispatch_packet_path: &str) -> (Option<String>, Option<String>) {
    let Some(packet) = std::fs::read_to_string(dispatch_packet_path)
        .ok()
        .and_then(|raw| serde_json::from_str::<serde_json::Value>(&raw).ok())
    else {
        return (None, None);
    };
    let packet_field = |field: &str| {
        packet
            .get(field)
            .or_else(|| {
                packet
    ...
    (packet_field("packet_id"), packet_field("attempt_id"))
}
    ...
    if field == "packet_path" {
        ...
        return match (
            ...
            (Some(existing), Some(expected)) => {
                host_bridge_packet_paths_equivalent(existing, expected)
            }
...
fn existing_host_bridge_request_has_retryable_completion_evidence(
    ...
    ["result_path", "receipt_path"].iter().any(|field| {
        ...
        let retryable = artifact.as_ref().is_some_and(|artifact| {
...
fn existing_host_bridge_request_needs_adapter_refresh(
    ...
    let explicit_legacy_alias = existing
        .get("adapter_params")
        .and_then(serde_json::Value::as_object)
        .is_some_and(|params| {
            ["spawn_tool", "wait_tool", "close_tool", "dispose_tool"]
                .iter()
    ...
    legacy_unconfigured
        && expected_configured
        && HostBridgeAdapterOperations::from_request_value(expected).is_ok()
        && [
            "receipt_mode",
            "adapter_kind",
            "adapter_capability_id",
...
fn materialize_host_tool_bridge_request(
    ...
    let adapter_contract_hash = blake3::hash(
        &serde_json::to_vec(&adapter_contract_snapshot)
            .map_err(|error| format!("host_bridge_adapter_contract_snapshot_invalid:{error}"))?,
    )
    .to_hex()
    .to_string();
    ...
    let (packet_id, packet_attempt_id) = dispatch_packet_identity_fields(dispatch_packet_path);
    let packet_id = packet_id.unwrap_or_else(|| {
        format!(
            "{}::{}::delivery",
            receipt.run_id.trim(),
            receipt.dispatch_target.trim()
        )
    });
    let attempt_id = normalized_host_bridge_attempt_id(
        &receipt.run_id,
    ...
    if let Some(existing) = read_existing_host_bridge_request(&paths.request_path)? {
        ...
        if existing
            .get("packet_path")
            .and_then(serde_json::Value::as_str)
            .is_some_and(|path| host_bridge_packet_paths_equivalent(path, dispatch_packet_path))
        {
            ...
            } else if existing_host_bridge_request_needs_pending_contract_refresh(
                &existing, &request,
            ) {
                replace_existing_request = true;
...
fn host_bridge_receipt_binding_value(
    request: &serde_json::Value,
    receipt: &crate::state_store::RunGraphDispatchReceipt,
    backend_id: &str,
) -> serde_json::Value {
...
fn validate_host_bridge_request_dispatch_binding(
    ...
    let decision = validate_dispatch_receipt_binding(&DispatchReceiptBindingInput {
        ...
        receipt: Some(host_bridge_receipt_binding_value(
            request, receipt, backend_id,
        )),
...
fn validate_completed_host_bridge_artifacts(
    ...
    if !host_bridge_packet_paths_equivalent(request_result_path, &rendered_result_path) {
        return Err(
            "Host bridge request result_path does not match the resolved result path.".into(),
        );
    }
    if !host_bridge_packet_paths_equivalent(request_receipt_path, &rendered_receipt_path) {
        return Err(
    ...
    if !bridge_receipt
        .get("request_path")
        .and_then(serde_json::Value::as_str)
        .is_some_and(|value| host_bridge_packet_paths_equivalent(value, request_path))
    {
        return Err(
    ...
    if !bridge_receipt
        .get("result_path")
        .and_then(serde_json::Value::as_str)
        .is_some_and(|value| host_bridge_packet_paths_equivalent(value, &rendered_result_path))
    {
        return Err(
...
"#,
    ...
    #[test]
    fn missing_internal_host_bridge_registry_is_a_terminal_routing_blocker() {
        ...
        assert!(host_bridge_registry_missing_for_internal_backend(
            &role_selection,
            Some(&overlay),
            "internal_subagents",
            None,
        ...
        .expect("configured entry should parse");
        assert!(!host_bridge_registry_missing_for_internal_backend(
            &role_selection,
            Some(&overlay),
            "internal_subagents",
    ...
    #[test]
    fn adapter_contract_snapshot_replay_detects_hash_source_and_operation_drift() {
        let adapter_operations = serde_json::json!({
            "adapter_kind": "fixture_adapter",
            "adapter_capability_id": "fixture_capability",
            "invocation_mode": "fixture_parent",
            "dispatch_transport": "host_tool_bridge",
            "receipt_mode": "fixture_receipt",
            "operations": {
                "spawn": "fixture.spawn.v1",
                "wait": "fixture.wait.v1",
                "dispose": "fixture.dispose.v1"
            },
        ...
        for (field, value) in [
            ...
            (
                "adapter_contract_source",
                serde_json::json!("other.config.yaml"),
            ),
            (
                "adapter_operations",
                serde_json::json!({
...
"#,
        ...
        let error = materialize_host_tool_bridge_request(
            ...
            Some(&entry),
            &dispatch_packet_path.display().to_string(),
            "internal_subagents",
            "junior",
            &receipt,
...
"#,
    ...
    fn retryable_blocked_host_bridge_request_can_be_rearmed_but_activation_only_cannot() {
        ...
        assert!(
            super::existing_host_bridge_request_has_retryable_completion_evidence(
                &existing, &expected
            )
        );
        assert!(
            super::existing_host_bridge_request_needs_retryable_blocked_refresh(
                &existing, &expected
            )
        );
    ...
    fn host_tool_bridge_materialize_rearms_retryable_blocked_request() {
        ...
        std::fs::write(&result_path, blocked_artifact.to_string()).expect("write blocked result");
        std::fs::write(&receipt_path, blocked_artifact.to_string()).expect("write blocked receipt");
        assert!(result_path.exists());
        assert!(super::host_bridge_artifact_has_retryable_completion_blocker(&blocked_artifact));
        ...
        assert_eq!(
            rearmed_request["owned_paths"],
            serde_json::json!(["src/lib.rs"])
        );
        assert!(
            !result_path.exists(),
            "old blocked result must not be reused"
        );
        assert!(
            !receipt_path.exists(),
            "old blocked receipt must not be reused"
        );
        let _ = std::fs::remove_dir_all(project_root);
    }
...
"#,
        ...
        let task_id = request["task_id"]
            .as_str()
            .expect("request task id should render");
        let backend_id = request["backend_id"]
            .as_str()
            .expect("request backend id should render");
        let packet_id = request["packet_id"]
            .as_str()
            .expect("request packet id should render");
        let attempt_id = request["attempt_id"]
            .as_str()
            .expect("request attempt id should render");
        std::fs::write(
            &result_path,
            serde_json::to_string_pretty(&serde_json::json!({
...
"#,
    ...
    fn host_bridge_refreshes_legacy_unconfigured_same_lane_request_when_codex_defaults_available() {
        ...
        let mut legacy_request = serde_json::from_str::<serde_json::Value>(
            &std::fs::read_to_string(&request_path).expect("stale request should persist"),
        )
