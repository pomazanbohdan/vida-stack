...
const TASK_CLOSE_AFTER_HELP: &str = "Examples:\n  vida task close <task-id> --reason \"proof passed\"\n  vida task close <task-id> --reason-file close-reason.txt\n  vida task close <task-id> --reason \"proof passed\" --json\n  vida task close <task-id> --reason \"proof passed\" --summary\n  vida task close <task-id> --reason \"proof passed\" --include-global-progress --json\n  vida task close <task-id> --reason \"proof passed\" --release --install\n\nOutput:\n  Default output is compact human-readable task close output for operators and omits unrelated epic progress.\n  Blocked default output includes blocker codes and next actions without requiring --json.\n  Use --json only when automation needs the machine-readable close payload.\n  Use --summary to make the compact task-scoped projection explicit.\n  Use --include-global-progress only when expanded epic progress is explicitly required.\n  Use `vida task progress <epic-id>` for standalone progress inspection.\n  Use --render plain|color|color_emoji to choose the default human rendering mode.";
...
pub(crate) struct TaskCloseArgs {
    ...
    #[arg(
        long = "summary",
        help = "Explicitly request the default compact task-scoped close projection"
    )]
    pub(crate) summary: bool,
