...
fn build_lane_envelope_with_owned_scope(
    ...
    build_lane_envelope_with_owned_scope_and_rework_route(
        summary,
        status,
        exception_path_metadata_path,
        exception_path_metadata,
...
fn build_lane_envelope_with_owned_scope_and_rework_route(
    summary: crate::state_store::RunGraphDispatchReceiptSummary,
    status: Option<crate::state_store::RunGraphStatus>,
    exception_path_metadata_path: Option<String>,
    exception_path_metadata: Option<ExceptionTakeoverMetadata>,
    ...
    let downstream_dispatch_trace_path = summary.downstream_dispatch_trace_path.clone();
    let downstream_dispatch_status = summary.downstream_dispatch_status.clone();
    let downstream_dispatch_blockers = summary.downstream_dispatch_blockers.clone();
    let downstream_dispatch_target = summary.downstream_dispatch_target.clone();
    let downstream_dispatch_ready = summary.downstream_dispatch_ready;
    let host_bridge_result_path = summary
        .dispatch_surface
        .as_deref()
        .is_some_and(|surface| surface.contains("--host-bridge-request"))
        .then(|| dispatch_result_path.clone())
        .flatten();
    let host_bridge_receipt_path = downstream_dispatch_trace_path.clone();
    ...
    let current_exception_path_metadata = root_local_write_allowed
        ...
        .flatten();
    let next_action = lane_ready_downstream_next_action(&summary, status.as_ref(), blocked)
        .or_else(|| {
    ...
    let artifact_refs = serde_json::json!({
        ...
        "downstream_dispatch_trace_path": downstream_dispatch_trace_path.clone(),
        "downstream_dispatch_status": downstream_dispatch_status.clone(),
        "downstream_dispatch_ready": downstream_dispatch_ready,
        "downstream_dispatch_target": downstream_dispatch_target.clone(),
        "downstream_dispatch_blockers": downstream_dispatch_blockers.clone(),
        "host_bridge_result_path": host_bridge_result_path,
        "host_bridge_receipt_path": host_bridge_receipt_path,
    });
    let operator_contracts = render_operator_contract_envelope(
...
fn lane_ready_downstream_next_action(
    ...
    if blocked
        || status.is_some_and(lane_status_is_terminal_closure_without_next_unit)
        || !lane_summary_has_ready_downstream_handoff(summary)
    {
        return None;
    }
    let command =
...
fn lane_status_is_terminal_closure_without_next_unit(
    status: &crate::state_store::RunGraphStatus,
) -> bool {
    let next_node = status
        .next_node
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty());
    taskflow_authority::terminal_closure::status_is_terminal_closure_without_next_unit(
        &taskflow_authority::terminal_closure::TerminalClosureStatus {
            run_id: &status.run_id,
            active_node: &status.active_node,
            next_node,
            status: &status.status,
            lifecycle_stage: &status.lifecycle_stage,
            resume_target: Some(&status.resume_target),
        },
    )
}
...
fn lane_blocked_next_action(
    ...
    if let Some(action) = lane_authorized_rework_next_action(summary, authorized_rework_route) {
        return Some(action);
    }
...
async fn lane_authorized_rework_route(
    store: &crate::state_store::StateStore,
    summary: &crate::state_store::RunGraphDispatchReceiptSummary,
    status: Option<&crate::state_store::RunGraphStatus>,
) -> Result<Option<crate::runtime_dispatch_result_evidence::DispatchReworkRoute>, String> {
...
async fn lane_rework_route_for_envelope(
    store: &crate::state_store::StateStore,
    summary: &crate::state_store::RunGraphDispatchReceiptSummary,
    status: Option<&crate::state_store::RunGraphStatus>,
    truth: &mut LaneShowTruth,
) -> Option<crate::runtime_dispatch_result_evidence::DispatchReworkRoute> {
    match lane_authorized_rework_route(store, summary, status).await {
        Ok(route) => route,
        Err(error) => {
            let blocker_code = error
                .split(':')
                .next()
                .unwrap_or("team_flow_rework_blocked");
            if !truth.blocker_codes.iter().any(|code| code == blocker_code) {
                truth.blocker_codes.push(blocker_code.to_string());
            }
            truth.blocked = true;
            truth.next_actions.push(format!(
                "rehydrate dispatch rework authority before continuing: {error}"
            ));
            None
        }
    }
...
fn blocked_source_target_from_summary_packet(
    ...
    let packet_path =
        canonicalize_existing_regular_state_path(&state_root, &packet_path, "dispatch packet")
            .ok()?;
...
fn lane_summary_has_ready_downstream_handoff(
    ...
    crate::runtime_dispatch_receipt_helpers::dispatch_summary_has_clean_ready_downstream_handoff(
        Some(summary),
        Some(&summary.run_id),
    ) || crate::runtime_dispatch_receipt_helpers::dispatch_summary_has_clean_routed_agent_handoff(
        Some(summary),
        Some(&summary.run_id),
    )
}
...
fn emit_lane_retire_blocked_envelope(
    as_json: bool,
    run_id: &str,
    receipt_id: &str,
    ...
    let envelope = lane_retire_blocked_envelope(
        run_id,
        receipt_id,
        blocker_code,
        reason,
    ...
    if crate::surface_render::print_surface_json(
        &envelope,
        as_json,
        "lane retire blocked surface should serialize",
    ) {
        return ExitCode::from(2);
    }
...
fn lane_retire_blocked_envelope(
    run_id: &str,
    receipt_id: &str,
    blocker_code: &'static str,
    reason: String,
    ...
    let next_actions = vec![format!(
        "Inspect `{}` and retry `vida lane retire {}` only after the blocker is resolved.",
        operator_output::command_text::human_command(&format!("vida lane show {run_id}")),
        crate::shell_quote(run_id)
    )];
    let artifact_refs = serde_json::json!({
        "surface": "vida lane retire",
        "run_id": run_id,
...
async fn acquire_lane_retire_closed_run_claim(
    store: &StateStore,
    operator_session_projection: &serde_json::Value,
    run_id: &str,
    task_id: &str,
    receipt_id: &str,
) -> Result<(), crate::state_store::StateStoreError> {
    let current_session = &operator_session_projection["current_session"];
    let current_session_id = current_session["session_id"]
        .as_str()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or_else(|| crate::state_store::StateStoreError::InvalidTaskRecord {
            reason: "lane retire requires current session evidence before claiming stale run"
                .to_string(),
        })?;
    let claim_session_id = current_session["fallback_replaces_legacy_stable_worktree_state_hash"]
        .as_str()
        .map(str::trim)
        .filter(|value| !value.is_empty())
    ...
    let worktree_environment_id = current_session["worktree_environment_id"]
        ...
        .filter(|value| !value.is_empty())
        .map(str::to_string)
        .unwrap_or_else(|| store.root().display().to_string());

    store
        .acquire_orchestrator_claim(crate::state_store::AcquireOrchestratorClaimRequest {
            claim_id: format!("lane-retire-closed-run-{run_id}-{receipt_id}"),
            ...
            lane_id: None,
            claim_kind: "lane_retire_closed_task_stale_run".to_string(),
            conflict_domain: Some(format!("lane-retire-closed-run:{run_id}")),
...
fn lane_mutation_status_guard(
    ...
    let terminal_completed_without_next_unit = status.lifecycle_stage == "closure_complete"
        && status
            ...
            .filter(|value| !value.is_empty())
            .is_none();
    let recovery_terminal = recovery.is_some_and(|recovery| recovery.is_terminal_closure());
...
fn write_lane_packet(path: &str, packet: &serde_json::Value) -> Result<(), String> {
    let mut compact_packet = packet.clone();
    crate::runtime_dispatch_state::normalize_persisted_dispatch_packet_role_selection(
        &mut compact_packet,
    )?;
    let encoded = serde_json::to_string_pretty(&compact_packet)
        .map_err(|error| format!("Failed to encode persisted lane packet `{path}`: {error}"))?;
    if encoded.len() as u64 > MAX_LANE_PACKET_READ_BYTES {
        return Err(format!(
            "Persisted lane packet `{path}` exceeds {} bytes.",
            MAX_LANE_PACKET_READ_BYTES
        ));
    }
...
struct HostBridgeCompletionEvidence {
    result_path: String,
    receipt_path: String,
    submitted_result_path: Option<String>,
...
impl HostBridgeCompletionResultArgs<'_> {
    ...
    fn has_explicit_result_contract(&self) -> bool {
        self.result_file.is_some()
            || self.decision.is_some()
            || self.verdict.is_some()
            || self.blocker_codes.is_some()
            || !self.blocker_code.is_empty()
            || self.rework_target.is_some()
    }

    fn routable_rework_next_node(&self) -> Option<String> {
        let rework_requested = [self.decision, self.verdict]
            .into_iter()
            .flatten()
            .map(str::trim)
            .any(host_bridge_completion_result_value_is_rework_alias);
        if !rework_requested || !self.explicit_blocker_codes().is_empty() {
            return None;
        }
        self.allowed_next_node
    ...
    fn indicates_blocked(&self) -> bool {
        let decision_or_verdict = [self.decision, self.verdict];
        decision_or_verdict
            .into_iter()
            .flatten()
            .map(str::trim)
            ...
            || (decision_or_verdict
                ...
                .map(str::trim)
                .any(host_bridge_completion_result_value_is_rework_alias)
                && self.routable_rework_next_node().is_none())
            || (self
                .rework_target
                .is_some_and(host_bridge_completion_rework_target_is_present)
                && self.routable_rework_next_node().is_none())
...
fn host_bridge_completion_result_value_is_blocked(value: &str) -> bool {
    matches!(
        ...
        "blocked" | "failed" | "failure" | "error" | "retry" | "retry_required"
    )
}

fn host_bridge_completion_result_value_is_rework_alias(value: &str) -> bool {
    matches!(
        value.trim().to_ascii_lowercase().as_str(),
        "rework" | "needs_rework" | "rework_requested" | "rework_required"
    )
}

fn host_bridge_completion_allowed_next_node_is_present(value: Option<&str>) -> bool {
    value
        .map(str::trim)
        .filter(|value| !value.is_empty())
...
fn host_bridge_completion_rework_target_is_present(value: &str) -> bool {
    ...
    !matches!(
        normalized.as_str(),
        "none" | "null" | "no_rework" | "not_applicable" | "n_a" | "na"
    )
}

fn supplied_host_bridge_completion_result_allowed_next_node(
    args: &HostBridgeCompletionResultArgs<'_>,
    result: Option<&serde_json::Value>,
) -> Option<String> {
    args.allowed_next_node
        .filter(|value| host_bridge_completion_allowed_next_node_is_present(Some(value)))
        .or_else(|| {
            result?
...
fn write_rework_downstream_packet_fallback(
    ...
    if source_packet
        ...
        .and_then(|selection| selection.get("compiled_bundle"))
...
fn reconcile_lane_completion_allowed_next_node(
    surface_label: &str,
    ...
    let requested_allowed_next_node = requested_allowed_next_node
        .map(str::trim)
        .filter(|value| !value.is_empty());
    let persisted_allowed_next_node = persisted_allowed_next_node
        .map(str::trim)
        .filter(|value| !value.is_empty());
    let terminal_closure_target = role_selection
        .map(|selection| {
            crate::runtime_dispatch_state::terminal_closure_downstream_target_for_role_selection(
                selection, receipt,
            )
        })
        .transpose()?
    ...
    if requested_allowed_next_node == Some("next") {
        return Err(format!(
            "{surface_label} allowed_next_node `next` is not a concrete runtime route."
        ));
    }
    let Some(requested) = requested_allowed_next_node else {
        if let Some(persisted) = persisted_allowed_next_node {
            if terminal_closure_route(persisted) && terminal_closure_target.is_some() {
    ...
    let lawful_requested = if terminal_closure_route(requested) {
        let configured_lawful_requested = if let Some(selection) = role_selection {
            ...
            if configured_sequence.iter().any(|candidate| {
                [
                    candidate.node_id.as_str(),
                    candidate.dispatch_target.as_str(),
                    candidate.dispatch_alias.as_str(),
                ]
                .into_iter()
                .any(|identity| {
                    crate::runtime_assignment_policy::canonical_dispatch_target_name(identity)
                        == requested_target
                })
            }) {
                crate::runtime_dispatch_state::lawful_explicit_downstream_dispatch_target_for_role_selection(
                    selection,
                    receipt,
                    requested,
                )?
        ...
        let Some(selection) = role_selection else {
            return Err(format!(
                "{surface_label} TeamFlow authority selection is missing for `{requested}`"
            ));
        };
        if rework_target
            .map(str::trim)
        ...
        {
            return Err(format!(
                "{surface_label} TeamFlow rework admission requires typed receipt evidence for `{requested}`"
            ));
        }
        crate::runtime_dispatch_state::lawful_explicit_downstream_dispatch_target_for_role_selection(
            selection,
    ...
    if role_selection.is_some() && lawful_requested.is_none() {
        return Err(format!(
            "{surface_label} allowed_next_node `{requested}` is not the next lawful lane after `{}` in the execution plan.",
            receipt.dispatch_target
        ));
    }
    if let Some(persisted) = persisted_allowed_next_node {
        let effective_persisted =
            crate::runtime_dispatch_state::canonical_terminal_closure_dispatch_target(persisted)
                .unwrap_or(persisted);
        let persisted_transition_target = role_selection
            .map(|selection| {
                crate::runtime_dispatch_state::lawful_explicit_downstream_dispatch_target_for_role_selection(
                    selection,
                    receipt,
            ...
            .transpose()?
            .flatten();
        let persisted_matches = persisted == effective_requested
            || effective_persisted == effective_requested
            || persisted_transition_target.as_deref() == Some(effective_requested.as_str());
        let stale_terminal_closure = terminal_closure_route(persisted)
            && (lawful_requested.is_some() || terminal_closure_target.is_some());
...
fn rework_allowed_next_node_from_execution_plan(
    ...
    role_selection: Option<&crate::RuntimeConsumptionLaneSelection>,
    receipt: &crate::state_store::RunGraphDispatchReceipt,
    _completed_dispatch_target: &str,
    rework_target: Option<&str>,
) -> Result<Option<String>, String> {
    let Some(rework_target) = rework_target
        .map(str::trim)
        .filter(|value| !value.is_empty())
    else {
        return Ok(None);
    ...
    let Some(role_selection) = role_selection else {
        return Err("team_flow_role_selection_missing_for_rework".to_string());
    };
    let explicit_target = rework_target;
    crate::runtime_dispatch_state::lawful_explicit_rework_dispatch_target_for_role_selection(
        state_root,
        role_selection,
        receipt,
        explicit_target,
    ...
    .map(|target| target.or_else(|| Some(rework_target.to_string())))
}

fn canonical_implementation_rework_target_from_execution_plan(
    role_selection: Option<&crate::RuntimeConsumptionLaneSelection>,
    ...
    let completed_dispatch_target = completed_dispatch_target.trim().replace('-', "_");
    let ordered_nodes = authority.ordered_nodes().collect::<Vec<_>>();
    let completed_index = ordered_nodes.iter().position(|step| {
        step.dispatch_alias == completed_dispatch_target
            || step.node.node_id == completed_dispatch_target
    });
    Ok(ordered_nodes
        .iter()
        .enumerate()
        .take_while(|(index, _)| completed_index.is_none_or(|completed| *index < completed))
        .find(|(_, step)| {
            step.node.task_class == "implementation"
                || step.node.task_class.starts_with("implementation_")
        })
        .map(|(_, step)| step.dispatch_alias.clone()))
}

fn dispatch_contract_blocker_from_execution_plan(
    role_selection: Option<&crate::RuntimeConsumptionLaneSelection>,
    current_dispatch_target: &str,
) -> Option<String> {
    let role_selection = role_selection?;
    let authority =
        match crate::runtime_dispatch_state::require_persisted_team_flow_authority_for_selection(
            role_selection,
        ) {
            Ok(authority) => authority,
            Err(blocker) => return Some(blocker.code),
        };
    let selected_node_id = crate::runtime_dispatch_state::selected_flow_node_ref(role_selection)
        .map(str::trim)
        .filter(|value| !value.is_empty());
    let current_dispatch_target = current_dispatch_target.trim();
    let Some(current_dispatch_target) =
        (!current_dispatch_target.is_empty()).then_some(current_dispatch_target)
    else {
        return Some("team_flow_current_node_missing".to_string());
    };
    let selected_node = match selected_node_id {
        Some(selected_node_id) => {
            match authority.resolve_target(Some(&role_selection.execution_plan), selected_node_id) {
                Ok(node) => Some(node),
                Err(blocker) => return Some(blocker.code.to_string()),
            }
        }
        None => None,
    };
    let current_node = match authority.resolve_target(
    ...
    if let Some(selected_node) = selected_node {
        if selected_node.node_id != current_node.node_id {
            return Some("team_flow_selected_node_dispatch_target_mismatch".to_string());
        }
    }
    None
}
...
fn read_supplied_host_bridge_completion_result(
    ...
    requestless_expected_binding: Option<(&str, &str)>,
    ...
    let result = read_host_bridge_json_artifact_at_path(&canonical_path)?;
    if let Some((run_id, dispatch_target)) = requestless_expected_binding {
        let expected_binding = serde_json::json!({
            "run_id": run_id,
            "dispatch_target": dispatch_target,
        });
        validate_materialized_host_bridge_result_contract(
            &canonical_path,
            &expected_binding,
            &result,
        )?;
...
fn supplied_host_bridge_completion_result_path(
    state_root: &Path,
    result_file: Option<&str>,
    ...
    let normalized_path =
        crate::runtime_dispatch_state::normalize_persisted_runtime_path(result_file);
    canonicalize_existing_state_path(
        state_root,
        Path::new(&normalized_path),
        "host bridge result",
    )
...
fn emit_host_bridge_completion_error_envelope(
    ...
    let envelope = host_bridge_completion_error_envelope(run_id, request_path, result_file, error);
    crate::surface_render::print_surface_json(
        &envelope,
        true,
        "host bridge completion error envelope should serialize",
    );
    ExitCode::from(2)
}

fn host_bridge_completion_error_envelope(
    run_id: &str,
    request_path: Option<&str>,
    result_file: Option<&str>,
    error: &str,
) -> BlockedLaneEnvelope {
    let blocker_codes = vec!["host_bridge_completion_result_invalid".to_string()];
    let next_actions = vec![
        "inspect the host bridge request, supplied result file, and canonical result/receipt paths"
            .to_string(),
        "retry `vida agent host-bridge --submit-result` or `vida lane complete --host-bridge-result-file` with a validated result artifact".to_string(),
    ];
    let artifact_refs = serde_json::json!({
        "surface": "vida lane complete",
        "run_id": run_id,
        "host_bridge_request": request_path,
        "host_bridge_result_file": result_file,
        "error": error,
    });
    let operator_contracts = render_operator_contract_envelope(
        "blocked",
...
fn supplied_host_bridge_completion_result_rework_next_node(
    args: &HostBridgeCompletionResultArgs<'_>,
    result: Option<&serde_json::Value>,
) -> Option<String> {
    args.routable_rework_next_node().or_else(|| {
        let result = result?;
        let result_rework_requested = ["decision", "verdict"]
            .into_iter()
            .filter_map(|field| result.get(field).and_then(serde_json::Value::as_str))
            .map(str::trim)
            .any(host_bridge_completion_result_value_is_rework_alias);
        if !result_rework_requested
...
fn supplied_host_bridge_completion_result_is_blocked(
    ...
    let result_rework_target = result
        .get("rework_target")
        .and_then(serde_json::Value::as_str);
    let routable_rework_next_node =
        supplied_host_bridge_completion_result_rework_next_node(args, Some(result));
    let rework_alias_without_target = [args.decision, args.verdict]
        .into_iter()
        .flatten()
        .chain(
            ["decision", "verdict"]
...
fn trusted_host_bridge_completion_request_context(
    ...
    let packet_confirms_active_request =
    ...
    let completed_refresh_context = receipt.dispatch_status == "executed"
        && status.status == "blocked"
        && status.active_node.trim() == dispatch_target
        && matches!(
            request
                .get("status")
                .and_then(serde_json::Value::as_str)
    ...
    if !adapter_gate_context && !retryable_completion_context && !completed_refresh_context {
        return Ok(None);
    }
    ...
    if !receipt_target_matches_request && !packet_confirms_active_request {
        return Err(
...
fn validated_host_bridge_paths_from_receipt(
    ...
    if let Some(authoritative_packet_path) = paths.packet_path.as_ref() {
        ...
        if !taskflow_host_bridge::host_bridge_packet_paths_equivalent(
            &request_packet_path.display().to_string(),
            &authoritative_packet_path.display().to_string(),
        ) {
            return Err(
...
fn host_bridge_implementation_artifacts(
    ...
    if !taskflow_artifacts.artifacts.is_empty() {
        let artifacts = host_bridge_persisted_attempt_artifacts(
            &taskflow_artifacts.artifact_refs,
            &taskflow_artifacts.authority_keys,
            state_root,
        )
        .unwrap_or_else(|| serde_json::Value::Array(taskflow_artifacts.artifacts));
...
pub(crate) fn host_bridge_persisted_attempt_artifacts(
    ...
    let mut artifacts = Vec::with_capacity(artifact_refs.len());
    for artifact_ref in artifact_refs {
        ...
        let artifact = serde_json::from_str::<serde_json::Value>(&raw).ok()?;
        if !host_bridge_request_artifacts_are_taskflow_authorized(
            &serde_json::Value::Array(vec![artifact.clone()]),
            authority_keys,
        ) {
            return None;
        }
...
fn host_bridge_implementation_scope_validation(
    ...
    let proof_artifact_paths = host_bridge_request_proof_artifact_paths(request)
        .iter()
    ...
    let admission =
        taskflow_host_bridge::artifact_scope::host_bridge_implementation_attempt_scope_decision(
            request, artifacts,
        );
...
async fn authoritative_owned_paths_for_lane_completion(
    store: &StateStore,
    run_id: &str,
    packet: &serde_json::Value,
) -> Vec<String> {
    match store.show_task(run_id).await {
        Ok(task) if !task.planner_metadata.owned_paths.is_empty() => {
            task.planner_metadata.owned_paths
        }
        _ => owned_paths_from_lane_packet(packet),
    }
}
...
async fn taskflow_implementation_artifacts_for_host_bridge_request(
    ...
    for authority_key in
        host_bridge_request_artifact_ref_authority_keys(&request, &task, store.root())
    {
...
fn host_bridge_request_artifact_ref_authority_keys(
        ...
        .filter(|value| !value.is_empty())
    {
        ...
        let Some(authority_key) = host_bridge_request_artifact_authority_key(&artifact, task)
        else {
            continue;
        };
        if !keys.iter().any(|existing| existing == &authority_key) {
...
fn host_bridge_request_artifact_authority_key(
    artifact: &serde_json::Value,
    task: &crate::state_store::TaskRecord,
) -> Option<crate::runtime_dispatch_packets::TaskflowImplementationArtifactAuthority> {
    ...
    let attempt_id = object.get("attempt_id")?.as_str()?.trim();
    let task_id = object.get("task_id")?.as_str()?.trim();
    let stage_id = object.get("stage_id")?.as_str()?.trim();
    let freshness = object.get("freshness")?.as_str()?.trim();
    let consolidation_receipt_id = object.get("consolidation_receipt_id")?.as_str()?.trim();
    if attempt_id.is_empty()
        || task_id != task.id
        || stage_id != "implementation"
        || freshness != task.updated_at
        || consolidation_receipt_id.is_empty()
    {
        return None;
    }
    Some(
        crate::runtime_dispatch_packets::TaskflowImplementationArtifactAuthority {
            ...
            consolidation_receipt_id: consolidation_receipt_id.to_string(),
        },
    )
}
...
fn host_bridge_request_blocked_contract_allows_next_node(
    state_root: &Path,
    request_path: Option<&str>,
    ...
    let Some(allowed_next_node) = allowed_next_node
        .map(str::trim)
        .filter(|value| !value.is_empty() && !value.eq_ignore_ascii_case("next"))
    else {
        return false;
    };
    let Ok(request) = read_host_bridge_request(state_root, request_path) else {
...
fn materialize_host_bridge_completion_evidence(
    ...
    let retryable_completion_request = retry_override_has_routable_blocked_completion
        || host_bridge_request_has_retryable_completion_evidence(state_root, request_path)
    ...
    let request_id = request
        ...
        .filter(|value| !value.is_empty())
        .ok_or_else(|| {
            "host_bridge_completion_request_identity_missing: request_id is required".to_string()
        })?;
    ...
    let packet_path = request
        ...
        .filter(|value| !value.is_empty())
        .ok_or_else(|| {
            "host_bridge_completion_request_identity_missing: packet_path is required".to_string()
        })?;
    let backend_id = request
        ...
        .filter(|value| !value.is_empty())
        .ok_or_else(|| {
            "host_bridge_completion_request_identity_missing: backend_id is required".to_string()
        })?;
    let summary = summary
        .map(str::trim)
        .filter(|value| !value.is_empty())
    ...
    let implementation_artifacts = if supplied_no_code_change {
        HostBridgeImplementationArtifacts {
            ...
            source: "explicit_no_code_change",
            artifact_refs: Vec::new(),
            blocker_codes: Vec::new(),
        }
    } else {
    ...
    if let Some(blocker_code) =
        dispatch_contract_blocker_from_execution_plan(role_selection, dispatch_target)
    {
        blocker_codes.push(blocker_code.to_string());
    }
    if requires_implementation_artifacts && !supplied_no_code_change {
    ...
    let authority_decision =
        decide_host_bridge_completion_authority(HostBridgeCompletionAuthorityInput {
            decision: supplied_decision
                .map(str::trim)
                .filter(|value| !value.is_empty())
                .unwrap_or(if blocker_codes.is_empty() {
                    "approve"
                } else {
                    "blocked"
                })
                .to_string(),
            verdict: supplied_verdict
                .map(str::trim)
                .filter(|value| !value.is_empty())
                .unwrap_or(if blocker_codes.is_empty() {
                    "pass"
    ...
    blocker_codes = authority_decision.blocker_codes.clone();
    ...
    let canonical_rework_target = if let Some(target) = supplied_rework_target {
        Some(target.to_string())
    } else if !blocker_codes.is_empty() {
        canonical_implementation_rework_target_from_execution_plan(role_selection, dispatch_target)?
    } else {
        None
    };
    let blocked_contract_allows_next = allowed_next_node
        .map(str::trim)
        .filter(|value| !value.is_empty() && !value.eq_ignore_ascii_case("next"))
        .is_some_and(|allowed| {
            host_bridge_blocked_result_contract(&request)
                .is_some_and(host_bridge_blocked_result_contract_is_retryable)
                && host_bridge_blocked_result_contract_allowed_next_node(&request) == Some(allowed)
        });
    ...
    let result_verdict = host_bridge_result_verdict_fields_for_gate(
        dispatch_target,
        &blocker_codes,
        canonical_rework_target.as_deref(),
    );
    let supplied_rework_requested = [supplied_decision, supplied_verdict]
        ...
        .map(str::trim)
        .any(host_bridge_completion_result_value_is_rework_alias)
        || allowed_next_node.map(str::trim) == Some(result_verdict.allowed_next_node.as_str());
    let rework_allowed_next_node = rework_allowed_next_node_from_execution_plan(
        state_root,
        role_selection,
        persisted_receipt,
        dispatch_target,
    ...
    let effective_allowed_next_node = if blocker_codes.is_empty() {
    ...
    let result = serde_json::json!({
        ...
        "submitted_result_path": submitted_result_path.clone(),
        "submitted_result_source": if submitted_result_path.is_some() { "operator_supplied_result_file" } else { "runtime_generated_completion" },
        "summary": summary,
    ...
    if let Some(object) = request.as_object_mut() {
        ...
        object.insert(
            "submitted_result_path".to_string(),
...
async fn rehydrate_lane_completion_packet_context(
    store: &StateStore,
    run_id: &str,
    status: Option<&crate::state_store::RunGraphStatus>,
    packet: &serde_json::Value,
    ...
    let status = status.ok_or_else(|| {
        format!("lane_completion_role_selection_authority_identity_missing:{run_id}:run_status")
    })?;
    if status.run_id.trim() != run_id.trim() {
        return Err(format!(
            "lane_completion_role_selection_authority_run_mismatch:{}:{run_id}",
            status.run_id
        ));
    }
    ...
    if task_id.is_empty() {
        return Err(format!(
            "lane_completion_role_selection_authority_identity_missing:{run_id}:task_id"
        ));
    }
    store.show_task(task_id).await.map_err(|error| {
        format!(
            "lane_completion_role_selection_authority_task_unavailable:{run_id}:{task_id}:{error}"
        )
    })?;
    let role_selection = serde_json::to_value(role_selection).map_err(|error| {
        format!("Failed to serialize persisted lane role selection for rehydration: {error}")
    })?;
    ...
    .await
    .map_err(|error| format!("Failed to rehydrate persisted lane role selection: {error}"))?;
    if role_selection.compiled_bundle.is_null() {
        return Err(
            "lane_completion_role_selection_rehydration_missing_compiled_bundle".to_string(),
        );
    }
    Ok(Some((role_selection, run_graph_bootstrap)))
}
...
pub(crate) async fn run_lane(args: ProxyArgs) -> ExitCode {
    ...
    match &command {
        LaneCommand::ShowLatest { as_json } => {
            ...
            let mut truth = derive_lane_show_truth_with_exception_metadata(
                &summary,
                recovery.as_ref(),
                exception_path_metadata.as_ref(),
            );
            let authorized_rework_route =
                lane_rework_route_for_envelope(&store, &summary, status.as_ref(), &mut truth).await;
            let envelope = build_lane_envelope_with_owned_scope_and_rework_route(
                summary,
                status,
                exception_path_metadata_path,
                exception_path_metadata,
        ...
        LaneCommand::TakeoverReady { run_id, as_json } => {
            ...
            let authorized_rework_route =
                lane_rework_route_for_envelope(&store, &summary, status.as_ref(), &mut truth).await;
            let lane_envelope = build_lane_envelope_with_owned_scope_and_rework_route(
                summary,
                status,
                exception_path_metadata_path
                    .exists()
    ...
    match command {
        ...
        LaneCommand::Complete {
            ...
            let original_receipt_dispatch_target = receipt.dispatch_target.clone();
            ...
            {
                if !pending_receipt_has_persisted_dispatch_evidence {
            ...
            let mut lane_completion_packet_context = match rehydrate_lane_completion_packet_context(
                &store,
                run_id,
                status.as_ref(),
                &packet,
            ...
            if let (Some((role_selection, _)), Some(status)) =
                (lane_completion_packet_context.as_mut(), status.as_ref())
            {
                if let Err(error) =
                    crate::taskflow_run_graph::project_selected_node_for_run_graph_status(
                        role_selection,
                        status,
            ...
            let supplied_completion_result = match read_supplied_host_bridge_completion_result(
                ...
                host_bridge_request
                    .is_none()
                    .then_some((run_id, receipt.dispatch_target.as_str())),
            ) {
                Ok(result) => result,
                Err(error) => {
                    return emit_host_bridge_completion_error_envelope(
            ...
            let supplied_completion_result_path = match supplied_host_bridge_completion_result_path(
                store.root(),
                supplied_completion_result_args.result_file,
            ) {
                Ok(path) => path,
            ...
            let supplied_completion_allowed_next_node =
                supplied_host_bridge_completion_result_allowed_next_node(
                    &supplied_completion_result_args,
                    supplied_completion_result.as_ref(),
                );
            ...
            let host_bridge_evidence = if let Some(request_path) = host_bridge_request {
                ...
                .await;
                let authoritative_owned_paths =
                    authoritative_owned_paths_for_lane_completion(&store, run_id, &packet).await;
                ...
                {
                    original_receipt_dispatch_target.as_str()
                } else {
                ...
                match materialize_host_bridge_completion_evidence(
                    store.root(),
                    request_path,
                    supplied_completion_result_path.as_deref(),
                    run_id,
                    ...
                    allowed_next_node
                        .map(str::trim)
                        .filter(|value| !value.is_empty())
                        .or(supplied_completion_allowed_next_node.as_deref()),
                    ...
                    allow_reconciled_request_paths,
                    retry_host_bridge_completion,
                    host_bridge_result_declares_no_code_change(supplied_completion_result.as_ref()),
                ) {
            ...
            let effective_allowed_next_node = allowed_next_node
                ...
                .filter(|value| !value.is_empty())
                .map(str::to_string)
                .or_else(|| supplied_completion_allowed_next_node.clone())
                .or_else(|| {
            ...
            let typed_rework_completion_allows_next = blocked_contract_allows_next
                || (supplied_completion_result_rework_target
                    .as_deref()
                    .is_some_and(|target| !target.trim().is_empty())
                    && host_bridge_evidence.as_ref().is_some_and(|evidence| {
                        evidence.execution_state == "blocked"
                            && evidence.allowed_next_node.as_deref()
                                == effective_allowed_next_node.as_deref()
                    }));
            let effective_allowed_next_node = if terminal_closure_route(&persisted_completed_target)
            {
                None
            } else if typed_rework_completion_allows_next {
                effective_allowed_next_node.clone()
            } else {
            ...
            let accepted_rework_completion = typed_rework_completion_allows_next
                ...
                && ([decision, verdict]
                    ...
                    .map(str::trim)
                    .any(host_bridge_completion_result_value_is_rework_alias)
                    || supplied_completion_result.as_ref().is_some_and(|result| {
                        ["decision", "verdict"]
                            .into_iter()
                            .filter_map(|field| {
            ...
            let completed_target = if accepted_rework_completion {
                effective_allowed_next_node
                    ...
                    .unwrap_or(persisted_completed_target.as_str())
                    .to_string()
            } else {
                persisted_completed_target
            };
            ...
            if let Some(evidence) = host_bridge_evidence.as_ref() {
                ...
                if let Some(value) = host_bridge_result_file
                    .map(str::trim)
                    .filter(|value| !value.is_empty())
                {
                    dispatch_command.push_str(&format!(
            ...
            if !completion_blocked {
                ...
                match lane_completion_packet_context.as_ref() {
                    Some((role_selection, run_graph_bootstrap)) => {
            ...
            if !completion_blocked {
                if let Some(value) = host_bridge_result_file_allowed_next_node(
                ...
                .or_else(|| (!completed_target.trim().is_empty()).then(|| completed_target.clone()))
                {
                    if host_bridge_result_file_rework_target(&completion_result_path).is_some()
                        || host_bridge_result_file_completed_target(&completion_result_path)
                            .as_deref()
                            == Some(value.as_str())
                ...
                && receipt.downstream_dispatch_status.as_deref() == Some("packet_ready")
            {
                let fallback_packet_context = if lane_completion_packet_context.is_none() {
                    ...
                    match fallback_packet {
                        Some(fallback_packet) => match rehydrate_lane_completion_packet_context(
                            &store,
                            run_id,
                            status.as_ref(),
                            &fallback_packet,
            ...
            if receipt.dispatch_status == "executed" {
                if let Some(current_status) = status.as_ref() {
                    ...
                    if let Err(error) =
                        crate::taskflow_run_graph::persist_selected_node_for_run_graph_transition(
                            &store,
                            &executed_status,
                        )
                        .await
                    {
                        eprintln!(
                            "Failed to persist TeamFlow selected node after lane completion: {error}"
                        );
                        return ExitCode::from(1);
                    }
                    if let Err(error) =
        ...
        LaneCommand::Retire {
                ...
                Err(error) => {
                    ...
                    match metadata_task_id {
                        Some(task_id) => match store.show_task(task_id).await {
                            Ok(task)
                                if crate::state_store::StateStore::task_status_is_closed_like(
                                    &task.status,
                                ) => {}
                            Ok(task) => {
                                return emit_lane_retire_blocked_envelope(
                                    ...
                                    run_id,
                                    receipt_id,
                                    "lane_retire_exception_unit_not_closed",
                                    format!(
                            ...
                            Err(metadata_error) => {
                                ...
                                if !missing_exception_task || !missing_task_stale_blocked_run {
                                    return emit_lane_retire_blocked_envelope(
                                        as_json,
                                        run_id,
                                        receipt_id,
                                        "lane_retire_exception_unit_verification_failed",
                                        format!(
                        ...
                        None => {
                            if !missing_task_stale_blocked_run {
                                return emit_lane_retire_blocked_envelope(
                                    ...
                                    run_id,
                                    receipt_id,
                                    "lane_retire_task_verification_failed",
                                    format!(
            ...
            if let Err(error) = acquire_lane_retire_closed_run_claim(
                &store,
                &operator_session_projection,
                run_id,
                &status.task_id,
            ...
            .await
            {
                eprintln!("Failed to claim closed stale run `{run_id}` before retire: {error}");
                return ExitCode::from(1);
            }
            ...
            if let Err(error) = store
                .record_reconciled_terminal_closure_run_graph_status(&retired_status)
                .await
            {
                return emit_lane_retire_blocked_envelope(
                    as_json,
                    ...
                    format!("Failed to persist retired run-graph status `{run_id}`: {error}"),
                    receipt.dispatch_packet_path.clone(),
                    receipt.dispatch_result_path.clone(),
                );
            }
...
mod tests {
    ...
    fn lane_complete_role_selection(_dev_task_id: &str) -> crate::RuntimeConsumptionLaneSelection {
        let project_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("..")
            .join("..");
        let config =
            crate::runtime_dispatch_state::load_project_overlay_yaml_for_root(&project_root)
                .expect("lane fixture project config should load");
        let compiled_bundle =
            crate::build_compiled_agent_extension_bundle_for_root(&config, &project_root)
                .expect("lane fixture compiled bundle should build");
        crate::runtime_lane_summary::build_runtime_lane_selection_from_bundle(
            &compiled_bundle,
            "test",
            &crate::launcher_activation_snapshot::pack_router_keywords_json(&config),
            "continue development",
        )
        .expect("lane fixture selection should resolve from project config")
    }

    fn lane_complete_role_selection_for_node(
        task_id: &str,
        node_id: &str,
    ) -> crate::RuntimeConsumptionLaneSelection {
        let mut selection = lane_complete_role_selection(task_id);
        let flow_id = selection.compiled_bundle["team_flow_authority"]["resolved_all_flow_payload"]
            ["flows"]
            .as_array()
            .and_then(|flows| {
                flows.iter().find_map(|flow| {
                    ...
                    let authority =
                        ...
                        .ok()?;
                    let node = authority.node(node_id)?;
                    (node.node.included
                        && node
                            .node
                            .rework_targets
                            .iter()
                            .any(|target| target == "coder"))
                    .then(|| flow_id.to_owned())
                })
            })
            .unwrap_or_else(|| panic!("canonical config should expose {node_id} flow"));
        let compiled_bundle = selection.compiled_bundle.clone();
        ...
        .unwrap_or_else(|error| panic!("canonical {node_id} flow should normalize: {error}"));
        selection
    }

    fn lane_complete_role_selection_with_configured_terminal_predecessor(
        task_id: &str,
    ) -> crate::RuntimeConsumptionLaneSelection {
        let mut selection = lane_complete_role_selection(task_id);
        let flow_id = selection.compiled_bundle["team_flow_authority"]["resolved_all_flow_payload"]
            ...
            .and_then(|flows| {
                flows.iter().find_map(|flow| {
                    let lanes = flow["lanes"].as_array()?;
                    let terminal_id = lanes
                        .iter()
                        .find(|lane| lane["terminal"].as_bool() == Some(true))
                        .and_then(|lane| lane["node_id"].as_str())?;
                    let has_included_predecessor = lanes.iter().any(|lane| {
                        lane["included"].as_bool() != Some(false)
                            && lane["next_node"].as_str() == Some(terminal_id)
                    });
                    has_included_predecessor
                        .then(|| flow["flow_id"].as_str().map(ToOwned::to_owned))
                        .flatten()
                })
            })
            .expect("project config should expose a flow with terminal predecessor");
        let compiled_bundle = selection.compiled_bundle.clone();
        ...
        .expect("terminal predecessor fixture should normalize from project config");
        selection
    }

    async fn write_lane_activation_snapshot_for_store(
        store: &crate::state_store::StateStore,
    ) -> Result<(), String> {
        let project_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("..")
        ...
        let config =
            crate::runtime_dispatch_state::load_project_overlay_yaml_for_root(&project_root)?;
        let bundle = crate::build_compiled_agent_extension_bundle_for_root(&config, &project_root)
        ...
        let snapshot = crate::state_store::LauncherActivationSnapshot {
            ...
            pack_router_keywords: crate::launcher_activation_snapshot::pack_router_keywords_json(
                &config,
            ),
        };
        store
    ...
    fn compact_lane_completion_packet(
        role_selection: &crate::RuntimeConsumptionLaneSelection,
        run_id: &str,
    ) -> serde_json::Value {
        let mut persisted = serde_json::to_value(role_selection)
            .expect("lane completion role selection should serialize");
        persisted["compiled_bundle"] = serde_json::Value::Null;
        serde_json::json!({
            "run_id": run_id,
            "role_selection_full": persisted,
            "run_graph_bootstrap": {"run_id": run_id}
        })
    }

    fn canonical_rework_lane_packet(
        role_selection: &crate::RuntimeConsumptionLaneSelection,
        run_id: &str,
        source_dispatch_target: &str,
        dispatch_target: &str,
        ...
        let mut packet = compact_lane_completion_packet(role_selection, run_id);
        packet["source_dispatch_target"] = serde_json::json!(source_dispatch_target);
        packet["dispatch_target"] = serde_json::json!(dispatch_target);
        packet["activation_runtime_role"] = serde_json::json!(activation_runtime_role);
        packet["packet_template_kind"] = serde_json::json!(packet_template_kind);
        packet["owned_paths"] = serde_json::json!(["crates/vida/src/lane_surface.rs"]);
        packet["read_only_paths"] = serde_json::json!([".vida/data/state/runtime-consumption"]);
        packet["delivery_task_packet"] = serde_json::json!({
            "goal": goal,
        ...
        if packet_template_kind == "verifier_proof_packet" {
            let mut verifier_packet = packet["delivery_task_packet"].clone();
            verifier_packet["proof_goal"] = serde_json::json!(goal);
            packet["verifier_proof_packet"] = verifier_packet;
        }
        packet["downstream_dispatch_active_target"] = serde_json::json!(dispatch_target);
        packet["downstream_dispatch_ready"] = serde_json::json!(false);
        packet["downstream_dispatch_blockers"] = serde_json::json!([blocker_code]);
        packet["downstream_dispatch_status"] = serde_json::json!("blocked");
        packet["downstream_lane_status"] = serde_json::json!("lane_blocked");
        packet
    }
    ...
    fn canonical_rework_route_for_test(
        ...
        crate::runtime_dispatch_result_evidence::DispatchReworkRoute {
            ...
            receipt_backed: true,
            outcome_blocker_codes: vec!["verification_rework_required".to_string()],
        }
    }

    fn canonical_lane_completion_route(
        role_selection: &crate::RuntimeConsumptionLaneSelection,
    ) -> Result<
        (
    ...
    > {
        ...
        sequence
            .iter()
            .find_map(|current| {
                let next_node_id = current.next_node.as_deref()?;
                let next = sequence
                    .iter()
                    .find(|candidate| candidate.node_id == next_node_id)?;
                Some((current.clone(), next.clone()))
            })
            .ok_or_else(|| "team_flow_canonical_successor_missing".to_string())
    }

    #[tokio::test]
    async fn lane_completion_compact_role_selection_rehydrates_before_rework_authority() {
        let _guard = acquire_lane_surface_test_lock();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_nanos())
            .unwrap_or(0);
        let root = std::env::temp_dir().join(format!(
            "vida-lane-completion-compact-rehydrate-{}-{}",
            std::process::id(),
            nanos
        ));
        let store = StateStore::open(root.clone())
            .await
            .expect("open lane completion state store");
        write_lane_activation_snapshot_for_store(&store)
            .await
            .expect("write lane completion activation snapshot");
        let task_id = "lane-completion-compact-rehydrate-task";
        store
            .create_task_with_fixture_parent(crate::state_store::CreateTaskRequest {
                task_id,
                title: "Lane completion compact rehydrate",
                display_id: None,
                description: "Lane completion compact rehydrate",
                issue_type: "defect",
                status: "in_progress",
                priority: 1,
                parent_id: None,
                ...
                planner_metadata: crate::state_store::TaskPlannerMetadata::default(),
                created_by: "test",
                source_repo: &root.display().to_string(),
            })
            .await
            .expect("create lane completion task");
        let run_id = "lane-completion-compact-rehydrate-run";
        let mut status =
            crate::taskflow_run_graph::default_run_graph_status(task_id, "worker", "worker");
        status.run_id = run_id.to_string();
        let packet = compact_lane_completion_packet(&lane_complete_role_selection(task_id), run_id);
        let (rehydrated, _) =
            rehydrate_lane_completion_packet_context(&store, run_id, Some(&status), &packet)
                .await
                .expect("compact lane packet should rehydrate")
                .expect("compact lane packet should include context");
        assert!(rehydrated.compiled_bundle.is_object());
        let (current_node, next_node) = canonical_lane_completion_route(&rehydrated)
            .expect("configured successor should resolve");
        assert_eq!(
            current_node.next_node.as_deref(),
            Some(next_node.node_id.as_str())
        );
        let mut receipt = sample_receipt("executed");
        receipt.run_id = run_id.to_string();
        receipt.dispatch_target = current_node.node_id.clone();
        let requested = next_node.node_id.clone();
        let resolved = reconcile_lane_completion_allowed_next_node(
            "lane completion test",
            Some(&rehydrated),
            &receipt,
            Some(&requested),
            None,
        ...
        .expect("rehydrated completion should admit the next lawful lane");
        assert_eq!(
            resolved.as_deref(),
            Some(next_node.dispatch_target.as_str())
        );
        drop(store);
        let _ = std::fs::remove_dir_all(&root);
    }
    ...
    #[tokio::test]
    async fn lane_completion_tampered_or_missing_authority_blocks_before_packet_mutation() {
        let _guard = acquire_lane_surface_test_lock();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_nanos())
            .unwrap_or(0);
        let root = std::env::temp_dir().join(format!(
            "vida-lane-completion-authority-block-{}-{}",
            std::process::id(),
            nanos
        ));
        let store = StateStore::open(root.clone())
        ...
        write_lane_activation_snapshot_for_store(&store)
            .await
            .expect("write lane completion activation snapshot");
        let task_id = "lane-completion-authority-block-task";
        store
            .create_task_with_fixture_parent(crate::state_store::CreateTaskRequest {
                task_id,
                title: "Lane completion authority block",
                display_id: None,
                description: "Lane completion authority block",
                issue_type: "defect",
                status: "in_progress",
                priority: 1,
                parent_id: None,
            ...
            .await
            .expect("create lane completion task");
        let run_id = "lane-completion-authority-block-run";
        let mut status =
            crate::taskflow_run_graph::default_run_graph_status(task_id, "worker", "worker");
        status.run_id = run_id.to_string();
        let valid_compact_packet =
            compact_lane_completion_packet(&lane_complete_role_selection(task_id), run_id);
        let (valid_context, _) = rehydrate_lane_completion_packet_context(
            &store,
            run_id,
            Some(&status),
            &valid_compact_packet,
        )
        .await
        .expect("valid compact TeamFlow authority should rehydrate")
        .expect("valid compact TeamFlow authority should include context");
        assert!(valid_context.compiled_bundle.is_object());
        let mut tampered = valid_compact_packet.clone();
        ...
        let error =
            rehydrate_lane_completion_packet_context(&store, run_id, Some(&status), &tampered)
                .await
                .expect_err("tampered TeamFlow authority must fail closed");
        assert!(error.contains("team_flow_authority_selected_node_id_unknown"));
        assert_eq!(tampered, tampered_before);

        let missing_identity = valid_compact_packet;
        let mut missing_status = status;
        missing_status.task_id.clear();
        let missing_before = missing_identity.clone();
        let error = rehydrate_lane_completion_packet_context(
            &store,
            run_id,
            Some(&missing_status),
            &missing_identity,
        ...
        .await
        .expect_err("missing task identity must fail closed");
        assert!(error.contains("authority_identity_missing"));
        assert_eq!(missing_identity, missing_before);
        drop(store);
        let _ = std::fs::remove_dir_all(&root);
    }
    ...
    fn canonical_host_bridge_identity(dispatch_target: &str) -> (String, String, String, String) {
        let bundle = crate::team_flow_authority_adapter::test_support::canonical_compiled_bundle();
        let backend_id = bundle
            .get("carrier_runtime")
            .and_then(|value| value.get("executor_backend_relation"))
        ...
        if let Some(flows) = bundle
            ...
            .and_then(|value| value.get("flows"))
            .and_then(serde_json::Value::as_array)
        {
            for flow in flows {
                let Some(lanes) = flow.get("lanes").and_then(serde_json::Value::as_array) else {
                    continue;
                };
                for lane in lanes {
                    let matches_canonical_identity = [
                        lane.get("node_id"),
                        lane.get("dispatch_target"),
                        lane.get("dispatch_alias"),
                    ]
                    .into_iter()
                    .flatten()
                    .filter_map(serde_json::Value::as_str)
                    ...
                    let carrier_id = lane
                        ...
                        .and_then(serde_json::Value::as_str)
                        .or_else(|| {
                            lane.get("carrier_relation")
                                .and_then(|relation| relation.get("selected_id"))
                                .and_then(serde_json::Value::as_str)
                        })
                        .or_else(|| {
                            lane.get("carrier_runtime_assignment")
                                .and_then(|assignment| assignment.get("selected_carrier_id"))
                                .and_then(serde_json::Value::as_str)
                        })
                        .expect("canonical carrier id")
                        .to_string();
                    let runtime_role = lane
                        .get("runtime_role")
                        .and_then(serde_json::Value::as_str)
                        .expect("canonical runtime role")
                        .to_string();
                    let task_class = lane
                        .get("task_class")
                        .and_then(serde_json::Value::as_str)
                        .expect("canonical task class")
                        .to_string();
                    selected = Some((carrier_id, runtime_role, task_class));
                    break;
                }
                if selected.is_some() {
                    break;
        ...
        let (carrier_id, runtime_role, task_class) =
            selected.expect("canonical dispatch target identity");
        (backend_id, carrier_id, runtime_role, task_class)
    }
    ...
    fn current_host_bridge_request_fixture(
        ...
        ) = canonical_host_bridge_identity(dispatch_target);
        object
        ...
        object
            .entry("runtime_role")
            .or_insert_with(|| serde_json::json!(canonical_runtime_role));
        object
            .entry("backend_id")
            .or_insert_with(|| serde_json::json!(canonical_backend_id));
        object
            .entry("carrier_id")
            .or_insert_with(|| serde_json::json!(canonical_carrier_id));
        object
            .entry("task_class")
            .or_insert_with(|| serde_json::json!(canonical_task_class));
        object
    ...
    fn normalize_current_host_bridge_request_file(
        ...
        std::fs::write(request_path, request.to_string())
            .expect("normalize host bridge request fixture");
    }
    ...
    fn normalize_current_host_bridge_packet_file(
        ...
        for (field, value) in [
            ...
            (
                "attempt_id",
                request.get("attempt_id").cloned().unwrap_or_else(|| {
                    serde_json::json!(format!("{run_id}::host-bridge-fixture-attempt"))
                }),
            ),
            (
                "packet_id",
                request.get("packet_id").cloned().unwrap_or_else(|| {
                    serde_json::json!(format!("{run_id}::host-bridge-fixture-packet"))
                }),
            ),
            ...
            ("dispatch_transport", serde_json::json!("host_tool_bridge")),
        ] {
        ...
        std::fs::write(packet_path, packet.to_string())
            .expect("normalize host bridge packet fixture");
    }
    ...
    fn lane_completion_reconciles_stale_current_lane_marker_to_requested_next() {
        let role_selection = lane_complete_role_selection("ldr-074j");
        let configured_nodes =
            crate::runtime_dispatch_state::typed_lane_node_sequence(&role_selection, true)
                .expect("configured TeamFlow nodes should resolve");
        let current = configured_nodes
            .first()
            .expect("configured TeamFlow current node should exist");
        let requested_next = configured_nodes
            .get(1)
            .expect("configured TeamFlow successor should exist");
        let mut receipt = sample_receipt("bridge_request_pending");
        receipt.dispatch_target = current.node_id.clone();
        receipt.downstream_dispatch_target = Some(current.node_id.clone());
        receipt.downstream_dispatch_active_target = Some(current.node_id.clone());

        let target = reconcile_lane_completion_allowed_next_node(
            "Lane completion",
            Some(&role_selection),
            &receipt,
            Some(requested_next.node_id.as_str()),
            Some(current.node_id.as_str()),
            None,
        )
        ...
        assert_eq!(
            target.as_deref(),
            Some(requested_next.dispatch_target.as_str())
        );
    }
    ...
    #[test]
    fn lane_completion_accepts_terminal_closure_after_final_execution_lane() {
        let role_selection = lane_complete_role_selection_with_configured_terminal_predecessor(
            "terminal-closure-run",
        );
        let configured_nodes =
            crate::runtime_dispatch_state::typed_lane_node_sequence(&role_selection, true)
                .expect("configured TeamFlow nodes should resolve");
        let terminal_node = configured_nodes
            .iter()
            .find(|node| node.terminal)
            .expect("configured TeamFlow terminal node should exist");
        let predecessor = configured_nodes
            .iter()
            .find(|node| node.next_node.as_deref() == Some(terminal_node.node_id.as_str()))
            .expect("configured TeamFlow terminal predecessor should exist");
        let mut receipt = sample_receipt("bridge_request_pending");
        receipt.dispatch_kind = "agent_lane".to_string();
        receipt.dispatch_target = predecessor.node_id.clone();
        receipt.downstream_dispatch_active_target = Some(predecessor.node_id.clone());

        let target = reconcile_lane_completion_allowed_next_node(
            "Host bridge completion",
            Some(&role_selection),
            &receipt,
            Some(terminal_node.node_id.as_str()),
            None,
            None,
        )
        .expect("configured terminal node should follow its predecessor");
        assert_eq!(
            target.as_deref(),
            Some(terminal_node.dispatch_target.as_str())
        );

        receipt.dispatch_target = terminal_node.node_id.clone();
        receipt.downstream_dispatch_active_target = Some(terminal_node.node_id.clone());
        for requested in [
            terminal_node.node_id.as_str(),
            "closure",
            "terminal_closure",
            "release_closure",
        ] {
            ...
            .expect_err("terminal nodes and closure aliases must not fabricate a successor");
            assert!(
                error.contains("team_flow_node_resolution_missing")
                    || error.contains("not the next lawful lane"),
    ...
    fn lane_completion_terminal_alias_precedence_matrix() {
        let role_selection = lane_complete_role_selection("run-host-bridge-prover-pass-summary");
        let (current, requested_next) = canonical_lane_completion_route(&role_selection)
            .expect("configured TeamFlow successor should resolve");
        let mut receipt = sample_receipt("bridge_request_pending");
        receipt.dispatch_target = current.node_id.clone();
        receipt.downstream_dispatch_active_target = Some(current.node_id.clone());
        receipt.downstream_dispatch_last_target = Some(current.node_id.clone());
        receipt.downstream_dispatch_target = Some(requested_next.node_id.clone());

        assert_eq!(
            crate::runtime_dispatch_state::lawful_explicit_downstream_dispatch_target_for_role_selection(
                &role_selection,
                &receipt,
                &requested_next.node_id,
            )
            .expect("typed TeamFlow downstream target")
            .as_deref(),
            Some(requested_next.dispatch_target.as_str())
        );
        assert!(
            ...
            .expect("typed TeamFlow terminal target")
            .is_none()
        );
        assert_eq!(
            reconcile_lane_completion_allowed_next_node(
                ...
                Some(&role_selection),
                &receipt,
                Some(&requested_next.node_id),
                Some(&requested_next.node_id),
                None,
            )
            .expect("configured TeamFlow successor should win before terminal alias fallback")
            .as_deref(),
            Some(requested_next.dispatch_target.as_str())
        );
        ...
        assert!(
            mismatch.contains("team_flow_node_resolution_missing")
        ...
        assert!(
            unknown.contains("team_flow_node_resolution_missing")
                || unknown.contains("not the next lawful lane"),
    ...
    #[tokio::test]
    async fn lane_show_latest_prefers_active_exception_takeover_over_current_session_receipt() {
        let _guard = acquire_lane_surface_test_lock();
        let saved_session_id = std::env::var("VIDA_SESSION_ID").ok();
        unsafe {
            std::env::set_var(
                "VIDA_SESSION_ID",
                "lane-show-latest-active-exception-priority",
            );
        }
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            ...
            .unwrap_or(0);
        let root = std::env::temp_dir().join(format!(
            "vida-lane-surface-latest-active-priority-{}-{}",
            std::process::id(),
            nanos
        ));
        let store = StateStore::open(root.clone()).await.expect("open store");
        let _state_override = ProxyStateDirOverrideGuard::install(root.clone());
        let active_run_id = "run-active-exception-priority";
        let current_run_id = "run-current-session-non-exception";
        ...
        current_status.active_node = "analyst".to_string();
        current_status.lifecycle_stage = "analyst_dispatch_ready".to_string();
        store
            .record_run_graph_status(&current_status)
            .await
            .expect("persist current-session status");

        let mut current_receipt = sample_receipt("routed");
        current_receipt.run_id = current_run_id.to_string();
        current_receipt.dispatch_target = "analyst".to_string();
        current_receipt.lane_status = crate::LaneStatus::LaneRunning.as_str().to_string();
        current_receipt.blocker_code = None;
        current_receipt.recorded_at = "2026-06-25T01:00:00Z".to_string();
        store
            .record_run_graph_dispatch_receipt(&current_receipt)
            .await
            .expect("persist current-session receipt");
        store
            .acquire_current_session_run_graph_claim_for_test(
                "lane-show-latest-current-session-non-exception-claim",
                current_run_id,
                current_run_id,
                "current-session-non-exception",
                "crates/vida/src/status_surface.rs",
            )
            .await
            .expect("claim current-session non-exception run");
        ...
        active_status.active_node = "analyst".to_string();
        active_status.lifecycle_stage = "analyst_blocked".to_string();
        store
            .record_run_graph_status(&active_status)
            .await
            .expect("persist active exception status");

        let mut active_receipt = sample_receipt("routed");
        active_receipt.run_id = active_run_id.to_string();
        active_receipt.dispatch_target = "analyst".to_string();
        active_receipt.lane_status = crate::LaneStatus::LaneExceptionTakeover
            .as_str()
            .to_string();
        active_receipt.exception_path_receipt_id = Some("active-exception-1".to_string());
        active_receipt.supersedes_receipt_id = Some("active-exception-1".to_string());
        active_receipt.recorded_at = "2026-06-24T01:00:00Z".to_string();
        store
            .record_run_graph_dispatch_receipt(&active_receipt)
            .await
            .expect("persist active exception receipt");

        let metadata = ExceptionTakeoverMetadata {
            run_id: Some(active_run_id.to_string()),
            dispatch_target: Some("analyst".to_string()),
            dispatch_packet_path: None,
            source_exception_path_receipt_id: Some("active-exception-1".to_string()),
            reason_class: "open_delegated_cycle".to_string(),
            active_bounded_unit: "active-exception-priority".to_string(),
            owned_write_scope: vec!["crates/vida/src/lane_surface.rs".to_string()],
            why_delegated_or_rerouted_path_is_not_currently_lawful: "delegated lane is open"
                .to_string(),
            why_local_write_is_the_smallest_safe_bounded_workaround: "bounded status parity"
                .to_string(),
            return_to_normal_posture_condition: "focused proof passes".to_string(),
            verification_plan: vec!["lane show latest selects active exception".to_string()],
            recorded_at: "2026-06-24T01:00:00Z".to_string(),
        };
        write_exception_takeover_metadata(&root, active_run_id, &metadata)
            .expect("persist active exception metadata");
        ...
        let cached_json: serde_json::Value =
            serde_json::from_str(&cached).expect("cached lane show projection should be json");
        assert_eq!(cached_json["status"], "pass");
        assert_eq!(cached_json["run_id"], active_run_id);
        assert_eq!(cached_json["lane_status"], "lane_exception_takeover");
        assert_eq!(cached_json["exception_path_metadata_state"], "active");
        assert_eq!(cached_json["root_local_write_allowed"], true);
    ...
    #[tokio::test]
    async fn lane_show_latest_prefers_active_exception_takeover_over_stale_current_session_receipt()
    {
        let _guard = acquire_lane_surface_test_lock();
        let saved_session_id = std::env::var("VIDA_SESSION_ID").ok();
        unsafe {
            std::env::set_var("VIDA_SESSION_ID", "lane-show-latest-active-exception");
        }
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_nanos())
            .unwrap_or(0);
        let root = std::env::temp_dir().join(format!(
            "vida-lane-surface-latest-exception-over-current-{}-{}",
            std::process::id(),
            nanos
        ));
        let store = StateStore::open(root.clone()).await.expect("open store");
        let _state_override = ProxyStateDirOverrideGuard::install(root.clone());
        let labels: Vec<String> = Vec::new();
        let stale_run_id = "run-stale-current-session";
        let active_run_id = "run-active-exception-takeover";

        for task_id in [stale_run_id, active_run_id] {
            store
                .create_task_with_fixture_parent(crate::state_store::CreateTaskRequest {
                    task_id,
                    title: "Lane latest priority fixture",
                    display_id: None,
                    description: "Lane latest priority fixture",
                    issue_type: "defect",
                    status: "open",
                    priority: 1,
                    parent_id: None,
        ...
        stale_status.active_node = "junior".to_string();
        stale_status.next_node = Some("coach".to_string());
        stale_status.lifecycle_stage = "junior_complete".to_string();
        store
            .record_run_graph_status(&stale_status)
            .await
            .expect("persist stale status");
        ...
        stale_receipt.run_id = stale_run_id.to_string();
        stale_receipt.dispatch_target = "junior".to_string();
        stale_receipt.lane_status = crate::LaneStatus::LaneCompleted.as_str().to_string();
        stale_receipt.blocker_code = None;
        stale_receipt.recorded_at = "2026-06-24T00:00:00Z".to_string();
        store
            .record_run_graph_dispatch_receipt(&stale_receipt)
            .await
            .expect("persist stale receipt");
        store
            .record_run_graph_continuation_binding(
                &crate::state_store::RunGraphContinuationBinding {
                    ...
                    task_id: stale_run_id.to_string(),
                    ...
                    why_this_unit: "stale current-session continuation fixture".to_string(),
        ...
        active_status.status = "ready".to_string();
        active_status.active_node = "planning".to_string();
        active_status.next_node = Some("analyst".to_string());
        active_status.lifecycle_stage = "analyst_dispatch_ready".to_string();
        active_status.resume_target = "dispatch.analyst".to_string();
        ...
        active_receipt.exception_path_receipt_id = Some("active-exception-1".to_string());
        active_receipt.supersedes_receipt_id = Some("active-exception-1".to_string());
        active_receipt.recorded_at = "2026-06-25T00:00:00Z".to_string();
        store
            .record_run_graph_dispatch_receipt(&active_receipt)
            .await
            .expect("persist active exception receipt");
    ...
    #[test]
    fn lane_help_hides_legacy_completion_verdict_flags() {
        let help = lane_usage();

        assert!(help.contains("--host-bridge-result-file <path>"));
        for flag in [
            "--decision",
            "--verdict",
            "--allowed-next-node",
            ...
            assert!(
                !help.contains(flag),
                "canonical lane help must not advertise legacy completion verdict flag {flag}"
            );
        }
    }
    ...
    #[test]
    fn supplied_host_bridge_completion_rework_target_none_does_not_block_pass_result() {
        let args = HostBridgeCompletionResultArgs {
            result_file: None,
    ...
    #[test]
    fn host_bridge_completion_lane_args_normalizes_rework_alias() {
        let args = HostBridgeCompletionResultArgs {
            result_file: None,
            decision: Some("rework"),
            verdict: Some("rework"),
            allowed_next_node: Some("developer"),
            blocker_codes: Some("[]"),
            blocker_code: Vec::new(),
        ...
        assert_eq!(
            args.routable_rework_next_node().as_deref(),
            Some("developer")
        );

        let rework_target_args = HostBridgeCompletionResultArgs {
            result_file: None,
            decision: Some("rework"),
            verdict: Some("needs_update"),
            allowed_next_node: None,
            blocker_codes: Some("[]"),
            blocker_code: Vec::new(),
            rework_target: Some("developer"),
        };
        let rework_target_result = serde_json::json!({
            "status": "pass",
            "execution_state": "executed",
            "decision": "rework",
        ...
        assert!(!supplied_host_bridge_completion_result_is_blocked(
            &rework_target_args,
            Some(&rework_target_result)
        ));
        assert_eq!(
            rework_target_args.routable_rework_next_node().as_deref(),
            Some("developer")
        );

        let missing_target_args = HostBridgeCompletionResultArgs {
            allowed_next_node: None,
            ..args
        };
        let missing_target_result = serde_json::json!({
            "status": "pass",
            "execution_state": "executed",
            "decision": "rework",
            "verdict": "rework",
        ...
        assert!(supplied_host_bridge_completion_result_is_blocked(
            &missing_target_args,
            Some(&missing_target_result)
        ));

        let canonical_blocked_args = HostBridgeCompletionResultArgs {
            result_file: None,
            decision: Some("rework_required"),
            verdict: Some("rework_required"),
            allowed_next_node: Some("developer_rework"),
            blocker_codes: Some("[]"),
            blocker_code: Vec::new(),
            rework_target: Some("developer"),
        };
        assert!(!supplied_host_bridge_completion_result_is_blocked(
            &canonical_blocked_args,
            None
        ));
        let canonical_missing_target_args = HostBridgeCompletionResultArgs {
            allowed_next_node: None,
            rework_target: None,
            ..canonical_blocked_args
        };
        assert!(supplied_host_bridge_completion_result_is_blocked(
            &canonical_missing_target_args,
            None
        ));
    }
    ...
    fn lane_envelope_surfaces_ready_downstream_agent_init_execute_command() {
        ...
        assert_eq!(
            ...
            Some("vida agent-init --downstream-packet packet.json --execute-dispatch")
        );
        assert_eq!(
    ...
    #[test]
    fn lane_envelope_suppresses_downstream_command_for_terminal_closure() {
        let mut receipt = sample_receipt("executed");
        receipt.blocker_code = None;
        receipt.lane_status = crate::LaneStatus::LaneRunning.as_str().to_string();
        receipt.downstream_dispatch_ready = true;
        ...
        assert_eq!(envelope.status, "pass");
        assert!(envelope.next_actions.is_empty());
        assert!(envelope.next_action.is_none());
        assert!(envelope.recommended_command.is_none());
        assert!(envelope.recommended_surface.is_none());
    }

    #[test]
    fn lane_envelope_surfaces_routed_agent_dispatch_packet_execute_command() {
        let mut receipt = sample_receipt("routed");
        receipt.blocker_code = None;
    ...
    fn lane_envelope_prefers_authorized_rework_handoff_over_exception_takeover() {
        ...
        let packet_path =
            root.join(".vida/data/state/runtime-consumption/dispatch-packets/tester.json");
        let result_path =
            root.join(".vida/data/state/runtime-consumption/dispatch-results/tester-result.json");
        ...
        let role_selection = lane_complete_role_selection_for_node("run-lane-test", "tester");
        fs::write(
            &packet_path,
            canonical_rework_lane_packet(
                &role_selection,
                "run-lane-test",
                "tester",
                "tester",
                ...
                "Tester can request configured rework.",
                "verification",
                "verifier",
                "tester rework advances to coder",
                "cargo test -p vida lane_envelope_prefers_authorized_rework_handoff_over_exception_takeover",
                "tester rework envelope",
        ...
        let authorized_route = canonical_rework_route_for_test();
        let envelope = build_lane_envelope_with_owned_scope_and_rework_route(
            summary,
            None,
            None,
        ...
        assert_eq!(
            ...
            Some(
                "continue the authorized `coder` rework handoff from receipt-backed completion evidence"
            )
        );
    ...
    #[test]
    fn host_bridge_complete_reports_artifact_refs_on_projection_reject() {
        let mut receipt = sample_receipt("blocked");
        receipt.dispatch_surface = Some("vida lane complete --host-bridge-request".to_string());
        receipt.dispatch_result_path = Some("host-tool-bridge/results/run-host.json".to_string());
        receipt.downstream_dispatch_result_path =
            Some("runtime-consumption/dispatch-results/run-completion.json".to_string());
        receipt.downstream_dispatch_trace_path =
            Some("host-tool-bridge/receipts/run-host.json".to_string());
        receipt.downstream_dispatch_status = Some("blocked".to_string());
        ...
        assert_eq!(
            envelope.artifact_refs["host_bridge_result_path"],
            "host-tool-bridge/results/run-host.json"
        );
        assert_eq!(
            envelope.artifact_refs["host_bridge_receipt_path"],
            "host-tool-bridge/receipts/run-host.json"
        );
        assert_eq!(
            envelope.artifact_refs["downstream_dispatch_status"],
            "blocked"
        );
        assert_eq!(
            envelope.artifact_refs["downstream_dispatch_blockers"],
            serde_json::json!(["host_bridge_completion_result_blocked"])
        );
        assert_eq!(
            envelope.artifact_refs["downstream_dispatch_target"],
            "developer"
        );
        assert_eq!(envelope.artifact_refs["downstream_dispatch_ready"], false);
    }
    ...
    fn lane_blocked_open_cycle_without_scope_does_not_emit_placeholder_takeover_command() {
        ...
        assert_eq!(command, "vida task show task-lane-test");
        assert!(!command.contains("<owned-write-scope>"));
        assert_eq!(
            envelope.recommended_surface.as_deref(),
            Some("vida task show")
    ...
    #[test]
    fn lane_show_blocked_json_operator_contract() {
        let mut receipt = sample_receipt("bridge_request_pending");
        ...
        receipt.dispatch_packet_path =
            Some("runtime-consumption/dispatch-packets/lane-show-contract.json".to_string());
        receipt.dispatch_result_path =
            Some("runtime-consumption/dispatch-results/lane-show-contract.json".to_string());
        ...
        let envelope = build_lane_envelope(
            ...
            vec!["Inspect the blocked lane before local writes.".to_string()],
        );

        assert_eq!(envelope.status, "blocked");
        assert_eq!(
            envelope.blocker_codes,
            vec!["host_tool_bridge_adapter_required"]
        );
        assert_eq!(envelope.artifact_refs["root_local_write_allowed"], false);
        assert_eq!(
            envelope.artifact_refs["dispatch_packet_path"],
            "runtime-consumption/dispatch-packets/lane-show-contract.json"
        );
        assert_eq!(envelope.shared_fields["status"], envelope.status);
        assert_eq!(
            envelope.shared_fields["artifact_refs"],
            envelope.operator_contracts["artifact_refs"]
        );
        assert_eq!(
            envelope.operator_contracts["contract_id"],
            "release-1-operator-contracts"
        );
        assert_eq!(envelope.operator_contracts["status"], "blocked");
        assert_eq!(
            envelope.operator_contracts["blocker_codes"],
            serde_json::json!(["host_tool_bridge_adapter_required"])
        );
        assert_eq!(
            envelope.operator_contracts["next_actions"],
            serde_json::json!(envelope.next_actions.clone())
        );
    }
    ...
    #[tokio::test]
    async fn lane_show_json_ignores_stale_cache_for_terminal_closure() {
        let _guard = acquire_lane_surface_test_lock();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_nanos())
            .unwrap_or(0);
        let root = std::env::temp_dir().join(format!(
            "vida-lane-surface-terminal-stale-cache-{}-{}",
            std::process::id(),
            nanos
        ));
        let store = StateStore::open(root.clone()).await.expect("open store");
        let _state_override = ProxyStateDirOverrideGuard::install(root.clone());
        let run_id = "run-terminal-stale-cache";
        crate::operator_projection_cache::write_json_projection(
            store.root(),
            &lane_show_projection_name(run_id),
            &serde_json::json!({
                ...
                "recommended_command": "vida agent-init --downstream-packet stale.json --execute-dispatch",
                "recommended_surface": "vida agent-init",
                "next_action": {
                    "command": "vida agent-init --downstream-packet stale.json --execute-dispatch",
                    "surface": "vida agent-init",
                    "reason": "stale cached command"
                }
            }),
        );

        let mut status =
            crate::taskflow_run_graph::default_run_graph_status(run_id, run_id, "closure");
        status.status = "completed".to_string();
        status.active_node = "closure".to_string();
        status.next_node = None;
        status.lifecycle_stage = "closure_complete".to_string();
        ...
        receipt.blocker_code = None;
        receipt.downstream_dispatch_ready = true;
        receipt.downstream_dispatch_target = Some("prover".to_string());
        receipt.downstream_dispatch_status = Some("packet_ready".to_string());
        receipt.downstream_dispatch_packet_path =
            Some("runtime-consumption/downstream-dispatch-packets/stale-prover.json".to_string());
        receipt.downstream_dispatch_command = Some(
            "vida agent-init --downstream-packet stale-prover.json --execute-dispatch --json"
                .to_string(),
        );
        store
            .record_run_graph_dispatch_receipt(&receipt)
        ...
        let cached = read_cached_lane_show_projection(&root, &lane_show_projection_name(run_id))
            .expect("lane show projection cache should be rewritten");
        let cached_json: serde_json::Value =
            serde_json::from_str(&cached).expect("cached lane show projection should be json");
        assert_eq!(cached_json["status"], "pass");
        assert_eq!(cached_json["run_id"], run_id);
        assert_eq!(cached_json["lane_status"], "lane_completed");
        assert_eq!(cached_json["recommended_command"], serde_json::Value::Null);
        assert_eq!(cached_json["recommended_surface"], serde_json::Value::Null);
        assert_eq!(cached_json["next_action"], serde_json::Value::Null);
    ...
    #[tokio::test]
    async fn lane_retire_closed_task_stale_run_owner_guard_json() {
        let _guard = acquire_lane_surface_test_lock();
        let saved_session_id = std::env::var("VIDA_SESSION_ID").ok();
        unsafe {
            std::env::set_var("VIDA_SESSION_ID", "session-owner-lane-retire");
        }
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_nanos())
            .unwrap_or(0);
        let root = std::env::temp_dir().join(format!(
            "vida-lane-surface-retire-owner-guard-{}-{}",
            std::process::id(),
            nanos
        ));
        let store = StateStore::open(root.clone()).await.expect("open store");
        let _state_override = ProxyStateDirOverrideGuard::install(root.clone());
        let run_id = "run-lane-retire-owner-guard";
        let task_id = "task-lane-retire-owner-guard";

        store
            .create_task_with_fixture_parent(crate::state_store::CreateTaskRequest {
                task_id,
                title: "Retire closed task run with foreign owner",
                display_id: None,
                description: "",
                issue_type: "task",
                status: "closed",
        ...
        let packet_dir = root.join("runtime-consumption").join("dispatch-packets");
        std::fs::create_dir_all(&packet_dir).expect("create packet dir");
        let packet_path = packet_dir.join("run-lane-retire-owner-guard.json");
        std::fs::write(&packet_path, "{\"run_id\":\"run-lane-retire-owner-guard\"}")
            .expect("write dispatch packet");
        ...
        store
            .acquire_orchestrator_claim(crate::state_store::AcquireOrchestratorClaimRequest {
                claim_id: "foreign-lane-retire-owner-guard".to_string(),
                state_root_id: "foreign-state-root".to_string(),
                worktree_environment_id: "foreign-worktree".to_string(),
                orchestrator_session_id: "session-owner-lane-retire".to_string(),
                process_id: None,
                task_id: Some(task_id.to_string()),
                run_id: Some(run_id.to_string()),
                lane_id: None,
                claim_kind: "test_foreign_run_owner".to_string(),
            ...
            .await
            .expect("record foreign owner claim");
        unsafe {
            std::env::set_var("VIDA_SESSION_ID", "session-current-lane-retire");
        }
        assert!(
            !store
                .current_session_can_mutate_run_graph_run(run_id)
                .await
                .expect("owner check should run"),
            "fixture must reproduce the owner guard that record_run_graph_status would reject"
        );
        drop(store);
        wait_for_state_unlock(&root);
        ...
        let args = ProxyArgs {
            args: vec![
                ...
                run_id.to_string(),
                "--receipt-id".to_string(),
                "retire-owner-guard-1".to_string(),
                "--reason".to_string(),
                "closed stale run with foreign owner".to_string(),
                "--json".to_string(),
            ],
        };
        assert_eq!(run_lane(args).await, ExitCode::SUCCESS);
        ...
        assert_eq!(retired.status, "completed");
        assert_eq!(retired.lifecycle_stage, "closure_complete");
        assert_eq!(retired.policy_gate, "closed_task_stale_run_retired");
        assert_eq!(retired.resume_target, "none");
        assert!(!retired.recovery_ready);

        let blocked = lane_retire_blocked_envelope(
            run_id,
            "retire-owner-guard-1",
            "closed_task_active_run_projection_mismatch",
            "blocked by owner guard".to_string(),
            Some(
                "runtime-consumption/dispatch-packets/run-lane-retire-owner-guard.json".to_string(),
            ),
            Some(
                "runtime-consumption/dispatch-results/run-lane-retire-owner-guard.json".to_string(),
            ),
        );
        ...
        assert_eq!(blocked.artifact_refs["run_id"], run_id);
        assert_eq!(blocked.artifact_refs["receipt_id"], "retire-owner-guard-1");
        assert_eq!(
            blocked.artifact_refs["dispatch_packet_path"],
            "runtime-consumption/dispatch-packets/run-lane-retire-owner-guard.json"
        );
        assert_eq!(
            blocked.artifact_refs["dispatch_result_path"],
            "runtime-consumption/dispatch-results/run-lane-retire-owner-guard.json"
        );
        assert!(!blocked.next_actions.is_empty());
    ...
    async fn lane_retire_rejects_exception_takeover_missing_task_stale_blocked_run_without_closed_unit(
        ...
        let retained_receipt = store
            .run_graph_dispatch_receipt(run_id)
            .await
            .expect("read retained receipt")
            .expect("receipt should exist");
        assert_eq!(
            retained_receipt.dispatch_status,
            seeded_receipt.dispatch_status
        );
        assert_eq!(retained_receipt.lane_status, seeded_receipt.lane_status);
        assert_eq!(
            retained_receipt.dispatch_target,
            seeded_receipt.dispatch_target
        );
        ...
        assert_eq!(retained_binding.status, seeded_binding.status);
        assert_eq!(retained_binding.task_id, seeded_binding.task_id);
        assert_eq!(
            retained_binding.active_bounded_unit,
    ...
    async fn lane_complete_records_host_bridge_result_and_receipt_evidence() {
        ...
        std::fs::write(
            ...
            serde_json::json!({
                ...
                "completion_receipt_id": "reviewer-rework-retry-1",
                "decision": "rework",
                "evidence": ["implementation_conformance_review", "rework_or_approve_decision"],
    ...
    fn host_bridge_completion_rejects_implementation_no_code_without_patch_artifacts() {
        ...
        let nanos = SystemTime::now()
            ...
            .unwrap_or(0);
        let root = std::env::temp_dir().join(format!(
            "vida-lane-surface-host-bridge-no-code-{}-{}",
            std::process::id(),
            nanos
        ));
        let run_id = "run-host-bridge-no-code";
        let packet_path =
            root.join("runtime-consumption/downstream-dispatch-packets/run-no-code.json");
        let dispatch_result_path =
            root.join("runtime-consumption/dispatch-results/run-no-code-activation.json");
        let request_path = root.join("host-tool-bridge/requests/run-no-code.json");
        let result_path = root.join("host-tool-bridge/results/run-no-code.json");
        let bridge_receipt_path = root.join("host-tool-bridge/receipts/run-no-code.json");
        ...
        let evidence = materialize_host_bridge_completion_evidence(
            ...
            "receipt-no-code",
            Some("agent-1"),
            Some("implementation analysis did not require code changes"),
            None,
            None,
            Some("approve"),
            Some("pass"),
        ...
        .expect("implementation no-code marker should record blocked artifact evidence");
    ...
    #[tokio::test]
    async fn lane_complete_preserves_persisted_target_before_context_reconciliation() {
        let _guard = acquire_lane_surface_test_lock();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_nanos())
            .unwrap_or(0);
        let root = std::env::temp_dir().join(format!(
            "vida-lane-surface-host-bridge-context-target-{}-{}",
            std::process::id(),
            nanos
        ));
        let store = StateStore::open(root.clone()).await.expect("open store");
        let _state_override = ProxyStateDirOverrideGuard::install(root.clone());
        let run_id = "run-host-bridge-context-target";
        let persisted_target = "architect";
        let request_target = "designer";

        let packet_path =
            root.join("runtime-consumption/downstream-dispatch-packets/run-context-target.json");
        let activation_result_path =
            root.join("runtime-consumption/dispatch-results/run-context-target.json");
        let request_path = root.join("host-tool-bridge/requests/run-context-target.json");
        let result_path = root.join("host-tool-bridge/results/run-context-target.json");
        let bridge_receipt_path = root.join("host-tool-bridge/receipts/run-context-target.json");
        ...
        std::fs::write(
            ...
            serde_json::json!({
                ...
                "delivery_task_packet": {
                    "goal": "Complete host bridge lane evidence.",
                    "scope_in": ["dispatch_target:designer"],
                    "handoff_task_class": "implementation",
                    "handoff_runtime_role": "worker",
                    "owned_paths": ["crates/vida/src/lane_surface.rs"],
                    "read_only_paths": [".vida/data/state/runtime-consumption"],
                    "definition_of_done": ["host bridge completion is receipt-backed"],
                    "verification_command": "cargo test -p vida lane_complete_preserves_persisted_target_before_context_reconciliation",
                    "proof_target": "host bridge completion receipt",
                    "stop_rules": ["stop if packet contract is invalid"],
                    "blocking_question": "none"
                },
        ...
        status.task_id = run_id.to_string();
        status.active_node = request_target.to_string();
        status.next_node = Some(request_target.to_string());
        status.status = "blocked".to_string();
        status.policy_gate = "host_tool_bridge_adapter_required".to_string();
        store
            .record_run_graph_status(&status)
        ...
        receipt.downstream_dispatch_packet_path = Some(packet_path.display().to_string());
        receipt.downstream_dispatch_status = Some("blocked".to_string());
        receipt.downstream_dispatch_active_target = Some(request_target.to_string());
        receipt.selected_backend = Some("internal_subagents".to_string());
        store
            .record_run_graph_dispatch_receipt(&receipt)
            .await
        ...
        let exit = run_lane(ProxyArgs {
            args: vec![
                ...
                run_id.to_string(),
                "--receipt-id".to_string(),
                "host-bridge-context-target".to_string(),
                "--host-bridge-request".to_string(),
                request_path.display().to_string(),
                "--host-agent-id".to_string(),
                "agent-1".to_string(),
        ...
        let after = store
            ...
            .expect("read receipt after")
            .expect("receipt should exist");
        assert_eq!(after.dispatch_target, persisted_target);
    ...
    #[test]
    fn requestless_lane_completion_rejects_dispatch_packet_as_result_artifact() {
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_nanos())
            .unwrap_or(0);
        let root = std::env::temp_dir().join(format!(
            "vida-lane-requestless-result-binding-{}-{nanos}",
            std::process::id()
        ));
        let packet_path = root.join("runtime-consumption/dispatch-packets/stale.json");
        std::fs::create_dir_all(packet_path.parent().expect("packet parent"))
            .expect("create packet parent");
        std::fs::write(
            &packet_path,
        ...
        let error = read_supplied_host_bridge_completion_result(
            &root,
            Some(&packet_path_string),
            None,
            Some(("run-stale-result", "developer")),
        )
        .expect_err("dispatch packet must not be accepted as requestless completion result");

        assert!(
            error.contains("missing artifact_kind `host_tool_bridge_result`"),
            "unexpected error: {error}"
        );
        let _ = std::fs::remove_dir_all(root);
    }
    ...
    #[test]
    fn host_bridge_completion_result_error_envelope_is_actionable_json_contract() {
        let envelope = host_bridge_completion_error_envelope(
            "run-invalid-result",
            Some("host-tool-bridge/requests/request.json"),
            Some("host-tool-bridge/staged-results/invalid.json"),
            "host bridge supplied result is missing run_id required by request binding",
        );
        ...
        assert!(envelope.next_actions.iter().any(|action| action.contains(
            "retry `vida agent host-bridge --submit-result` or `vida lane complete --host-bridge-result-file`"
        )));
        assert_eq!(
            envelope.artifact_refs["host_bridge_request"],
            "host-tool-bridge/requests/request.json"
        );
        assert_eq!(
    ...
    #[tokio::test]
    async fn lane_complete_host_bridge_result_file_invalid_result_fails_before_materialization() {
        let _guard = acquire_lane_surface_test_lock();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_nanos())
        ...
        let root = std::env::current_dir()
            ...
            .join("target/tmp")
            .join(format!(
                "vida-lane-surface-host-bridge-invalid-result-{}-{}",
                std::process::id(),
                nanos
            ));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).expect("create host bridge invalid result state root");
        let store = StateStore::open(root.clone()).await.expect("open store");
        let _state_override = ProxyStateDirOverrideGuard::install(root.clone());
        let run_id = "run-host-bridge-invalid-result";
        let packet_path = root.join(
            "runtime-consumption/downstream-dispatch-packets/run-host-bridge-invalid-result.json",
        );
        let request_path =
            root.join("host-tool-bridge/requests/run-host-bridge-invalid-result.json");
        let result_path = root.join("host-tool-bridge/results/run-host-bridge-invalid-result.json");
        let bridge_receipt_path =
            root.join("host-tool-bridge/receipts/run-host-bridge-invalid-result.json");
        let staged_result_path =
            root.join("host-tool-bridge/staged-results/run-host-bridge-invalid-result.json");
        for path in [
            &packet_path,
            &request_path,
            &result_path,
        ...
        std::fs::write(
            ...
            serde_json::json!({
                ...
                "packet_template_kind": "delivery_task_packet",
                "delivery_task_packet": {
                    "goal": "Validate invalid host bridge result handling.",
                    "scope_in": ["dispatch_target:verification"],
                    "handoff_task_class": "implementation",
                    "handoff_runtime_role": "verifier",
                    "owned_paths": ["crates/vida/src/lane_surface.rs"],
                    "read_only_paths": ["crates/vida/src"],
                    "definition_of_done": ["invalid result fails closed"],
                    "verification_command": "vida lane complete",
                    "proof_target": "host bridge result validation",
                    "stop_rules": ["stop after validation"],
                    "blocking_question": "none"
                },
        ...
        std::fs::write(
            ...
            serde_json::json!({
                "schema_version": 1,
                "status": "pending",
                "request_id": "run-host-bridge-invalid-result",
                "run_id": run_id,
                "task_id": run_id,
                "dispatch_target": "verification",
                "packet_path": packet_path.display().to_string(),
        ...
        .expect("write invalid staged result");
        let activation_result_path =
            root.join("runtime-consumption/dispatch-results/run-host-bridge-invalid-result.json");
        std::fs::create_dir_all(activation_result_path.parent().expect("activation parent"))
            .expect("create activation parent");
        std::fs::write(
            &activation_result_path,
        ...
        let exit = run_lane(ProxyArgs {
            args: vec![
                ...
                run_id.to_string(),
                "--receipt-id".to_string(),
                "host-bridge-invalid-result".to_string(),
                "--host-bridge-request".to_string(),
                request_path.display().to_string(),
                "--host-bridge-result-file".to_string(),
                staged_result_path.display().to_string(),
                "--host-agent-id".to_string(),
    ...
    fn read_lane_packet_reads_contained_packet_and_rejects_traversal_symlink_and_oversized_file() {
        ...
        let compact_packet_path = root.join("runtime-consumption/packets/compact.json");
        std::fs::create_dir_all(compact_packet_path.parent().expect("compact packet parent"))
            .expect("create compact packet parent");
        write_lane_packet(
            compact_packet_path
                .to_str()
                .expect("compact packet path utf-8"),
            &serde_json::json!({
                "run_id": "run-compact",
                "role_selection_full": {
                    "compiled_bundle": {"large_projection": "omitted-at-transport"},
        ...
        .expect("compact lane packet should write");
        let compact_packet = read_lane_packet(
            &root,
            compact_packet_path
                .to_str()
                .expect("compact packet path utf-8"),
        ...
        assert!(
            serde_json::to_vec(&compact_packet)
                .expect("compact lane packet should serialize")
                .len()
                < MAX_LANE_PACKET_READ_BYTES as usize
        );
    ...
    #[tokio::test]
    async fn host_bridge_closed_task_request_artifacts_supply_receipt_backed_authority() {
        let _guard = acquire_lane_surface_test_lock();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_nanos())
            .unwrap_or(0);
        let root = std::env::temp_dir().join(format!(
            "vida-lane-surface-host-bridge-closed-artifact-authority-{}-{}",
            std::process::id(),
            nanos
        ));
        let store = StateStore::open(root.clone()).await.expect("open store");
        let run_id = "run-host-bridge-closed-artifact-authority";
        let task = store
            .create_task_with_fixture_parent(crate::state_store::CreateTaskRequest {
                task_id: run_id,
                title: "Host bridge closed artifact authority",
                display_id: None,
                description: "",
                issue_type: "task",
                status: "closed",
        ...
        let normalized_artifact_ref = "host-tool-bridge/implementation-artifacts/closed-authority-attempt-1-0-patch_proposal.json";
        ...
        assert!(evidence.authority.is_some());
        assert!(evidence.blocker_codes.is_empty());
        assert!(evidence.taskflow_artifacts.artifacts.is_empty());
        assert_eq!(evidence.taskflow_artifacts.authority_keys.len(), 1);
        let artifacts = host_bridge_implementation_artifacts(
            &request,
            evidence.taskflow_artifacts,
            "closed-authority-receipt-1",
            evidence.authority.as_ref(),
            root.as_path(),
        );
        assert_eq!(artifacts.source, "host_bridge_request");
        assert!(artifacts.blocker_codes.is_empty());
        let authority = evidence.authority.as_ref().expect("authority");
        let validation = host_bridge_implementation_scope_validation(
            &request,
            &artifacts.artifacts,
            crate::runtime_dispatch_packets::ImplementationArtifactAuthority {
    ...
    #[tokio::test]
    async fn lane_complete_uses_authoritative_owned_paths_after_request_scope_stales() {
        let _guard = acquire_lane_surface_test_lock();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_nanos())
            .unwrap_or(0);
        let root = std::env::temp_dir().join(format!(
            "vida-lane-surface-host-bridge-stale-owned-paths-{}-{}",
            std::process::id(),
            nanos
        ));
        let store = StateStore::open(root.clone()).await.expect("open store");
        let run_id = "run-host-bridge-stale-owned-paths";
        let task = store
            .create_task_with_fixture_parent(crate::state_store::CreateTaskRequest {
                task_id: run_id,
                title: "Host bridge stale owned paths",
                display_id: None,
                description: "",
                issue_type: "task",
                status: "open",
        ...
        assert_eq!(
            authoritative_owned_paths,
            vec!["crates/taskflow-host-bridge/src".to_string()]
        );

        let request_path = root.join("host-tool-bridge/requests/stale-owned-paths.json");
        let result_path = root.join("host-tool-bridge/results/stale-owned-paths.json");
        let receipt_path = root.join("host-tool-bridge/receipts/stale-owned-paths.json");
        let activation_result_path =
            root.join("runtime-consumption/dispatch-results/stale-owned-paths-activation.json");
        for path in [
            &request_path,
            &result_path,
            &receipt_path,
        ...
        let implementation_artifact = serde_json::json!({
            ...
            "stage_id": "implementation",
            "freshness": task.updated_at,
            "consolidation_receipt_id": "stale-owned-paths-receipt-1",
            "changed_files": ["crates/taskflow-host-bridge/src/completion.rs"],
            "source_artifact_ref": "host-tool-bridge/artifacts/stale-owned-paths.json",
            "source_artifact_kind": "patch_proposal",
            "receipt_backed": true
        });
        let adapter_operations = serde_json::json!({
        ...
        let evidence = materialize_host_bridge_completion_evidence(
            ...
            "developer",
            &receipt,
            "stale-owned-paths-receipt-1",
            Some("developer-1"),
            Some("developer completed stale owned_paths request"),
            Some("coach"),
            None,
            Some("approve"),
            Some("pass"),
            ...
            HostBridgeTaskflowImplementationEvidence {
                ...
                taskflow_artifacts:
                    crate::runtime_dispatch_packets::TaskflowImplementationArtifacts {
                        authority_keys: vec![
                        crate::runtime_dispatch_packets::TaskflowImplementationArtifactAuthority {
                            attempt_id: "stale-owned-paths-attempt-1".to_string(),
                            task_id: run_id.to_string(),
                            stage_id: "implementation".to_string(),
                            freshness: task.updated_at.clone(),
                            consolidation_receipt_id: "stale-owned-paths-receipt-1".to_string(),
                        },
                    ],
                        ..Default::default()
                    },
        ...
        .expect("current TaskFlow owned_paths should authorize stale request completion");

        assert_eq!(evidence.execution_state, "executed");
    ...
    fn pending_host_bridge_completion_rejects_oversized_persisted_dispatch_result() {
        ...
        let result_path = root.join("host-tool-bridge/results/pending-original.json");
        let receipt_path = root.join("host-tool-bridge/receipts/pending-original.json");
        std::fs::create_dir_all(packet_path.parent().expect("packet parent"))
            .expect("create packet parent");
        std::fs::create_dir_all(
    ...
    #[tokio::test]
    async fn lane_complete_host_bridge_accepts_attempt_scope_with_proof_artifacts() {
        let _guard = acquire_lane_surface_test_lock();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_nanos())
            .unwrap_or(0);
        let root = std::env::temp_dir().join(format!(
            "vida-lane-surface-host-bridge-proof-artifact-scope-{}-{}",
            std::process::id(),
            nanos
        ));
        let store = StateStore::open(root.clone()).await.expect("open store");
        let _state_override = ProxyStateDirOverrideGuard::install(root.clone());
        let run_id = "run-host-bridge-proof-artifact-scope";
        let task = store
            .create_task_with_fixture_parent(crate::state_store::CreateTaskRequest {
                task_id: run_id,
                title: "Host bridge proof artifact scope",
                display_id: None,
                description: "",
                issue_type: "task",
                status: "open",
                ...
                execution_semantics: crate::state_store::TaskExecutionSemantics::default(),
                planner_metadata: crate::state_store::TaskPlannerMetadata {
                    owned_paths: vec!["src/lib/features/list_view/data".to_string()],
                    ..Default::default()
                },
                created_by: "test",
                source_repo: "",
        ...
        let changed_files = serde_json::json!([
            "src/lib/features/list_view/data/record_chatter_repository.dart",
        ...
        let proof_artifact_paths = serde_json::json!([
            "src/test/features/list_view/domain/models/record_chatter_models_test.dart",
            "src/test/features/list_view/data/record_chatter_repository_test.dart",
            "src/test/features/list_view/presentation/stac/widgets/record_detail_view_test.dart"
        ]);
        let artifact_path = root.join("attempt-artifacts/proof-artifact-scope.json");
        std::fs::create_dir_all(artifact_path.parent().expect("artifact parent"))
            .expect("create artifact parent");
        std::fs::write(
            &artifact_path,
        ...
        .expect("write proof-scoped artifact");
        store
            .record_task_attempt(crate::state_store::RecordTaskAttemptRequest {
                attempt_id: Some("attempt-proof-artifact-scope".to_string()),
                task_id: run_id.to_string(),
                stage_id: "implementation".to_string(),
                backend: "internal_subagents".to_string(),
                model_profile: "middle".to_string(),
                ...
                artifact_refs: vec![artifact_path.display().to_string()],
                consolidation_receipt_id: Some(
                    "attempt-proof-artifact-scope-consolidation-receipt".to_string(),
                ),
                selected_model_profile_readiness_status: None,
                budget_posture: None,
                cap_posture: None,
        ...
        let packet_path =
            root.join("runtime-consumption/downstream-dispatch-packets/run-proof-scope.json");
        std::fs::create_dir_all(packet_path.parent().expect("packet parent"))
            .expect("create packet parent");
        std::fs::write(
            &packet_path,
            serde_json::json!({
                ...
                "delivery_task_packet": {
                    ...
                    "owned_paths": ["src/lib/features/list_view/data"],
                    "proof_artifact_paths": proof_artifact_paths,
                    "definition_of_done": ["proof tests are attached"],
                    "verification_command": "flutter test src/test/features/list_view",
                    "proof_target": "host bridge completion receipt",
                    "stop_rules": ["stop if bridge evidence is missing"],
                    "blocking_question": "none"
                },
        ...
        let request_path = root.join("host-tool-bridge/requests/run-proof-scope.json");
        let result_path = root.join("host-tool-bridge/results/run-proof-scope.json");
        let bridge_receipt_path = root.join("host-tool-bridge/receipts/run-proof-scope.json");
        std::fs::create_dir_all(request_path.parent().expect("request parent"))
            .expect("create request parent");
        std::fs::write(
            &request_path,
            current_host_bridge_request_fixture(serde_json::json!({
                ...
                "implementation_artifacts": [{
                    "artifact_kind": "patch_proposal",
                    "attempt_id": "attempt-proof-artifact-scope",
                    "task_id": run_id,
                    "stage_id": "implementation",
                    "freshness": task.updated_at,
                    "receipt_backed": true,
                    "consolidation_receipt_id": "attempt-proof-artifact-scope-consolidation-receipt",
                    "changed_files": changed_files
                }],
                "result_path": result_path.display().to_string(),
                "receipt_path": bridge_receipt_path.display().to_string()
        ...
        .expect("write request");
        let activation_result_path =
            root.join("runtime-consumption/dispatch-results/run-proof-scope-activation.json");
        std::fs::create_dir_all(activation_result_path.parent().expect("activation parent"))
            .expect("create activation parent");
        std::fs::write(
            &activation_result_path,
        ...
        let args = ProxyArgs {
            args: vec![
                ...
                run_id.to_string(),
                "--receipt-id".to_string(),
                "host-bridge-proof-artifact-scope".to_string(),
                "--host-bridge-request".to_string(),
                request_path.display().to_string(),
                "--host-agent-id".to_string(),
                "agent-1".to_string(),
        ...
        assert_eq!(bridge_result["scope_validation"]["status"], "pass");
        assert_eq!(
            bridge_result["scope_validation"]["proof_artifact_paths"],
            proof_artifact_paths
        );
    ...
    fn host_bridge_completion_accepts_configured_state_root_subdirectories() {
        ...
        let packet_path =
            root.join("runtime-consumption/downstream-dispatch-packets/run-custom.json");
        std::fs::create_dir_all(request_path.parent().expect("request parent"))
            .expect("create request parent");
        let request = current_host_bridge_request_fixture(
            serde_json::json!({
    ...
    fn host_bridge_completion_rejects_retryable_contract_for_completed_request() {
        ...
        normalize_current_host_bridge_request_file(
            ...
            "implementer",
            &root.join(
                "runtime-consumption/downstream-dispatch-packets/run-completed-contract.json",
            ),
            &result_path,
            &receipt_path,
        );
    ...
    fn host_bridge_completion_retry_accepts_routable_rework_with_blocker_codes() {
        ...
        let role_selection = lane_complete_role_selection("run-rework-blockers");
        ...
        std::fs::write(
            ...
            serde_json::json!({
                ...
                "role_selection_full": serde_json::to_value(&role_selection)
                    .expect("serialize canonical role selection"),
                "run_graph_bootstrap": {"run_id": "run-rework-blockers"}
            })
            .to_string(),
        )
        .expect("write packet");
        ...
        std::fs::write(
            ...
            serde_json::json!({
                ...
                "run_id": "run-rework-blockers",
                "dispatch_target": "coder",
                "completion_receipt_id": "receipt-rework-blockers",
                "decision": "rework",
                "evidence": ["changed_files", "verification_notes", "explicit_blockers"],
                "execution_state": "bridge_request_pending",
                "host_tool_bridge_request": {
                    "request_path": request_path.display().to_string(),
                    "packet_path": packet_path.display().to_string(),
    ...
    fn host_bridge_completion_retry_replaces_stale_blocked_receipt_for_result_with_next_node() {
        ...
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let root = std::env::temp_dir().join(format!(
            "vida-host-bridge-retry-blocked-next-receipt-{}-{nanos}",
            std::process::id()
        ));
        ...
        std::fs::write(
            ...
            serde_json::json!({
                "artifact_kind": "host_tool_bridge_receipt",
                "status": "blocked",
                "completion_receipt_id": "stale-blocked-receipt"
            })
            .to_string(),
        )
        .expect("write stale receipt");
        ...
        .expect("retry completion should replace receipt for blocked result with next node");
    ...
    #[test]
    fn host_bridge_completion_rejects_existing_canonical_result_without_overwrite() {
        let _guard = acquire_lane_surface_test_lock();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("system time should be after epoch")
            .as_nanos();
        let root = std::env::temp_dir().join(format!(
            "vida-host-bridge-existing-result-{}-{nanos}",
            std::process::id()
        ));
        let request_path = root.join("host-tool-bridge/requests/run-existing.json");
        let result_path = root.join("host-tool-bridge/results/run-existing.json");
        let receipt_path = root.join("host-tool-bridge/receipts/run-existing.json");
        std::fs::create_dir_all(request_path.parent().expect("request parent"))
            .expect("create request parent");
        std::fs::create_dir_all(result_path.parent().expect("result parent"))
            .expect("create result parent");
        std::fs::write(
            ...
            serde_json::json!({
                ...
                "run_id": "run-existing",
                "dispatch_target": "implementer",
                "packet_path": root.join("runtime-consumption/downstream-dispatch-packets/run-existing.json").display().to_string(),
                "backend_id": "internal_subagents",
                "dispatch_transport": "host_tool_bridge",
                "result_path": result_path.display().to_string(),
                "receipt_path": receipt_path.display().to_string()
        ...
        .expect("write request");
        std::fs::write(&result_path, "{}").expect("seed canonical result");
        let activation_result_path =
            root.join("runtime-consumption/dispatch-results/run-existing-activation.json");
        std::fs::create_dir_all(activation_result_path.parent().expect("activation parent"))
            .expect("create activation parent");
        std::fs::write(
            &activation_result_path,
        ...
        .expect("write activation result");
        let mut receipt = sample_receipt("bridge_request_pending");
        receipt.run_id = "run-existing".to_string();
        receipt.dispatch_target = "implementer".to_string();
        receipt.dispatch_result_path = Some(activation_result_path.display().to_string());
        ...
        .expect_err("existing canonical result must fail closed");
    ...
    fn host_bridge_completion_rejects_non_retry_submitted_result_when_canonical_evidence_exists() {
        ...
        std::fs::create_dir_all(
            supplied_result_path
                .parent()
                .expect("submitted result parent"),
        )
    ...
    fn host_bridge_completion_accepts_quality_gate_rework_over_stale_persisted_route() {
        ...
        std::fs::create_dir_all(packet_path.parent().expect("packet parent"))
            .expect("create packet parent");
        let role_selection = lane_complete_role_selection_for_node("run-rework", "tester");
        std::fs::write(
            &packet_path,
            canonical_rework_lane_packet(
                &role_selection,
                ...
                "verifier",
                "tester rework advances to coder",
                "cargo test -p vida host_bridge_completion_accepts_quality_gate_rework_over_stale_persisted_route",
                "tester rework materializes coder packet",
                "verification_rework_required",
            )
            .to_string(),
        )
        ...
        std::fs::write(
            ...
            serde_json::json!({
                ...
                "blocked_result_contract": {
                    ...
                    "rework_target_required_when_blocked": true,
                    "allowed_next_node": "coder",
                    "allowed_blocker_codes": ["verification_rework_required"]
                }
            })
            .to_string(),
        )
        ...
        std::fs::write(
            ...
            serde_json::json!({
                ...
                "run_id": "run-rework",
                "dispatch_target": "tester",
                "completion_receipt_id": "receipt-rework",
                "decision": "rework",
                "evidence": ["verification_evidence", "failing_or_passing_checks"],
                "execution_state": "bridge_request_pending",
                "host_tool_bridge_request": {
                    "request_path": request_path.display().to_string(),
                    "result_path": result_path.display().to_string(),
        ...
        let supplied_blockers = vec!["verification_rework_required".to_string()];
        let evidence = materialize_host_bridge_completion_evidence(
            &root,
            request_path.to_str().expect("utf8 request path"),
            None,
        ...
        assert_eq!(evidence.execution_state, "blocked");
        assert_eq!(evidence.allowed_next_node, Some("coder".to_string()));
        let result: serde_json::Value = serde_json::from_str(
            &std::fs::read_to_string(&result_path).expect("result should be readable"),
        )
        .expect("result should decode");
    ...
    async fn lane_complete_allowed_next_node_materializes_downstream_packet() {
        ...
        status.policy_gate = "single_task_scope_required".to_string();
        status.handoff_state = "awaiting_implementation".to_string();
        status.context_state = "sealed".to_string();
        status.checkpoint_kind = "execution_cursor".to_string();
        status.resume_target = "dispatch.coder_lane".to_string();
        status.recovery_ready = true;
        store
            .record_run_graph_status(&status)
            .await
        ...
        let role_selection = lane_complete_role_selection(run_id);
        let packet_path = root.join(
        ...
        std::fs::write(
            ...
            "activation_runtime_role": "worker",
                ...
                "read_only_paths": [".vida/data/state/runtime-consumption"],
                "delivery_task_packet": {
                    "goal": "Complete the coder lane evidence.",
                    "scope_in": ["dispatch_target:coder"],
                    "handoff_task_class": "implementation",
                    "handoff_runtime_role": "worker",
                    "owned_paths": ["crates/vida/src/lane_surface.rs"],
                    "read_only_paths": [".vida/data/state/runtime-consumption"],
    ...
    #[tokio::test]
    async fn lane_complete_designer_pass_accepts_autotester_over_stale_closure_route() {
        let _guard = acquire_lane_surface_test_lock();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_nanos())
            .unwrap_or(0);
        let root = std::env::temp_dir().join(format!(
            "vida-lane-surface-designer-stale-closure-{}-{}",
            std::process::id(),
            nanos
        ));
        let store = StateStore::open(root.clone()).await.expect("open store");
        ...
        store
            .create_task_with_fixture_parent(crate::state_store::CreateTaskRequest {
                task_id: run_id,
                title: "Designer pass keeps autotester route over stale closure",
                display_id: None,
                description: "",
                issue_type: "defect",
                status: "in_progress",
        ...
        status.policy_gate = "single_task_scope_required".to_string();
        status.handoff_state = "awaiting_design".to_string();
        status.context_state = "sealed".to_string();
        status.checkpoint_kind = "execution_cursor".to_string();
        ...
        role_selection.execution_plan["development_flow"]["dispatch_contract"]["lane_catalog"] = serde_json::json!({
            ...
            "autotester": {
                ...
                "packet_template_kind": "verifier_proof_packet",
                "activation_agent_type": "middle",
                "activation_runtime_role": "autotester",
                "runtime_assignment": {
                    "selected_backend_id": "internal_subagents",
                    "selected_carrier_id": "middle"
                }
        ...
        let packet_path =
            root.join("runtime-consumption/dispatch-packets/activity-meeting-designer.json");
        std::fs::create_dir_all(
            packet_path
                .parent()
                .expect("dispatch packet path should have parent"),
        ...
        std::fs::write(
            ...
            serde_json::json!({
                ...
                "read_only_paths": [".vida/data/state/runtime-consumption"],
                "delivery_task_packet": {
                    "goal": "Complete the designer lane evidence.",
                    "scope_in": ["dispatch_target:designer"],
                    "handoff_task_class": "design",
                    "handoff_runtime_role": "designer",
                    "owned_paths": ["crates/vida/src/lane_surface.rs"],
                    "read_only_paths": [".vida/data/state/runtime-consumption"],
                    "definition_of_done": ["designer pass advances to autotester over stale closure"],
                    "verification_command": "cargo test -p vida lane_complete_designer_pass_accepts_autotester_over_stale_closure_route",
                    "proof_target": "designer pass materializes autotester packet",
                    "stop_rules": ["stop if packet contract is invalid"],
                    "blocking_question": "none"
                },
                "role_selection_full": role_selection,
                ...
                "downstream_dispatch_active_target": "designer",
                "downstream_dispatch_ready": false,
                "downstream_dispatch_blockers": ["pending_design_evidence"],
                "downstream_dispatch_status": "blocked",
                "downstream_lane_status": "lane_blocked"
            })
            .to_string(),
        ...
        receipt.dispatch_command = Some("vida agent-init".to_string());
        receipt.dispatch_packet_path = Some(packet_path.display().to_string());
        receipt.blocker_code = Some("pending_design_evidence".to_string());
        receipt.downstream_dispatch_target = Some("closure".to_string());
        receipt.downstream_dispatch_command = None;
        receipt.downstream_dispatch_note = None;
        receipt.downstream_dispatch_ready = false;
        receipt.downstream_dispatch_blockers = vec!["pending_design_evidence".to_string()];
        receipt.downstream_dispatch_packet_path = None;
        receipt.downstream_dispatch_status = Some("blocked".to_string());
        receipt.downstream_dispatch_active_target = Some("designer".to_string());
        store
            .record_run_graph_dispatch_receipt(&receipt)
            .await
            .expect("persist dispatch receipt");
        ...
        let args = ProxyArgs {
            args: vec![
                ...
                run_id.to_string(),
                "--receipt-id".to_string(),
                "completion-designer-pass-1".to_string(),
                "--host-bridge-summary".to_string(),
                "summary mentions blocker and rework, but typed completion result is pass"
                    .to_string(),
                "--decision".to_string(),
                "pass".to_string(),
                "--verdict".to_string(),
        ...
        assert!(after.downstream_dispatch_ready);
        assert!(after.downstream_dispatch_blockers.is_empty());
        let dispatch_result: serde_json::Value = serde_json::from_str(
            &std::fs::read_to_string(after.dispatch_result_path.as_deref().expect("result path"))
                .expect("read dispatch result"),
        )
        .expect("dispatch result should parse");
        assert_eq!(dispatch_result["status"], "pass");
        assert_eq!(dispatch_result["decision"], "approve");
        assert_eq!(dispatch_result["verdict"], "pass");
        assert_eq!(dispatch_result["allowed_next_node"], "autotester");
        assert_eq!(dispatch_result["blocker_codes"], serde_json::json!([]));
        assert_eq!(
            dispatch_result["summary_classifier_source"],
            "typed_blockers_only"
        );
        let downstream_packet_path = after
            .downstream_dispatch_packet_path
        ...
        .expect("downstream packet should parse");
        assert_eq!(downstream_packet["dispatch_target"], "autotester");
        assert_eq!(
            downstream_packet["downstream_dispatch_target"],
            "autotester"
        );
        assert_eq!(downstream_packet["source_dispatch_target"], "designer");
    ...
    async fn focused_host_bridge_run_graph_test_autotester_pass_reconciles_stale_designer_projection(
        ...
        let nanos = SystemTime::now()
            ...
            .unwrap_or(0);
        let root =
            std::env::temp_dir().join(format!("vida-lane-auto-{}-{}", std::process::id(), nanos));
        let _ = std::fs::remove_dir_all(&root);
        let store = StateStore::open(root.clone()).await.expect("open store");
        let _state_override = ProxyStateDirOverrideGuard::install(root.clone());
        let run_id = "activity-meeting-event-form-fields";
        store
            .create_task_with_fixture_parent(crate::state_store::CreateTaskRequest {
                task_id: run_id,
                title: "Autotester pass reconciles stale designer projection",
                display_id: None,
                description: "",
                issue_type: "defect",
                status: "in_progress",
        ...
        std::fs::write(
            ...
            serde_json::json!({
                ...
                "verifier_proof_packet": {
                    ...
                    "verification_command": "cargo test -p vida focused_host_bridge_run_graph_test_autotester_pass_reconciles_stale_designer_projection",
                    "proof_target": "autotester pass materializes developer packet",
                    "stop_rules": ["stop if packet contract is invalid"],
                    "blocking_question": "none"
                },
                "role_selection_full": role_selection,
        ...
        let result_path = root.join("host-tool-bridge/results/activity-autotester-result.json");
        let bridge_receipt_path =
            root.join("host-tool-bridge/receipts/activity-autotester-receipt.json");
        std::fs::create_dir_all(
            request_path
                .parent()
                .expect("request path should have parent"),
        ...
        std::fs::write(
            ...
            serde_json::json!({
                "schema_version": 1,
                "status": "pending",
                "request_id": "activity-autotester-request",
                "run_id": run_id,
                "task_id": run_id,
                "dispatch_target": "autotester",
                "task_class": "verification",
        ...
        receipt.downstream_dispatch_command = Some("vida agent-init".to_string());
        receipt.downstream_dispatch_note =
            Some("after autotester evidence is recorded, activate developer".to_string());
        receipt.downstream_dispatch_ready = false;
        receipt.downstream_dispatch_blockers = vec!["pending_autotest_evidence".to_string()];
        receipt.downstream_dispatch_packet_path = Some(packet_path.display().to_string());
        receipt.downstream_dispatch_status = Some("blocked".to_string());
        ...
        let args = ProxyArgs {
            args: vec![
                ...
                run_id.to_string(),
                "--receipt-id".to_string(),
                "completion-autotester-pass-1".to_string(),
                "--host-bridge-request".to_string(),
                request_path.display().to_string(),
                "--host-agent-id".to_string(),
        ...
        let binding = store
            ...
            .expect("continuation binding should exist");
        assert_eq!(binding.binding_source, "lane_complete");
        assert_eq!(binding.active_bounded_unit["active_node"], "autotester");
    ...
    #[tokio::test]
    async fn host_bridge_retry_completion_rework_rejects_stale_request_dispatch_target() {
        let _guard = acquire_lane_surface_test_lock();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_nanos())
            .unwrap_or(0);
        let root = std::env::temp_dir().join(format!(
            "vida-lane-surface-retry-rework-stale-request-{}-{}",
            std::process::id(),
            nanos
        ));
        let store = StateStore::open(root.clone()).await.expect("open store");
        write_lane_activation_snapshot_for_store(&store)
            ...
            .expect("write lane completion activation snapshot");
        let _state_override = ProxyStateDirOverrideGuard::install(root.clone());
        let run_id = "run-review-rework-stale-request";
        store
            .create_task_with_fixture_parent(crate::state_store::CreateTaskRequest {
                task_id: run_id,
                title: "Tester rework retries stale host bridge request",
                display_id: None,
                description: "",
                issue_type: "defect",
                status: "in_progress",
        ...
        status.policy_gate = "host_tool_bridge_adapter_required".to_string();
        status.handoff_state = "awaiting_verification".to_string();
        status.context_state = "sealed".to_string();
        status.checkpoint_kind = "execution_cursor".to_string();
        ...
        let role_selection = lane_complete_role_selection_for_node(run_id, "tester");
        let packet_path =
            root.join("runtime-consumption/downstream-dispatch-packets/tester-stale.json");
        std::fs::create_dir_all(packet_path.parent().expect("dispatch packet parent"))
            .expect("create dispatch packet dir");
        std::fs::write(
            &packet_path,
            canonical_rework_lane_packet(
                ...
                "verifier",
                "tester rework advances to coder",
                "cargo test -p vida host_bridge_retry_completion_rework_rejects_stale_request_dispatch_target",
                "tester rework materializes coder packet",
        ...
        let request_path = root.join("host-tool-bridge/requests/tester-stale-request.json");
        let result_path = root.join("host-tool-bridge/results/tester-stale-result.json");
        let bridge_receipt_path = root.join("host-tool-bridge/receipts/tester-stale-receipt.json");
        std::fs::create_dir_all(request_path.parent().expect("request parent"))
            .expect("create request dir");
        std::fs::write(
            &request_path,
            serde_json::json!({
                "schema_version": 1,
                "status": "blocked",
                "request_id": "tester-stale-request",
                "run_id": run_id,
                "task_id": run_id,
                "dispatch_target": "tester",
                "task_class": "verification",
        ...
        let activation_result_path =
            root.join("runtime-consumption/dispatch-results/tester-stale-activation.json");
        std::fs::create_dir_all(activation_result_path.parent().expect("activation parent"))
            .expect("create activation result dir");
        std::fs::write(
            &activation_result_path,
            serde_json::json!({
                ...
                "run_id": run_id,
                "dispatch_target": "tester",
                "completion_receipt_id": "tester-rework-retry-1",
                "decision": "rework_required",
                "verdict": "rework_required",
                "completion_verdict": "rework_required",
                "rework_target": "coder",
                ...
                "execution_evidence": {
                    "receipt_backed": true,
                    "receipt_id": "tester-rework-retry-1"
                },
                "execution_state": "blocked",
                "host_tool_bridge_request": {
                    "request_path": request_path.display().to_string(),
        ...
        receipt.downstream_dispatch_command = Some("vida agent-init".to_string());
        receipt.downstream_dispatch_note =
            Some("after tester evidence is recorded, activate coder".to_string());
        receipt.downstream_dispatch_ready = false;
        receipt.downstream_dispatch_blockers = vec!["pending_verification_evidence".to_string()];
        receipt.downstream_dispatch_packet_path = Some(packet_path.display().to_string());
        receipt.downstream_dispatch_status = Some("blocked".to_string());
        receipt.downstream_dispatch_active_target = Some("coder".to_string());
        receipt.activation_agent_type = Some("senior".to_string());
        receipt.activation_runtime_role = Some("worker".to_string());
        receipt.selected_backend = Some("internal_subagents".to_string());
        store
            .record_run_graph_dispatch_receipt(&receipt)
        ...
        store
            .run_graph_status(run_id)
            .await
            .expect("persisted run graph status should rehydrate before lane mutation");
        drop(store);
        wait_for_state_unlock(&root);
        ...
        let args = ProxyArgs {
            args: vec![
                ...
                run_id.to_string(),
                "--receipt-id".to_string(),
                "tester-rework-retry-1".to_string(),
                "--host-bridge-request".to_string(),
                request_path.display().to_string(),
                "--host-agent-id".to_string(),
                "agent-tester-1".to_string(),
                "--host-bridge-summary".to_string(),
                "tester requested configured rework".to_string(),
                "--decision".to_string(),
                ...
                "--retry-host-bridge-completion".to_string(),
                "--state-dir".to_string(),
                root.display().to_string(),
                "--json".to_string(),
            ],
        ...
        let after = store
            ...
            .expect("read receipt after")
            .expect("receipt should exist");
        assert_ne!(after.dispatch_status, "executed");
        assert_eq!(after.downstream_dispatch_target.as_deref(), Some("tester"));
        assert_eq!(
            after.downstream_dispatch_active_target.as_deref(),
            Some("coder")
        );
        ...
        assert_eq!(
            after.downstream_dispatch_blockers,
            vec!["pending_verification_evidence".to_string()]
        );
    ...
    #[tokio::test]
    async fn host_bridge_completion_rework_requires_typed_receipt_evidence() {
        let _guard = acquire_lane_surface_test_lock();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_nanos())
            .unwrap_or(0);
        let root = std::env::temp_dir().join(format!(
            "vida-lane-surface-rework-alias-complete-{}-{}",
            std::process::id(),
            nanos
        ));
        let store = StateStore::open(root.clone()).await.expect("open store");
        write_lane_activation_snapshot_for_store(&store)
            ...
            .expect("write lane completion activation snapshot");
        let _state_override = ProxyStateDirOverrideGuard::install(root.clone());
        let run_id = "run-lane-complete-rework-alias";
        store
            .create_task_with_fixture_parent(crate::state_store::CreateTaskRequest {
                task_id: run_id,
                title: "Rework alias routes to allowed next node",
                display_id: None,
                description: "",
                issue_type: "defect",
                status: "in_progress",
        ...
        let role_selection = lane_complete_role_selection_for_node(run_id, "tester");
        let packet_path = root.join(
            "runtime-consumption/dispatch-packets/run-lane-complete-rework-alias-tester.json",
        );
        std::fs::create_dir_all(
            packet_path
                .parent()
        ...
        std::fs::write(
            ...
            canonical_rework_lane_packet(
                ...
                "verifier",
                "verifier_proof_packet",
                "Complete the tester lane evidence.",
                "verification",
                "verifier",
                "rework alias advances to coder",
                "cargo test -p vida host_bridge_completion_rework_requires_typed_receipt_evidence",
                "rework alias materializes coder packet",
                "pending_verification_evidence",
            )
            .to_string(),
        )
        ...
        let activation_result_path = root.join(
            "runtime-consumption/dispatch-results/run-lane-complete-rework-alias-activation.json",
        );
        std::fs::create_dir_all(activation_result_path.parent().expect("activation parent"))
            .expect("create activation result dir");
        std::fs::write(
            ...
            serde_json::json!({
                ...
                "run_id": run_id,
                "dispatch_target": "tester",
                "completion_receipt_id": "completion-rework-alias-1",
                "decision": "rework_required",
                "verdict": "rework_required",
                "rework_target": "coder",
                "allowed_next_node": "coder",
                "blocker_code": "pending_verification_evidence",
                "blocker_codes": ["pending_verification_evidence"],
                "evidence": ["changed_files", "verification_notes", "explicit_blockers"],
                "execution_evidence": {
                    "receipt_backed": true,
                    "receipt_id": "completion-rework-alias-1"
                },
                "execution_state": "blocked",
                "host_tool_bridge_request": {
                    "packet_path": packet_path.display().to_string()
        ...
        receipt.blocker_code = Some("pending_verification_evidence".to_string());
        receipt.downstream_dispatch_target = None;
        receipt.downstream_dispatch_command = None;
        receipt.downstream_dispatch_note = None;
        receipt.downstream_dispatch_ready = false;
        ...
        receipt.downstream_dispatch_packet_path = None;
        receipt.downstream_dispatch_status = Some("blocked".to_string());
        receipt.downstream_dispatch_active_target = Some("tester".to_string());
        receipt.activation_runtime_role = Some("verifier".to_string());
        store
            .record_run_graph_dispatch_receipt(&receipt)
            .await
            .expect("persist dispatch receipt");
        ...
        let args = ProxyArgs {
            args: vec![
                ...
                run_id.to_string(),
                "--receipt-id".to_string(),
                "completion-rework-alias-1".to_string(),
                "--decision".to_string(),
                "rework_required".to_string(),
                "--verdict".to_string(),
                "rework_required".to_string(),
        ...
        let after = store
            ...
            .expect("receipt should exist");
        assert_eq!(after.dispatch_status, "blocked");
        assert_eq!(after.downstream_dispatch_target, None);
        assert_eq!(
            after.downstream_dispatch_active_target.as_deref(),
            Some("tester")
        );
        ...
        assert_eq!(
            after.blocker_code.as_deref(),
            Some("pending_verification_evidence")
        );
