...
pub(crate) fn materialize_host_cli_template(
    ...
    match mode.as_str() {
        ...
        PI_AGENT_PROJECTION_RENDER_MODE => {
            project_activator_host_cli_materialization::materialize_pi_agent_projection(
                project_root,
                cli_system,
                &entry_ref,
            )
        ...
        "copy_tree_only" => {
            ...
            copy_tree_if_missing(&source, &runtime_root)?;
            Ok(runtime_root)
        }
        other => Err(format!(
...
"#,
    ...
    #[test]
    fn project_activator_accepts_host_cli_selection_and_materializes_pi_projection() {
        let runtime = tokio::runtime::Runtime::new().expect("tokio runtime should initialize");
        let harness = TempStateHarness::new().expect("temp state harness should initialize");
        let _cwd = guard_current_dir(harness.path());
        ...
        let settings_path = harness.path().join(".pi/settings.json");
        let agent_path = harness.path().join(".pi/agents/pi-primary.md");
        let chain_path = harness.path().join(".pi/chains/vida-agent-init.chain.md");
        assert!(
            settings_path.is_file(),
            "Pi settings projection should exist"
        );
        assert!(
            agent_path.is_file(),
            "Pi primary agent projection should exist"
        );
        assert!(harness.path().join(".pi/chains").is_dir());
        assert!(
            chain_path.is_file(),
            "Pi agent-init chain projection should exist"
        );

        let settings = fs::read_to_string(&settings_path).expect("Pi settings should be readable");
        assert!(settings.contains("\"generated_by\": \"VIDA\""));
        assert!(settings.contains("\"source_of_truth\": \"vida.config.yaml\""));
        assert!(settings.contains("\"projection_only_not_authority\": true"));
        assert!(settings.contains("\"internal_agent_projection\""));
        assert!(settings.contains("\"projection_kind\": \"pi_internal_agent_projection\""));
        assert!(settings.contains("\"model_profiles\""));
        assert!(settings.contains("pi_gpt55_medium_guarded"));
        assert!(settings.contains("\"child_subagent_recursion_allowed\": false"));
        assert!(settings.contains("\"self_dispatch_allowed\": false"));
        assert!(settings.contains("\"write_capable_execution_requires_vida_write_guard\": true"));

        let agent =
            fs::read_to_string(&agent_path).expect("Pi agent projection should be readable");
        assert!(agent.contains("generated by VIDA"));
        assert!(agent.contains("source of truth is `vida.config.yaml`"));
        assert!(agent.contains("internal_agent_projection: true"));
        assert!(agent.contains("Carrier And Runtime Role Boundaries"));
        assert!(agent.contains("admissible model profiles are selected by VIDA runtime"));
        assert!(agent.contains("pi_gpt55_medium_guarded"));
        assert!(agent.contains("Do not launch child subagents"));
        assert!(agent.contains("Do not self-dispatch"));
        assert!(agent.contains("Do not perform write-capable Pi execution until VIDA supplies an active packet-owned write guard"));

        let chain = fs::read_to_string(&chain_path)
            .expect("Pi agent-init chain projection should be readable");
        assert!(chain.contains("projection_only: true"));
        assert!(chain.contains("vida agent-init"));
        assert!(chain.contains("Do not launch child subagents"));
        assert!(chain.contains("must not be treated as a VIDA dispatch receipt"));

        let config = fs::read_to_string(harness.path().join("vida.config.yaml"))
            .expect("config should exist");
        assert!(config.contains("cli_system: pi"));

        let view = super::build_project_activator_view(harness.path());
        assert_eq!(view["host_environment"]["selected_cli_system"], "pi");
        assert_eq!(
            view["host_environment"]["selected_cli_execution_class"],
            "external"
        );
        assert_eq!(view["host_environment"]["template_materialized"], true);
        assert_eq!(view["host_environment"]["materialization_required"], false);
        assert_eq!(view["host_environment"]["runtime_template_root"], ".pi");
        assert_eq!(
            view["normal_work_defaults"]["default_agent_topology"],
