...
mod tests {
    ...
    #[test]
    fn agent_orchestration_host_bridge_policy_is_default_in_bootstrap_surfaces() {
        ...
        for (name, body) in surfaces {
            assert!(
                body.contains("project default orchestration, runtime policy, or the user explicitly orders agent-first")
                    || body.contains("project configuration or runtime init reports agent-only/default orchestration")
                    || body.contains("VIDA config/init reports agent-only or default agent orchestration"),
                "{name} should treat runtime/default agent orchestration as explicit delegation intent"
            );
            assert!(
                !body.contains("If the user explicitly orders agent-first or parallel-agent execution"),
                "{name} should not require a fresh chat-only agent opt-in when default orchestration is active"
            );
        }
    }
...
9a. When project configuration or runtime init reports agent-only/default orchestration, that state is the project-level explicit delegation request for bounded VIDA dispatch and host bridge execution; do not wait for a second chat-only permission phrase before using configured agent carriers.\n\
...
13. If project default orchestration, runtime policy, or the user explicitly orders agent-first or parallel-agent execution, keep that routing intent sticky; do not silently substitute root-session coding.\n\
