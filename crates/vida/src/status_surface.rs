    ...
    match StateStore::open_existing(state_dir).await {
        Ok(store) => match store.storage_metadata_summary().await {
            Ok(storage_metadata) => {
                ...
                let host_agents = super::taskflow_task_bridge::infer_project_root_from_state_root(store.root())
                    .or_else(|| super::resolve_runtime_project_root().ok())
...
fn build_host_agent_status_summary(project_root: &Path) -> Option<serde_json::Value> {
    let overlay =
        super::project_activator_surface::read_yaml_file_checked(&project_root.join("vida.config.yaml"))
            .ok()?;
    ...
    match selected_cli_system {
        "codex" => {
            let overlay_roles = super::project_activator_surface::overlay_codex_agent_catalog(&overlay);
            let codex_roles = if overlay_roles.is_empty() {
                super::project_activator_surface::read_codex_agent_catalog(&project_root.join(".codex"))
            } else {
                overlay_roles
            };
            let strategy =
                super::read_json_file_if_present(&super::codex_worker_strategy_state_path(project_root))
                    .unwrap_or(serde_json::Value::Null);
            let scorecards =
                super::read_json_file_if_present(&super::codex_worker_scorecards_state_path(project_root))
                    .unwrap_or(serde_json::Value::Null);
            let observability =
