...
"#,
    ...
    async fn derive_seeded_run_graph_binds_generated_runtime_run_to_open_task_mentioned_in_request()
    {
        ...
        let alternate_flow =
            rehydrated.compiled_bundle["team_flow_authority"]["resolved_all_flow_payload"]["flows"]
                .as_array()
                .into_iter()
                .flatten()
                .find(|flow| {
    ...
    async fn rehydrate_persisted_role_selection_preserves_progressed_selected_node_identity() {
        ...
        persisted.execution_plan["development_flow"]["dispatch_contract"]["selected_node_id"] =
            serde_json::json!(progressed_node_id);
        persisted.execution_plan["development_flow"]["dispatch_contract"]["team_flow_authority_selected_node_id"] =
            serde_json::json!(progressed_node_id);
        ...
        assert_eq!(
            rehydrated.execution_plan["development_flow"]["dispatch_contract"]["team_flow_authority_selected_node_id"],
            serde_json::json!(progressed_node_id)
        );
    }
...
"#,
    ...
    async fn dispatch_init_refreshes_stale_configured_dev_team_lane_sequence() {
        ...
        stale_cache_payload["dispatch_init_dev_team_route_signature"]["allowed_next_lane_sequence"] =
            stale_allowed_next_lane_sequence.clone();
        stale_cache_payload["dispatch_init_dev_team_route_signature"]["execution_lane_sequence"] =
