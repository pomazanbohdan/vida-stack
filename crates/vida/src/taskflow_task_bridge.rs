...
#[cfg(test)]
static TEST_PROXY_STATE_DIR_OVERRIDE_GLOBAL: std::sync::Mutex<Option<PathBuf>> =
...
pub(crate) fn proxy_state_dir() -> PathBuf {
    ...
    #[cfg(test)]
    if let Some(path) = TEST_PROXY_STATE_DIR_OVERRIDE_GLOBAL
        .lock()
...
pub(crate) fn set_test_proxy_state_dir_override(path: Option<PathBuf>) {
    ...
    *TEST_PROXY_STATE_DIR_OVERRIDE_GLOBAL
        .lock()
...
fn read_runtime_consumption_snapshot(state_root: &Path) -> Result<serde_json::Value, String> {
    let snapshot_path = match crate::latest_final_runtime_consumption_snapshot_path(state_root)? {
        Some(path) => path,
        None => crate::latest_recorded_final_runtime_consumption_snapshot_path(state_root)?
            .ok_or_else(|| {
