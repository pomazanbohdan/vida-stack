...
pub(crate) fn external_cli_preflight_summary(
    ...
    if requires_external_cli && sandbox_active && !network_reachable {
        let blocker_code = crate::release1_contracts::BlockerCode::ExternalCliNetworkAccessUnavailableUnderSandbox;
...
"#,
        ...
        assert_eq!(summary["incident_baseline"]["recovery_outcome"], "pending_remediation");
        assert_eq!(
...
"#,
        ...
        let summary = external_cli_preflight_summary(&overlay, "opencode", entry);
        assert_eq!(summary["status"], "blocked");
        assert_eq!(summary["blocker_code"], "external_cli_network_access_unavailable_under_sandbox");
        assert_eq!(summary["trace_baseline"]["status"], "blocked");
        assert_eq!(summary["trace_baseline"]["outcome"], "blocked");
        assert_eq!(summary["incident_baseline"]["recovery_outcome"], "pending_remediation");
