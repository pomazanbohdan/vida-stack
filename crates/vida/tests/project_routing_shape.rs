...
fn assert_approval_gated_architecture_flow(config: &serde_yaml::Value, context: &str) {
    ...
    assert_eq!(
        ...
        Some("architect"),
        "{context} architecture_design execution preparation should use the architect flow role",
    );
    assert_eq!(
        yaml_string(&execution_prep_step["runtime_role"]),
        Some("solution_architect"),
        "{context} architecture_design execution preparation should match architect carrier profiles",
    );
    assert_eq!(
        ...
        "{context} architecture_design execution preparation should dispatch solution_architect",
    );
}
