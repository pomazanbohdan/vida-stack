...
fn configured_conversation_role(runtime_role: &str, task_class: &str) -> String {
    let config = canonical_runtime_config();
    if config
        .pointer(&format!("/agent_system/routing/{task_class}"))
        .is_some_and(serde_json::Value::is_object)
    {
        return task_class.to_string();
    }
    let policies = config
        .pointer("/agent_system/stage_attempt_policies")
        .and_then(serde_json::Value::as_object)
        .expect("stage attempt policies should be configured");
    for policy in policies.values() {
        let mut candidates = policy
            .pointer("/fanout/attempts")
            .and_then(serde_json::Value::as_array)
            .cloned()
            .unwrap_or_default();
        if let Some(consolidator) = policy.get("consolidator") {
            candidates.push(consolidator.clone());
        }
        if let Some(candidate) = candidates.into_iter().find(|candidate| {
            candidate["runtime_role"] == runtime_role && candidate["task_class"] == task_class
        }) {
            return candidate["conversation_role"]
                .as_str()
                .expect("configured assignment conversation role should be a string")
                .to_string();
        }
    }
    ...
    let roles = config
        ...
        .and_then(serde_json::Value::as_object)
        .expect("dev-team roles should be configured");
    let role_id = roles
        .iter()
        .find(|(_, role)| {
            role["runtime_role"] == runtime_role
                && role["task_classes"]
                    .as_array()
                    .is_some_and(|classes| classes.iter().any(|class| class == task_class))
        })
        .map(|(role_id, _)| role_id)
        .expect("configured runtime role/task class should resolve to a role");
    role_id
        .split('_')
        .next()
        .expect("configured role id should expose conversation-role prefix")
        .to_string()
}

fn authoritative_config_template_path() -> String {
    format!("{}/install/assets/vida.config.yaml.template", repo_root())
}

fn read_project_runtime_config(project_root: &str) -> serde_json::Value {
    let body = fs::read_to_string(format!("{project_root}/vida.config.yaml"))
        .expect("project runtime config should be readable");
    let yaml: serde_yaml::Value =
        serde_yaml::from_str(&body).expect("project runtime config should parse");
    serde_json::to_value(yaml).expect("project runtime config should convert to json")
}

fn assert_project_codex_backend_relation(project_root: &str) {
    let config = read_project_runtime_config(project_root);
    assert_eq!(
        config["host_environment"]["cli_system"], "codex",
        "temp overlay must select the configured Codex host system"
    );
    let relation = &config["host_environment"]["systems"]["codex"]["executor_backend_relation"];
    assert_eq!(relation["backend_id"], "internal_subagents");
    assert_eq!(relation["required_backend_class"], "internal");
}

fn assert_selected_team_flow_lane(
    compiled_bundle: &serde_json::Value,
    selected_node_id: &str,
    selected_task_class: &str,
) {
    let flows = compiled_bundle["team_flow_authority"]["resolved_all_flow_payload"]["flows"]
        .as_array()
        .expect("compiled TeamFlow config should expose flow rows");
    let lane = flows
        .iter()
        .flat_map(|flow| flow["lanes"].as_array().into_iter().flatten())
        .find(|lane| lane["node_id"].as_str() == Some(selected_node_id))
        .expect("selected TeamFlow node must be present in compiled repository config");
    assert_eq!(lane["task_class"], selected_task_class);
    assert!(lane["runtime_role"].as_str().is_some());
    assert!(lane["dispatch_alias"].as_str().is_some());
}
...
fn copy_directory_tree(source: &Path, destination: &Path) {
    ...
    for entry in fs::read_dir(source).expect("authoritative source directory should read") {
        let entry = entry.expect("authoritative source entry should read");
        ...
        if source_path.is_dir() {
            ...
            fs::copy(&source_path, &destination_path)
                .expect("authoritative source file should copy");
        }
    }
}
...
fn installed_vida() -> (String, Command) {
    ...
    let install_assets = format!("{install_root}/install/assets");
    fs::create_dir_all(&install_assets).expect("installed assets dir should exist");
    fs::copy(
        authoritative_config_template_path(),
        format!("{install_assets}/vida.config.yaml.template"),
    )
    .expect("installed config template should mirror canonical authority");
    fs::copy(
        format!("{}/vida.config.yaml", repo_root()),
        format!("{install_root}/vida.config.yaml"),
    )
    .expect("installed config should mirror canonical authority");
    for relative in ["AGENTS.scaffold.md", "AGENTS.sidecar.scaffold.md"] {
        fs::copy(
            format!("{}/install/assets/{relative}", repo_root()),
            format!("{install_assets}/{relative}"),
        )
        .expect("installed bootstrap scaffold should mirror canonical authority");
    }
    for relative in [
        ...
        copy_directory_tree(
            Path::new(&format!("{}/{relative}", repo_root())),
            Path::new(&format!("{install_root}/{relative}")),
        );
    }
...
fn assert_consume_final_dispatch_packet_owned_paths(
        ...
        parsed["payload"]["dispatch_receipt"]["dispatch_packet_path"].as_str()
    {
        ...
        assert_eq!(
            parsed["payload"]["dispatch_packet_preview"]["packet"]["delivery_task_packet"]["owned_paths"],
            expected
        );
    }
}
...
fn assert_consume_final_docflow_gate_blocks(parsed: &serde_json::Value, blocker_code: &str) {
    ...
    assert!(
        parsed["docflow_vida_gate_result"]["blocker_codes"]
            .as_array()
    ...
    assert!(
        parsed["docflow_vida_gate_result"]["next_actions"]
            .as_array()
            .is_some_and(|actions| {
...
fn write_runtime_lane_completion_result_fixture(path: &str, run_id: &str, completed_target: &str) {
    write_runtime_lane_completion_result_fixture_with_allowed_next(
        path,
        run_id,
        completed_target,
        None,
...
fn write_runtime_lane_completion_result_fixture_with_allowed_next(
    path: &str,
    run_id: &str,
    ...
    if let Some(next_node) = allowed_next_node {
        result["allowed_next_node"] = serde_json::json!(next_node);
    }
    write_file(path, &result.to_string());
}
...
fn bootstrap_project_runtime(project_id: &str, project_name: &str) -> (String, String) {
    ...
    let post_init_config = read_project_runtime_config(&project_root);
    let post_init_relation =
        &post_init_config["host_environment"]["systems"]["codex"]["executor_backend_relation"];
    assert_eq!(post_init_relation["backend_id"], "internal_subagents");
    assert_eq!(post_init_relation["required_backend_class"], "internal");
    ...
    assert_project_codex_backend_relation(&project_root);
...
fn doctor_with_timeout(state_dir: &str, args: &[&str]) -> std::process::Output {
    ...
    command_output_with_state_access_retry(&mut command)
}
...
fn command_output_via_files(
    ...
    let stdout = fs::File::create(stdout_path).expect("stdout file should create");
    let stderr = fs::File::create(stderr_path).expect("stderr file should create");
    let status = command
        .stdout(Stdio::from(stdout))
        .stderr(Stdio::from(stderr))
        .status()
    ...
    Output {
        status,
        stdout: fs::read(stdout_path).expect("stdout file should read"),
        stderr: fs::read(stderr_path).expect("stderr file should read"),
    }
}
...
fn root_version_succeeds() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    let expected_prefix = format!("vida {} (built ", env!("CARGO_PKG_VERSION"));
    let version = stdout.trim();
    assert!(
        version.starts_with(&expected_prefix),
        "version should include build timestamp prefix: {version}"
    );
    let timestamp = version
        .strip_prefix(&expected_prefix)
        .and_then(|value| value.strip_suffix(')'))
        .expect("version should wrap build timestamp in `(built ...)`");
...
fn taskflow_graph_summary_reports_ready_blocked_and_critical_path() {
    ...
    assert!(parsed["waves"].is_array() || parsed["waves"].is_object());
...
fn scheduler_execute_smoke_payload(requested_parallel_limit: u64) -> serde_json::Value {
    ...
    for attempt in 0..8 {
        ...
        if output.stdout.is_empty() {
            last_payload = Some(serde_json::json!({
                "status": "empty_stdout_retry",
                "stderr": String::from_utf8_lossy(&output.stderr).to_string()
            }));
            thread::sleep(Duration::from_millis(250 + (attempt as u64 * 250)));
            continue;
        }
        let payload: serde_json::Value =
            serde_json::from_slice(&output.stdout).expect("scheduler execute json should parse");
        let retryable_projection = payload["execute_supported"] == false
            && payload["blocker_codes"]
                .as_array()
                .into_iter()
                .flatten()
                .any(|code| {
                    code == "execution_preparation_gate_blocked"
                        || code == "continuation_binding_ambiguous"
                });
        if payload["execute_supported"] == true || !retryable_projection {
            return payload;
        }
    ...
    panic!(
        "scheduler execute smoke never reached supported projection after retries: {}",
        last_payload
            .expect("scheduler execute smoke should produce a payload")
            .to_string()
    );
}
...
#[test]
fn taskflow_scheduler_dispatch_execute_smoke_reports_projection_truth_for_parallel_cap_two() {
...
fn protocol_binding_check_lock_retry_preserves_boot_blocker_codes() {
    ...
    let doctor_output = doctor_with_timeout(&state_dir, &["doctor", "--json"]);
    assert_output_success(&doctor_output, "doctor after protocol-binding lock retry");
    let doctor_json: serde_json::Value =
...
fn bootstrap_init_surfaces_report_installed_vs_source_launcher_parity() {
    ...
    let current_vida = portable_test_path(&current_vida);
    ...
    for (project_id, project_name, args) in [
        ...
        let installed_paths = installed_paths
            .into_iter()
            .map(|path| portable_test_path(path))
            .collect::<std::collections::BTreeSet<_>>();
...
fn agent_init_dispatch_packet_reports_view_only_activation_semantics() {
    ...
    assert_output_success(&output, "agent-init dispatch packet");
    ...
    assert!(
        parsed["activation_semantics"]["next_lawful_action"]
            .as_str()
            .unwrap_or_default()
...
fn taskflow_golden_route_happy_path_stitches_bootstrap_dispatch_resume_status_and_doctor() {
    let (project_root, state_dir) =
        bootstrap_project_runtime("taskflow-golden-route-h11", "TaskFlow Golden Route H11");
    ...
    let initial_json: serde_json::Value =
        serde_json::from_slice(&initial.stdout).expect("initial consume final json should parse");
    assert_eq!(initial_json["surface"], "vida taskflow consume final");
    assert_eq!(
        initial_json["payload"]["dispatch_receipt"]["dispatch_surface"],
        "vida agent-init"
    );
    let dispatch_packet_path = initial_json["payload"]["dispatch_receipt"]["dispatch_packet_path"]
        .as_str()
    ...
    assert_eq!(initial_dispatch_target, "specification");
    ...
    let agent_view = vida()
        ...
        .expect("agent-init dispatch packet should run");
    assert!(
        agent_view.status.success(),
        "{}{}",
        String::from_utf8_lossy(&agent_view.stdout),
        String::from_utf8_lossy(&agent_view.stderr)
    );
    let agent_view_json: serde_json::Value =
        serde_json::from_slice(&agent_view.stdout).expect("agent-init json should parse");
    assert_eq!(
        agent_view_json["dispatch_mode"]["activation_view_only"],
        true
    );
    assert_eq!(
        agent_view_json["activation_semantics"]["executes_packet"],
        false
    );

    let source_dispatch_packet_path =
        initial_json["payload"]["dispatch_receipt"]["dispatch_packet_path"]
            .as_str()
            .expect("source dispatch packet path should be present");
    let run_id = initial_json["payload"]["dispatch_receipt"]["run_id"]
        .as_str()
        .expect("dispatch receipt run id should be present");
    let parent_task_id = "epic-golden-route-runtime-fixture";
    create_epic_parent_for_state(&state_dir, parent_task_id, "Golden route parent task");
    let create_run_task = project_bound_task_create_typed_with_timeout(
        &project_root,
        &state_dir,
        run_id,
        "Golden route runtime task",
        "task",
        Some(&parent_task_id),
    );
    assert!(
    ...
    downstream_packet_body["downstream_dispatch_target"] = serde_json::json!("closure");
    ...
    let expected_recovery_lifecycle_stage = if expected_resume_target == "closure" {
        "closure_active".to_string()
    } else {
        format!("{expected_run_graph_target}_complete")
    };

    let run_graph_latest = taskflow_run_graph_latest_with_timeout(&state_dir, true);
    assert!(run_graph_latest.status.success());
    let run_graph_latest_json: serde_json::Value =
        serde_json::from_slice(&run_graph_latest.stdout).expect("run-graph latest should parse");
    ...
    assert_output_success(&recovery_status, "golden route recovery status");
    ...
    if status_json["latest_run_graph_status"].is_object() {
        assert_eq!(status_json["latest_run_graph_status"]["run_id"], run_id);
    }
    ...
    if doctor_json["latest_run_graph_status"].is_object() {
        assert_eq!(doctor_json["latest_run_graph_status"]["run_id"], run_id);
    }
        ...
        handles.push(thread::spawn(move || {
            ...
            if output.stdout.is_empty() {
                ...
                output = vida()
                    ...
                    .env("VIDA_STATE_DIR", &state_dir)
                    .output()
                    .expect("parallel agent-init json retry should run");
            }
            output
        }));
    }
...
fn taskflow_consume_bundle_check_reports_ready_runtime_bundle() {
    ...
    let seam_docflow_surfaces =
        parsed["seam_closure_admission_receipt_check"]["docflow_proof_surfaces"]
            .as_array()
            .expect("seam docflow proof surfaces should be an array");
    assert!(
        seam_docflow_surfaces
            .iter()
...
fn assert_consume_bundle_check_pre_dispatch_blocker(output: &Output, label: &str) {
    assert!(!output.status.success(), "{label} should fail closed");
    let parsed = parse_json_output(output, label);
    let expected_surface = "vida taskflow consume bundle check";
    assert_eq!(parsed["surface"], expected_surface, "{label}: {parsed}");
    assert_eq!(parsed["status"], "blocked", "{label}: {parsed}");
    assert_eq!(
        parsed["error_kind"], "pre_dispatch_bootstrap",
        "{label}: {parsed}"
    );
    assert!(
        parsed["error"]
            .as_str()
            .is_some_and(|error| error.contains("Authoritative state root is not project-bound")),
        "{label} should retain the canonical human error: {parsed}"
    );
    let blocker_codes = parsed["blocker_codes"]
        .as_array()
        .expect("blocked pre-dispatch output should expose blocker_codes");
    assert!(
        blocker_codes
            .iter()
            .any(|code| { code.as_str() == Some("pre_dispatch_bootstrap_error") })
    );
    assert_eq!(
        parsed["artifact_refs"]["surface"], expected_surface,
        "{label} artifact surface should mirror the command surface"
    );
    assert_eq!(
        parsed["shared_fields"]["status"], parsed["status"],
        "{label} shared status should mirror top-level status"
    );
    assert_eq!(
        parsed["operator_contracts"]["status"], parsed["status"],
        "{label} operator status should mirror top-level status"
    );
    assert_eq!(
        parsed["shared_fields"]["blocker_codes"], parsed["blocker_codes"],
        "{label} shared blockers should mirror top-level blockers"
    );
    assert_eq!(
        parsed["operator_contracts"]["blocker_codes"], parsed["blocker_codes"],
        "{label} operator blockers should mirror top-level blockers"
    );
    assert!(parsed["artifact_refs"].is_object());
    assert!(parsed["shared_fields"].is_object());
    assert!(parsed["operator_contracts"].is_object());
}
...
fn status_and_consume_bundle_check_handle_legacy_pending_activation() {
    ...
    assert_consume_bundle_check_pre_dispatch_blocker(
        &check,
        "status and consume bundle check pending activation",
    );
...
fn explicit_root_and_state_dirs_keep_activation_status_canonical_through_status_and_consume_check()
{
    ...
    assert_consume_bundle_check_pre_dispatch_blocker(
        &check,
        "explicit root and state dirs consume bundle check",
    );
    let stderr = String::from_utf8_lossy(&check.stderr);
    assert!(
        stderr.contains("Authoritative state root is not project-bound"),
...
fn taskflow_consume_final_help_documents_task_metadata_and_readonly_options() {
    ...
    assert!(
        stdout.contains(
            "vida taskflow consume final --task-id <task-id> --from-task-metadata [--json]"
        )
    );
...
#[test]
fn taskflow_consume_final_blocks_downstream_closure_when_docflow_verdict_blocks() {
    let project_root = unique_state_dir();
    fs::create_dir_all(&project_root).expect("project root should exist");
    let state_dir = format!("{project_root}/.vida/data/state");
...
fn taskflow_consume_continue_resumes_from_persisted_final_snapshot() {
    ...
    let downstream_dispatch_packet_path =
        initial_json["payload"]["dispatch_receipt"]["downstream_dispatch_packet_path"]
            .as_str()
            .or_else(|| {
                initial_json["payload"]["dispatch_receipt"]["dispatch_packet_path"].as_str()
            })
...
fn agent_init_execute_dispatch_downstream_packet_does_not_actualize_forged_role_selection() {
    ...
    assert!(
        failure_output.contains("persisted_role_selection_compiled_bundle_missing")
...
fn taskflow_consume_continue_prefers_latest_final_snapshot_after_bundle_check() {
    ...
    .expect("dispatch packet should parse");
    let expected_resume_packet_path =
        initial_json["payload"]["dispatch_receipt"]["downstream_dispatch_packet_path"]
            .as_str()
    ...
    assert!(
        resumed_json["blocker_codes"]
            .as_array()
            .is_some_and(|codes| { codes.iter().any(|code| code == "docflow_check_blocking") })
    );
...
#[test]
fn taskflow_consume_continue_accepts_explicit_downstream_packet_with_task_authority() {
    let (project_root, state_dir) = bootstrap_project_runtime(
    ...
    let create_run_task = project_bound_task_create_with_timeout(
        ...
        &state_dir,
        run_id,
        "Explicit downstream packet with task authority",
    );
    assert!(
        create_run_task.status.success(),
        "{}{}",
    ...
    write_runtime_lane_completion_result_fixture(&completion_result_path, run_id, "developer");
    let downstream_dispatch_packet_path = materialize_downstream_dispatch_packet_fixture(
        &state_dir,
        source_dispatch_packet_path,
        run_id,
    ...
    assert!(
        resumed.status.success(),
        "explicit downstream packet with task authority should resume: {}{}",
        String::from_utf8_lossy(&resumed.stdout),
        String::from_utf8_lossy(&resumed.stderr)
    );
...
#[test]
fn taskflow_consume_continue_rejects_receipt_backed_executed_completed_downstream_packet_with_invalid_next()
 {
    let (project_root, state_dir) = bootstrap_project_runtime(
    ...
    assert!(
        resumed_json["error"]
            .as_str()
            .is_some_and(|error| error.contains("invalid_allowed_next_node_for_execution_plan"))
    );
    assert_eq!(
        resumed_json["dispatch_receipt"]["dispatch_target"],
        "developer"
...
#[test]
fn taskflow_consume_continue_accepts_packet_ready_downstream_packet_with_task_authority() {
    let (project_root, state_dir) = bootstrap_project_runtime(
    ...
    let create_run_task = project_bound_task_create_with_timeout(
        ...
        &state_dir,
        run_id,
        "Packet-ready downstream recovery fixture",
    );
    assert!(
        create_run_task.status.success(),
        "{}{}",
...
fn taskflow_consume_continue_ignores_packet_only_ready_downstream_without_receipt_authority() {
    ...
    assert_ne!(
        resumed_json["dispatch_receipt"]["dispatch_target"], expected_resume_target,
...
fn taskflow_consume_continue_routes_receipt_backed_ready_downstream_taskflow_packet() {
    ...
    assert!(
        resumed.status.success(),
        "receipt-backed downstream handoff should route and return pass after recording continuation binding"
    );
...
fn spec_design_parity_fixture(project_id: &str, project_name: &str) -> SpecDesignParityFixture {
    ...
    let initial_json: serde_json::Value =
        serde_json::from_slice(&initial.stdout).expect("initial consume final json should parse");
    assert_project_codex_backend_relation(&project_root);
    let dispatch_receipt = &initial_json["payload"]["dispatch_receipt"];
    let selected_node_id = initial_json["payload"]["role_selection"]["execution_plan"]
    ...
    let resolved_lanes = initial_json["payload"]["dispatch_packet_preview"]["packet"]
        ["run_graph_bootstrap"]["seed"]["role_selection"]["execution_plan"]["development_flow"]
        ["dispatch_contract"]["resolved_lanes"]
        .as_array()
        .expect("initial packet should expose resolved TeamFlow lanes");
    let selected_lane = resolved_lanes
        .iter()
        .find(|lane| lane["node_id"].as_str() == Some(selected_node_id))
        .expect("selected TeamFlow lane should match exact selected node id");
    let selected_dispatch_alias = selected_lane["dispatch_alias"]
        .as_str()
        .expect("selected TeamFlow lane should expose dispatch alias");
    let selected_task_class = selected_lane["task_class"]
        .as_str()
        .expect("selected TeamFlow lane should expose task class");
    assert_selected_team_flow_lane(
        &initial_json["payload"]["role_selection"]["compiled_bundle"],
        selected_node_id,
        selected_task_class,
    );
    let next_node_id = selected_lane["next_node"]
        .as_str()
        .expect("selected TeamFlow lane should expose next node");
    let next_lane = resolved_lanes
        .iter()
        .find(|lane| lane["node_id"].as_str() == Some(next_node_id))
        .expect("selected TeamFlow lane should expose its configured successor");
    let next_dispatch_alias = next_lane["dispatch_alias"]
        .as_str()
        .expect("successor TeamFlow lane should expose dispatch alias");
    assert_eq!(
        dispatch_receipt["dispatch_target"],
        selected_lane["dispatch_target"]
    );
    assert_ne!(
        selected_dispatch_alias, selected_task_class,
        "TeamFlow dispatch alias and task class must remain distinct"
    );
    assert_eq!(selected_lane["dispatch_alias"], selected_dispatch_alias);
    assert_eq!(selected_lane["task_class"], selected_task_class);
    assert_eq!(selected_lane["next_node"], next_node_id);
    assert_ne!(
        next_dispatch_alias, selected_dispatch_alias,
        "TeamFlow successor must not reuse the completed lane alias"
    );
    let source_dispatch_packet_path =
        initial_json["payload"]["dispatch_receipt"]["dispatch_packet_path"]
            .as_str()
    ...
    let run_id = initial_json["payload"]["dispatch_receipt"]["run_id"]
        .as_str()
        .expect("dispatch receipt run id should be present");
    let parent_id = format!("{run_id}-fixture-parent");
    let create_parent = project_bound_task_create_typed_with_timeout(
        &project_root,
        &state_dir,
        &parent_id,
        "Spec parity fixture parent",
        "epic",
        None,
    );
    assert!(
        ...
        "{}{}",
        String::from_utf8_lossy(&create_parent.stdout),
    ...
    let run_graph_meta = serde_json::json!({
        "next_node": next_node_id,
        "selected_backend": selected_lane["executor_backend_relation"]["selected_id"],
        "lane_id": selected_lane["lane_id"],
        "lifecycle_stage": selected_lane["stage"],
        "policy_gate": "not_required",
        "handoff_state": format!("awaiting_{next_node_id}"),
        "context_state": "sealed",
        "checkpoint_kind": "execution_cursor",
        "resume_target": format!("dispatch.{next_node_id}_lane"),
        "recovery_ready": true
    })
    .to_string();
    let update = run_command_with_state_lock_retry(|| {
    ...
    assert_eq!(recovery_json["recovery"]["resume_node"], next_node_id);
...
fn consume_final_uses_local_project_context_when_repo_context_is_missing() {
    ...
    assert_eq!(
        parsed["payload"]["docflow_activation"]["evidence"]["receipt_evidence"]["readiness_receipt_path"],
...
#[test]
fn project_activator_bootstrap_resolves_executor_backend_relation_after_host_selection() {
    let (project_root, state_dir) =
        bootstrap_project_runtime("activation-ordering", "Activation Ordering");

    let output = project_bound_taskflow_consume_final_with_timeout(
        &project_root,
        &state_dir,
        "activation ordering proof",
    );
    let stdout = String::from_utf8_lossy(&output.stdout);
    let parsed: serde_json::Value = serde_json::from_str(&stdout)
        .expect("activation ordering consume-final payload should parse");
    let compiled_bundle = &parsed["payload"]["role_selection"]["compiled_bundle"];
    assert!(
        compiled_bundle.is_object(),
        "fresh activation should persist a compiled bundle: {parsed}"
    );
    let relation = &compiled_bundle["carrier_runtime"]["executor_backend_relation"];
    assert_eq!(
        relation["backend_id"], "internal_subagents",
        "activation should resolve the configured executor backend: {relation}"
    );
    assert_eq!(relation["required_backend_class"], "internal");
    assert!(
        !relation.is_null(),
        "activation ordering must not persist a null executor relation"
    );
    assert!(
        !String::from_utf8_lossy(&output.stderr)
            .contains("team_flow_authority_materialization_blocked"),
        "activation ordering should not emit TeamFlow materialization blocker: {}",
        String::from_utf8_lossy(&output.stderr)
    );

    let config = fs::read_to_string(format!("{project_root}/vida.config.yaml"))
        .expect("activated project config should be readable");
    assert!(
        !config.contains("__HOST_CLI_SYSTEM__"),
        "host CLI placeholder must be resolved before bootstrap"
    );
...
fn taskflow_consume_final_routes_mixed_feature_delivery_requests_to_spec_first() {
    ...
    assert_eq!(parsed["payload"]["direct_consumption_ready"], false);
    assert_eq!(
        parsed["payload"]["role_selection"]["execution_plan"]["pre_execution_design_gate"]["design_runtime"],
        "vida docflow"
    );
    assert_eq!(
        parsed["payload"]["role_selection"]["execution_plan"]["pre_execution_design_gate"]["design_template"],
        "docs/product/spec/templates/feature-design-document.template.md"
    );
    assert_eq!(
        parsed["payload"]["role_selection"]["execution_plan"]["development_flow"]["activation_status"],
    ...
    let structured_todo =
        parsed["payload"]["role_selection"]["execution_plan"]["pre_execution_todo"]["items"]
            .as_array()
    ...
    assert_eq!(
        orchestration_contract["delegation_policy"]["canonical_project_delegated_execution_surface"],
        "vida agent-init"
    );
    assert_eq!(
    ...
    assert_eq!(
        orchestration_contract["delegation_policy"]["local_implementation_without_exception_path_forbidden"],
        true
    );
...
fn taskflow_bootstrap_spec_creates_epic_spec_task_and_design_doc() {
    ...
    assert!(
        parsed["admission"]["consumed_evidence"]
            .as_array()
...
#[cfg(unix)]
#[test]
fn taskflow_bootstrap_spec_rejects_product_spec_index_symlink() {
    let project_root = unique_state_dir();
    fs::create_dir_all(&project_root).expect("project root should exist");
    let state_dir = format!("{project_root}/.vida/data/state");
    ...
    let index_path = format!("{project_root}/docs/product/spec/index.md");
    let outside_index = format!("{project_root}/outside-product-spec-index.md");
    fs::write(&outside_index, "# Outside\n").expect("outside index should write");
    fs::remove_file(&index_path).expect("spec index should be removable");
    std::os::unix::fs::symlink(&outside_index, &index_path)
        .expect("spec index symlink should create");
    ...
    assert!(combined.contains("Refusing to use product spec index symlink"));
...
fn project_activator_rejects_foreign_env_state_dir_for_mutation() {
    ...
    assert!(
        stderr.contains("non-project state dir")
            || stderr.contains("default authoritative state dir")
            || stderr.contains("Failed to persist launcher activation snapshot")
            || stderr.contains("Unsupported host CLI system"),
        "{stderr}"
    );
...
fn project_activator_repair_rejects_foreign_env_state_dir_before_asset_mutation() {
    ...
    assert!(
        stderr.contains("non-project state dir")
            || stderr.contains("default authoritative state dir")
            || stderr.contains("Failed to persist launcher activation snapshot")
            || stderr.contains("Missing project overlay"),
        "{stderr}"
    );
    assert!(
...
fn status_json_exposes_host_agent_summary() {
    ...
    assert_eq!(parsed["host_agents"]["agents"]["count"], 4);
    assert_eq!(parsed["host_agents"]["subagent_backends"]["count"], 1);
    assert!(
        parsed["host_agents"]["internal_dispatch_alias_count"]
            .as_u64()
            .is_some_and(|count| count >= 6)
    );
    assert!(parsed["host_agents"]["named_lanes"].is_null());
    assert_eq!(
        parsed["host_agents"]["current_state"]["configured_agent_count"],
        4
    );
    assert_eq!(
        parsed["host_agents"]["current_state"]["configured_subagent_backend_count"],
        1
    );
    assert_eq!(
        parsed["host_agents"]["host_bridge_capacity"]["ready_to_attempt"],
        true
    );
    assert!(parsed["host_agents"].get("budget").is_none());

    fs::remove_dir_all(project_root).expect("temp root should be removed");
}

#[test]
fn project_activator_materializes_configured_codex_profiles_for_read_only_codex_tiers() {
    let project_root = unique_state_dir();
    fs::create_dir_all(&project_root).expect("project root should exist");
    ...
    let rerender = vida()
        .args([
            "project-activator",
            "--project-id",
            "configured-codex-materialization",
            "--project-name",
            "Configured Codex Materialization",
            "--language",
            "english",
...
fn taskflow_task_close_records_auto_feedback_and_budget() {
    ...
    let close = run_with_state_lock_retry(|| {
        vida()
            ...
            .env_remove("VIDA_ROOT")
            .env_remove("VIDA_HOME")
            .env("VIDA_TASK_CLOSE_FULL_FEEDBACK", "1")
            .env("VIDA_STATE_DIR", &state_dir)
            .output()
    ...
    let status_json: serde_json::Value =
        serde_json::from_slice(&status.stdout).expect("status json should parse");
    assert_eq!(
        status_json["host_agents"]["current_state"]["current_feedback_event_count"],
        0
    );
    assert_eq!(
        status_json["host_agents"]["host_bridge_capacity"]["ready_to_attempt"],
        true
    );
    assert!(status_json["host_agents"].get("budget").is_none());
...
fn taskflow_packet_latest_happy_path_selects_latest_run_graph_dispatch_packet() {
    ...
    let metadata = command_output_via_files(
        ...
        &format!("{state_dir}/packet-latest-metadata.stderr.txt"),
    );
    assert!(
        metadata.status.success(),
        "{}{}",
    ...
    let dispatch_init = command_output_via_files(
        dispatch_command,
        &format!("{state_dir}/packet-latest-dispatch-init.stdout.json"),
        &format!("{state_dir}/packet-latest-dispatch-init.stderr.txt"),
    );
    assert!(
    ...
    let packet_latest = run_command_with_state_lock_retry(|| {
        let mut command = vida();
        command.args(["taskflow", "packet", "latest", "--json"]);
        command.env("VIDA_STATE_DIR", &state_dir);
        command
    });
    assert!(
    ...
    let latest_packet: serde_json::Value =
        serde_json::from_slice(&packet_latest.stdout).expect("packet latest json should parse");
    assert_eq!(latest_packet["requested_run_id"], "latest");
    assert_eq!(latest_packet["run_id"], dispatch["run_id"]);
    assert_eq!(
        latest_packet["dispatch_receipt"]["run_id"],
        dispatch["dispatch_receipt"]["run_id"]
    );
    assert_eq!(
        latest_packet["dispatch_packet"]["path"],
        dispatch["dispatch_packet_path"]
    );
    assert_eq!(
        latest_packet["lawful_resume_inputs"]["dispatch_packet_path"],
        dispatch["dispatch_packet_path"]
    );
    ...
    let latest_run_graph: serde_json::Value = serde_json::from_slice(&run_graph_latest.stdout)
        .expect("run-graph latest json should parse");
    assert_eq!(
        latest_run_graph["run_graph_status"]["run_id"],
        latest_packet["run_id"]
    );
    assert_eq!(
        latest_run_graph["projection_truth"]["dispatch_receipt"]["dispatch_packet_path"],
        latest_packet["dispatch_packet"]["path"]
    );
...
fn taskflow_dispatch_init_uses_configured_dev_team_slice_for_owned_task() {
    ...
    let expected_proof_target = "cargo test -p vida --test boot_smoke taskflow_dispatch_init_uses_configured_dev_team_slice_for_owned_task -- --nocapture";
    let mut metadata_command = vida();
    metadata_command.args([
        "task",
        "update",
...
fn taskflow_factual_sandbox_h6_h8_runtime_packet_runner() {
    ...
    assert!(
        bootstrap_json["admission"]["consumed_evidence"]
            .as_array()
    ...
    assert!(
        bootstrap_json["epic"]["task_id"]
            .as_str()
            .expect("bootstrap epic task id should render")
            .starts_with("feature-")
    );
    assert!(
        bootstrap_json["spec_task"]["task_id"]
            .as_str()
    ...
    assert!(
        scheduler_json["selected_task_ids"]
            .as_array()
    ...
    assert!(
        recovery_json["recommended_command"]
            .as_str()
...
fn taskflow_continuation_bind_missing_run_fails_closed_with_blocker() {
    ...
    assert!(
        missing_bind_json["shared_fields"]["blocker_codes"]
            .as_array()
    ...
    assert!(
        missing_bind_json["shared_fields"]["next_actions"]
            .as_array()
...
fn task_root_mutation_commands_use_authoritative_db_store_without_taskflow_binary() {
    ...
    assert!(create_epic.status.success());
    assert!(
        !String::from_utf8_lossy(&create_epic.stderr).contains("delegated-taskflow-binary-ran")
    );
...
fn status_surface_reports_backend_and_bundle_receipt() {
    ...
    assert!(
        stdout.contains(
            "latest effective bundle receipt: effective-bundle-framework-agent-definition-"
        )
    );
...
fn doctor_surface_reports_integrity_checks() {
    ...
    assert!(
        stdout.contains(
            "task store: pass (0 total, 0 open, 0 in_progress, 0 closed, 0 epics, 0 ready)"
        )
    );
    assert!(stdout.contains(
    ...
    assert!(
        stdout.contains("boot compatibility: pass (backward_compatible (normal_boot_allowed))")
    );
    assert!(stdout.contains(
...
fn diagnostics_status_and_doctor_share_closed_run_projection_blocker() {
    ...
    let doctor = status_with_timeout(&project_root, &state_dir, &["doctor", "--json"]);
...
fn zombie_d_json_command(
    ...
    let output =
        bounded_vida_output_with_state_lock_retry(&["-k", "5s", "20s"], label, |command| {
            command
                .args(args)
                .current_dir(project_root)
                .env_remove("VIDA_ROOT")
    ...
    (parse_json_output(&output, label), output.status.success())
}

fn assert_zombie_d_operator_shape(value: &serde_json::Value, label: &str) {
    support::assert_release1_operator_shape(label, value);
    assert!(matches!(
        value["status"].as_str(),
...
fn zombie_d_run_graph_json_command(
    project_root: &str,
    state_dir: &str,
    run_id: &str,
    args: &[&str],
...
fn create_zombie_d_default_route_task(state_dir: &str, task_id: &str, title: &str) {
    let parent_id = format!("{task_id}-parent");
    let session_id = run_graph_test_session_id(task_id);
    let owned_path = format!("docs/{task_id}.md");
    create_epic_parent_for_state_with_session(
        state_dir,
        &parent_id,
        &format!("{title} parent"),
    ...
    let output = bounded_vida_output(
        &["-k", "5s", "20s"],
        "ZOMBIE-D default-route task create",
        |command| {
            command.args([
                "task",
                "create",
    ...
    assert_output_success(&output, "ZOMBIE-D default-route task create");
    let created = parse_json_output(&output, "ZOMBIE-D default-route task create");
    assert_eq!(created["task"]["id"], task_id);
    assert_eq!(created["task"]["status"], "open");
    assert_eq!(created["task"]["priority"], 1);
    assert!(
    ...
    assert!(
        created["task"]["planner_metadata"]["owned_paths"]
            ...
            .is_some_and(|paths| {
                paths
                    .iter()
                    .any(|path| path.as_str() == Some(owned_path.as_str()))
            })
    );
}
...
fn zombie_d_prepare_bridge_pending_task(
    ...
    let (seed, seed_success) = zombie_d_run_graph_json_command(
        ...
        "ZOMBIE-D run-graph seed",
    );
    assert!(
        seed_success,
        "ZOMBIE-D run-graph seed should succeed: {seed}"
    );
    ...
    let (advance, advance_success) = zombie_d_run_graph_json_command(
        ...
        state_dir,
        task_id,
        &["taskflow", "run-graph", "advance", task_id, "--json"],
        "ZOMBIE-D run-graph advance",
    );
    assert!(
        advance_success,
        "ZOMBIE-D run-graph advance should succeed: {advance}"
    );
    assert_eq!(advance["payload"]["status"]["run_id"], task_id);
    assert!(advance["payload"]["status"]["next_node"].is_string());

    let (recovery, recovery_success) = zombie_d_run_graph_json_command(
        project_root,
        state_dir,
        task_id,
        &["taskflow", "recovery", "status", task_id, "--json"],
        "ZOMBIE-D recovery after advance",
    );
    assert!(
        !recovery_success,
        "ZOMBIE-D recovery should expose the open delegated cycle: {recovery}"
    );
    ...
    assert!(
        ...
        "ZOMBIE-D recovery should expose the open delegated cycle blocker: {recovery}"
    );
    assert_eq!(
...
fn zombie_d_close_task(project_root: &str, state_dir: &str, task_id: &str, reason: &str) {
    ...
    assert!(success, "ZOMBIE-D close should succeed: {closed}");
    assert_eq!(closed["task"]["status"], "closed");
}

fn zombie_d_seed_stale_closed_task(
    project_root: &str,
    state_dir: &str,
    task_id: &str,
    title: &str,
) -> serde_json::Value {
    let dispatch = zombie_d_prepare_bridge_pending_task(project_root, state_dir, task_id, title);
    zombie_d_close_task(
        project_root,
        state_dir,
        task_id,
        "ZOMBIE-D close without terminal run closure truth",
    );
    dispatch
}

fn zombie_d_has_closed_task_gate(payload: &serde_json::Value) -> bool {
    let blocker = "closed_task_active_run_projection_mismatch";
    payload["blocker_codes"]
        .as_array()
        .is_some_and(|codes| codes.iter().any(|code| code.as_str() == Some(blocker)))
        || payload["continuation_binding"]["ambiguity_reason"].as_str() == Some(blocker)
        || payload["init"]["continuation_binding"]["ambiguity_reason"].as_str() == Some(blocker)
        || payload["diagnosis_detail"]["blocker_codes"]
            .as_array()
            .is_some_and(|codes| codes.iter().any(|code| code.as_str() == Some(blocker)))
}

fn zombie_d_recovery_commands(value: &serde_json::Value, commands: &mut Vec<String>) {
    match value {
        serde_json::Value::String(text)
            if text.contains("vida task reconcile-closed-runs --limit 25")
                && !text.contains("vida task reconcile-closed-runs --limit 25 --json") =>
        {
            let canonical = "vida task reconcile-closed-runs --limit 25";
            if !commands.iter().any(|command| command == canonical) {
                commands.push(canonical.to_string());
            }
        }
        serde_json::Value::Array(values) => {
            for value in values {
                zombie_d_recovery_commands(value, commands);
            }
        }
...
#[test]
fn orchestrator_init_closed_task_zombie_d_public_matrix() {
    let (project_root, state_dir) = bootstrap_project_runtime(
        "orchestrator-init-closed-task-zombie-d-public-matrix",
        "Orchestrator Init Closed Task ZOMBIE-D Public Matrix",
    );

    // Zero: fresh state has no stale closed-task active binding.
    let (zero_status, zero_status_success) = zombie_d_json_command(
        &project_root,
        &state_dir,
        &["status", "--json"],
        "ZOMBIE-D zero status",
    );
    assert!(
        zero_status_success,
        "fresh status should pass: {zero_status}"
    );
    assert!(!zombie_d_has_closed_task_gate(&zero_status));
    assert!(zero_status["latest_run_graph_status"].is_null());
    let (zero_orchestrator, zero_orchestrator_success) = zombie_d_json_command(
        &project_root,
        &state_dir,
        &["orchestrator-init", "--json"],
        "ZOMBIE-D zero orchestrator-init",
    );
    assert!(
        zero_orchestrator_success,
        "fresh orchestrator-init should pass: {zero_orchestrator}"
    );
    assert!(!zombie_d_has_closed_task_gate(&zero_orchestrator));

    // Simple: seed+advance produces healthy pre-close run-graph evidence without a dispatch receipt.
    let (simple_project_root, simple_state_dir) = bootstrap_project_runtime(
        "zombie-d-simple-control-project",
        "ZOMBIE-D Simple Control Project",
    );
    let simple_task_id = "zombie-d-simple-control";
    let simple_recovery = zombie_d_prepare_bridge_pending_task(
        &simple_project_root,
        &simple_state_dir,
        simple_task_id,
        "ZOMBIE-D simple healthy control",
    );
    assert_eq!(simple_recovery["recovery"]["run_id"], simple_task_id);
    assert_eq!(simple_recovery["recovery"]["resume_status"], "ready");
    assert!(!zombie_d_has_closed_task_gate(&simple_recovery));
    assert_eq!(
        simple_recovery["recovery"]["delegation_gate"]["delegated_cycle_open"],
        true
    );
    assert_eq!(
        simple_recovery["recovery"]["delegation_gate"]["delegated_cycle_state"],
        "handoff_pending"
    );
    let (simple_graph, simple_graph_success) = zombie_d_run_graph_json_command(
        &simple_project_root,
        &simple_state_dir,
        simple_task_id,
        &["taskflow", "run-graph", "status", simple_task_id, "--json"],
        "ZOMBIE-D simple run-graph status",
    );
    assert!(
        simple_graph_success,
        "healthy run-graph status should pass: {simple_graph}"
    );
    assert!(!zombie_d_has_closed_task_gate(&simple_graph));
    assert_eq!(simple_graph["run_graph_status"]["run_id"], simple_task_id);
    assert_eq!(
        simple_graph["delegation_gate"]["delegated_cycle_open"],
        true
    );
    assert_eq!(
    ...
    assert!(
        ...
        "healthy status should pass: {simple_status}"
    );
    ...
    assert_eq!(
        simple_status["latest_run_graph_gate"]["delegation_gate"]["delegated_cycle_state"],
        "handoff_pending"
    );
    let _ = fs::remove_dir_all(&simple_project_root);

    // One: one persisted bridge-pending run is closed without terminal run truth.
    let one_task_id = "zombie-d-one-stale-closed";
    let one_dispatch = zombie_d_seed_stale_closed_task(
        &project_root,
        &state_dir,
        one_task_id,
        "ZOMBIE-D one stale closed task",
    );
    assert_eq!(one_dispatch["recovery"]["run_id"], one_task_id);

    // Boundary: recovery observes the open delegated cycle and the close/reopen transition.
    let (one_graph, _) = zombie_d_json_command(
        &project_root,
        &state_dir,
    ...
    let (one_recovery, _) = zombie_d_json_command(
        &project_root,
        &state_dir,
        &["taskflow", "recovery", "status", one_task_id, "--json"],
        "ZOMBIE-D one recovery status",
    );
    assert!(zombie_d_has_closed_task_gate(&one_recovery));
    assert_eq!(one_recovery["recovery"]["run_id"], one_task_id);
    assert_eq!(
        one_recovery["recovery"]["delegation_gate"]["delegated_cycle_open"],
        true
    );

    let (reopened, reopened_success) = zombie_d_json_command(
        &project_root,
        &state_dir,
        &["task", "update", one_task_id, "--status", "open", "--json"],
        "ZOMBIE-D reopen",
    );
    assert!(reopened_success, "reopen should succeed: {reopened}");
    let (reopened_status, _) = zombie_d_json_command(
        &project_root,
        &state_dir,
        &["status", "--json"],
        "ZOMBIE-D reopened status",
    );
    assert!(!zombie_d_has_closed_task_gate(&reopened_status));
    zombie_d_close_task(
        &project_root,
        &state_dir,
        one_task_id,
        "ZOMBIE-D close after reopen",
    );

    // Many: the stale candidate plus two foreign candidates must select the newest run deterministically.
    let many_a = "zombie-d-foreign-a";
    let many_b = "zombie-d-foreign-b";
    zombie_d_seed_stale_closed_task(
        &project_root,
        &state_dir,
        many_a,
        "ZOMBIE-D foreign stale candidate A",
    );
    zombie_d_seed_stale_closed_task(
        &project_root,
        &state_dir,
        many_b,
        "ZOMBIE-D foreign stale candidate B",
    );

    let public_surfaces = [
        ("orchestrator-init", vec!["orchestrator-init", "--json"]),
        ("status", vec!["status", "--json"]),
        ("doctor", vec!["doctor", "--json"]),
        ("next-lawful", vec!["task", "next-lawful", "--json"]),
        (
            "run-graph",
            vec!["taskflow", "run-graph", "status", many_b, "--json"],
        ),
        (
            "recovery",
            vec!["taskflow", "recovery", "status", many_b, "--json"],
        ),
    ];
    for (label, args) in public_surfaces {
        ...
        assert_eq!(
            ...
            "{label} must publish blocked status: {payload}"
        );
        assert!(
            zombie_d_has_closed_task_gate(&payload),
            "{label} must publish the canonical closed-task gate: {payload}"
        );
        let serialized = payload.to_string();
        assert!(
            !serialized.contains(&format!("vida taskflow consume continue --run-id {many_b}")),
            "{label} emitted an impossible consume command: {payload}"
        );
        let mut recovery_commands = Vec::new();
        zombie_d_recovery_commands(&payload, &mut recovery_commands);
        assert_eq!(
            recovery_commands.len(),
            1,
            "{label} must expose exactly one lawful recovery action: {payload}"
        );
    }
    ...
    for args in [
        ...
        vec!["status", "--help"],
        vec!["doctor", "--help"],
        vec!["task", "next-lawful", "--help"],
        vec!["taskflow", "run-graph", "--help"],
        vec!["taskflow", "recovery", "--help"],
    ] {
        let help = status_with_timeout(&project_root, &state_dir, &args);
        assert!(
            help.status.success(),
            "{} help should succeed: {}",
            args.join(" "),
...
#[test]
fn team_flow_transition_cli_parity_zombie_d() {
    let (project_root, state_dir) = bootstrap_project_runtime(
        "team-flow-transition-cli-parity-zombie-d",
        "Team Flow Transition CLI Parity ZOMBIE-D",
    );

    let (routing, routing_success) = zombie_d_json_command(
        &project_root,
        &state_dir,
        &["taskflow", "validate-routing", "--json"],
        "team-flow routing JSON",
    );
    ...
    let (route, route_success) = zombie_d_json_command(
        &project_root,
        &state_dir,
        &[
            "taskflow",
    ...
    let (route_by_role, route_by_role_success) = zombie_d_json_command(
        &project_root,
        &state_dir,
        &[
            "taskflow",
    ...
    let compact = status_with_timeout(
        &project_root,
        &state_dir,
        &[
            "taskflow",
    ...
    assert!(!compact.stdout.is_empty() || !compact.stderr.is_empty());
    ...
    let help = status_with_timeout(
        &project_root,
        &state_dir,
        &["taskflow", "route", "explain", "--help"],
    );
    let help_text = format!(
        "{}{}",
        String::from_utf8_lossy(&help.stdout),
    ...
    let run_id = "team-flow-transition-cli-parity-zombie-d-run";
    create_zombie_d_default_route_task(
        &state_dir,
        run_id,
        "Team Flow Transition CLI Parity ZOMBIE-D Run",
    );

    let (dispatch, dispatch_success) = zombie_d_json_command(
        &project_root,
        &state_dir,
        &[
            "agent",
        ...
        "team-flow dev-team dispatch JSON",
    );
    assert!(
        dispatch_success,
        "dev-team dispatch should pass: {dispatch}"
    );
    let selected_lanes = dispatch["selected_lanes"]
        .as_array()
        .expect("selected_lanes should be an array");
    assert!(!selected_lanes.is_empty());
    assert_eq!(selected_lanes[0]["role_label"], "analyst");
    for lane in selected_lanes {
        let role_label = lane["role_label"]
            .as_str()
            .expect("selected lane role_label should be a string");
        assert!(!role_label.contains('-'));
        assert!(lane["runtime_role"].is_string());
        assert!(lane["task_class"].is_string());
    }
    ...
    let (seed, seed_success) = zombie_d_run_graph_json_command(
        ...
        "team-flow run-graph seed JSON",
    );
    assert!(seed_success, "run-graph seed should pass: {seed}");
    let (advance, advance_success) = zombie_d_run_graph_json_command(
        &project_root,
        &state_dir,
        run_id,
        &["taskflow", "run-graph", "advance", run_id, "--json"],
        "team-flow run-graph advance JSON",
    );
    assert!(advance_success, "run-graph advance should pass: {advance}");
    let next_node = advance["payload"]["status"]["next_node"]
        .as_str()
        .expect("run-graph advance should expose next_node");
    assert!(!next_node.contains('-'));

    let (graph, graph_success) = zombie_d_run_graph_json_command(
        &project_root,
        &state_dir,
        run_id,
        &["taskflow", "run-graph", "status", run_id, "--json"],
        "team-flow persisted run-graph JSON",
    );
    assert!(
        graph_success,
        "persisted run-graph status should pass: {graph}"
    );
    assert_zombie_d_operator_shape(&graph, "vida taskflow run-graph status");
    ...
    let bridge_boot = vida()
        .arg("boot")
        .current_dir(&bridge_project_root)
        .env_remove("VIDA_ROOT")
        .env_remove("VIDA_HOME")
        .env("VIDA_STATE_DIR", &bridge_state_dir)
        .output()
        .expect("host bridge parity boot should run");
    assert_output_success(&bridge_boot, "host bridge parity boot");
    let bridge_run_id = "team-flow-transition-cli-parity-zombie-d-host-bridge-run";
    ensure_run_graph_backing_smoke_task(
        &bridge_state_dir,
        bridge_run_id,
        "Team Flow Transition CLI Parity ZOMBIE-D Host Bridge Run",
        false,
    );
    let (bridge_seed, bridge_seed_success) = zombie_d_json_command(
        &bridge_project_root,
        &bridge_state_dir,
        &[
            "taskflow",
        ...
        "team-flow host bridge seed JSON",
    );
    assert!(
        bridge_seed_success,
        "host bridge seed should succeed: {bridge_seed}"
    );
    let bridge_update = status_with_timeout(
        &bridge_project_root,
        &bridge_state_dir,
        &[
            "taskflow",
            ...
            "in_progress",
            "implementation",
            r#"{"lifecycle_stage":"developer_active","resume_target":"dispatch.coach","handoff_state":"awaiting_coach","context_state":"sealed","checkpoint_kind":"execution_cursor","policy_gate":"not_required","recovery_ready":true}"#,
        ],
    );
    assert_output_success(&bridge_update, "host bridge parity run-graph update");
    let (dispatch_init, dispatch_init_success) = zombie_d_json_command(
        &bridge_project_root,
        &bridge_state_dir,
        &[
            "taskflow",
        ...
        "team-flow host bridge dispatch-init JSON",
    );
    assert!(
        dispatch_init_success,
        "host bridge parity dispatch-init should succeed: {dispatch_init}"
    );
    let dispatch_packet_path = dispatch_init["dispatch_packet_path"]
        .as_str()
        .expect("host bridge parity dispatch should expose packet path");
    let (agent_init, agent_init_success) = zombie_d_json_command(
        &bridge_project_root,
        &bridge_state_dir,
        &[
            "agent-init",
        ...
        "team-flow host bridge agent-init JSON",
    );
    assert!(
    ...
    let (direct_bridge, direct_bridge_success) = zombie_d_json_command(
        &bridge_project_root,
        &bridge_state_dir,
        &[
            "agent",
        ...
        "team-flow direct host bridge JSON",
    );
    assert!(
    ...
    let (bridge_persisted, bridge_persisted_success) = zombie_d_run_graph_json_command(
        &bridge_project_root,
        &bridge_state_dir,
        bridge_run_id,
        &["taskflow", "run-graph", "status", bridge_run_id, "--json"],
        "team-flow persisted host bridge JSON",
    );
    assert!(
...
fn doctor_surface_fail_closed_when_release_admission_evidence_is_incomplete() {
    ...
    let output = doctor_with_timeout(&state_dir, &["doctor", "--json"]);
    assert_output_success(&output, "doctor json release admission evidence");
...
fn doctor_surface_supports_color_render_mode() {
    ...
    let output = doctor_with_timeout(&state_dir, &["doctor", "--render", "color"]);
    assert_output_success(&output, "doctor color render");
...
fn agent_assignment_admissibility_zombie_d_public_matrix() {
    ...
    let boundary_cases = [
        BoundaryCase {
            ...
            conversation_role: configured_conversation_role("worker", "implementation"),
        },
        BoundaryCase {
            code: "B-verification",
            runtime_role: "verifier",
            task_class: "verification",
            conversation_role: configured_conversation_role("verifier", "verification"),
        },
        BoundaryCase {
            code: "B-coach",
            runtime_role: "coach",
            task_class: "coach",
            conversation_role: configured_conversation_role("coach", "coach"),
        },
    ];
    for case in boundary_cases {
        let selection_args = [
            ...
            case.task_class,
            "--conversation-role",
            case.conversation_role.as_str(),
            "--state-dir",
            state_dir.as_str(),
            "--json",
        ];
        let (selection, success) =
            zombie_d_json_command(&project_root, &state_dir, &selection_args, case.code);
