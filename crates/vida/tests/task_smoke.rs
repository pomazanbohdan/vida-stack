...
fn taskflow_scheduling_actualize_cli_contract() {
    ...
    assert_eq!(dry_run["scope"], "scheduling-actualize-root");
    assert_eq!(dry_run["scope_source"], "explicit");
    assert_eq!(dry_run["candidate_count"], 1);
    assert!(dry_run["skipped_count"].as_u64().unwrap_or(0) >= 1);
    assert!(dry_run["mutation_rationale"]
        .as_array()
        .is_some_and(|rows| rows.iter().any(|row| {
            row.as_str()
                .is_some_and(|value| value.contains("scope_source=explicit"))
        })));
    assert_eq!(
    ...
    assert_eq!(
        dry_run["candidates"][0]["proposed"]["parallel_group"],
        "explicit"
    );
    ...
    assert!(no_candidates["skipped_count"].as_u64().unwrap_or(0) >= 1);
    ...
    assert_eq!(top_level_preview["scope_source"], "explicit");
    ...
    assert_eq!(
        top_level_applied["candidates"][0]["proposed"]["parallel_group"],
        "explicit"
    );
...
#[test]
fn taskflow_scheduling_actualize_defaults_to_active_epic_or_requires_scope() {
    let state_dir = unique_state_dir();
    create_epic_parent(
        &state_dir,
        "actualize-active-epic",
        "Actualize active epic",
        "open",
    );
    let child = run_command_json(
    ...
    let no_active = run_command_capture(
        &[
            "taskflow",
            "scheduling",
            "actualize",
    ...
    assert!(!no_active.status.success());
    let no_active_payload: serde_json::Value =
        serde_json::from_slice(&no_active.stdout).expect("scope-required JSON should parse");
    assert_eq!(no_active_payload["status"], "blocked");
    assert_eq!(no_active_payload["scope_source"], "scope_required");
    assert_eq!(no_active_payload["blocker_codes"][0], "scope_required");
    ...
    let active_preview = run_command_json(
        &[
            "taskflow",
            "scheduling",
            "actualize",
    ...
    assert_eq!(active_preview["status"], "pass");
    assert_eq!(active_preview["scope"], "actualize-active-epic");
    assert_eq!(active_preview["scope_source"], "active_bounded_epic");
    assert_eq!(active_preview["candidate_count"], 1);
...
fn task_pack_finalize_includes_post_close_diagnostics() {
    ...
    create_epic_parent(
        &state_dir,
        &parent_id,
        "Pack finalize diagnostic parent",
        "open",
    );
    let task = run_command_json(
        &[
            ...
            "focused pack-finalize diagnostic proof",
            "--json",
        ],
        &state_dir,
    );
...
#[test]
fn agent_init_execute_dispatch_explicit_materialized_packet_reaches_host_bridge_gate() {
    let (project_root, state_dir) = project_bound_state_dir();
    append_internal_host_bridge_config(&project_root);

    let _ = run_and_assert_success(&["boot"], &state_dir);
    let parent_id = "agent-init-explicit-materialized-parent";
    let task_id = "agent-init-explicit-materialized-task";
    create_epic_parent(
        &state_dir,
        parent_id,
        "Agent init explicit materialized parent",
        "open",
    );
    let task = run_command_json(
        &[
            ...
            "create",
            task_id,
            "Agent init explicit materialized task",
            "--type",
            "runtime_defect",
            "--status",
            "in_progress",
    ...
    let packet_path = dispatch["packet_materialization"]["artifacts"][0]["dispatch_packet_path"]
        .as_str()
        .expect("materialized packet path should render");
    let output = run_with_state_lock_retry(|| {
        let mut command = vida();
        command
            ...
            .env("VIDA_STATE_DIR", &state_dir)
            .env_remove("VIDA_AGENT_INIT_EXECUTE_DISPATCH_WORKER");
        command
    });
    let execute: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap_or_else(|error| {
        panic!(
            "json output should parse for explicit dispatch packet: {error}\nstatus: {:?}\nstdout: {}\nstderr: {}",
            output.status.code(),
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        )
...
#[test]
fn taskflow_consume_continue_explicit_dispatch_packet_reaches_host_bridge_gate() {
    let (project_root, state_dir) = project_bound_state_dir();
    append_internal_host_bridge_config(&project_root);

    let _ = run_and_assert_success(&["boot"], &state_dir);
    let parent_id = "consume-continue-explicit-materialized-parent";
    let task_id = "consume-continue-explicit-materialized-task";
    create_epic_parent(
        &state_dir,
        parent_id,
        "Consume continue explicit materialized parent",
        "open",
    );
    let task = run_command_json(
        &[
            ...
            "create",
            task_id,
            "Consume continue explicit materialized task",
            "--type",
            "runtime_defect",
            "--status",
            "in_progress",
    ...
    let execute: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap_or_else(|error| {
        panic!(
            "json output should parse for consume continue explicit dispatch packet: {error}\nstatus: {:?}\nstdout: {}\nstderr: {}",
            output.status.code(),
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        )
...
#[test]
fn host_bridge_lock_timeout_classification_zombie_d() {
    let state_dir = unique_state_dir();
    run_and_assert_success(&["boot"], &state_dir);
    let request_path = format!("{state_dir}/host-tool-bridge/requests/request.json");
    let packet_path =
        format!("{state_dir}/runtime-consumption/downstream-dispatch-packets/run.json");
    ...
    for path in [&request_path, &packet_path, &result_path, &receipt_path] {
        fs::create_dir_all(
            std::path::Path::new(path)
                .parent()
                .expect("host bridge artifact parent"),
        )
        .expect("create host bridge artifact parent");
    }
    fs::write(
        &packet_path,
        serde_json::json!({"run_id": "zombie-lock", "dispatch_target": "worker"}).to_string(),
    )
    .expect("write non-empty packet");
    let request = serde_json::json!({
        "schema_version": 1,
        "status": "pending",
        "request_id": "zombie-lock-request",
        "run_id": "zombie-lock",
        "task_id": "zombie-lock",
    ...
    fs::write(&request_path, request.to_string()).expect("write host bridge request");
    ...
    let run_request = || {
        let output = {
            ...
            command
                ...
                .env("VIDA_STATE_DIR", &state_dir);
            command.output().expect("run host bridge request once")
        };
        let success = output.status.success();
        let payload: Value = serde_json::from_slice(&output.stdout).unwrap_or_else(|error| {
            panic!(
                "host bridge output should be JSON: {error}\nstatus: {:?}\nstdout: {}\nstderr: {}",
                output.status.code(),
                String::from_utf8_lossy(&output.stdout),
                String::from_utf8_lossy(&output.stderr)
            )
    ...
    // Z/O/M: zero receipts and every admissible request state keep the real missing-receipt
    // evidence instead of manufacturing a state-access blocker.
    for status in ["pending", "pass", "retryable_blocked"] {
        let mut request: Value = serde_json::from_str(
            &fs::read_to_string(&request_path).expect("read request for Z/O/M case"),
        )
        .expect("parse request for Z/O/M case");
        request["status"] = serde_json::json!(status);
        fs::write(&request_path, request.to_string()).expect("write Z/O/M request state");
        let (payload, success) = run_request();
        assert!(
            !success,
            "zero-receipt {status} case must fail closed: {payload}"
        );
        assert!(payload["blocker_codes"]
            .as_array()
            .expect("zero-receipt blockers")
            .iter()
    ...
    let mut request: Value = serde_json::from_str(
        &fs::read_to_string(&request_path).expect("read request for wrong-state case"),
    )
    .expect("parse request for wrong-state case");
    request["status"] = serde_json::json!("blocked");
    fs::write(&request_path, request.to_string()).expect("write wrong request state");
    let (wrong_state, wrong_state_success) = run_request();
    assert!(
        !wrong_state_success,
        "wrong request state must fail closed: {wrong_state}"
    );
    assert!(wrong_state["blocker_codes"]
        .as_array()
        .expect("wrong-state blockers")
        .iter()
        .any(|code| code == "host_bridge_request_not_pending"));

    request["status"] = serde_json::json!("pending");
    request["request_path"] = serde_json::json!(format!("{state_dir}/outside-request.json"));
    fs::write(&request_path, request.to_string()).expect("write malformed-path request");
    let (malformed, malformed_success) = run_request();
    assert!(
        !malformed_success,
        "malformed path must fail closed: {malformed}"
    );
    assert!(malformed["blocker_codes"]
        .as_array()
        .expect("malformed-path blockers")
        .iter()
        .any(|code| code == "host_bridge_request_path_mismatch"));

    request["request_path"] = serde_json::json!(&request_path);
    fs::write(&request_path, request.to_string()).expect("restore canonical request path");
    let lock = fs::OpenOptions::new()
        .create(true)
        .read(true)
        .write(true)
        .open(format!("{state_dir}/LOCK"))
        .expect("open state lock");
    lock.try_lock_exclusive().expect("hold state lock");
    let started = Instant::now();
    let (timed_out, timed_out_success) = run_request();
    assert!(
        !timed_out_success,
        "locked state must fail closed: {timed_out}"
    );
    assert!(
        started.elapsed() < Duration::from_secs(3),
        "strict timeout exceeded test bound"
    );
    assert!(timed_out["blocker_codes"]
        .as_array()
        .expect("timeout blockers")
        .iter()
        .any(|code| code == "authoritative_state_store_locked"));
    assert!(!timed_out["blocker_codes"]
        .as_array()
    ...
    drop(lock);
    let (retried, retried_success) = run_request();
    assert!(
        !retried_success,
        "receipt-free retry remains fail-closed: {retried}"
    );
    assert!(retried["blocker_codes"]
        .as_array()
        .expect("retry blockers")
        .iter()
        .any(|code| code == "host_bridge_dispatch_receipt_missing"));
    assert!(!retried["blocker_codes"]
        .as_array()
...
#[test]
fn task_close_preserves_unproven_active_run_before_reconcile() {
    let state_dir = unique_state_dir();
    fs::create_dir_all(&state_dir).expect("create state dir");
    ...
    assert!(
        ...
        "task close without receipt proof must preserve the stale active run blocker: {before}"
    );
    assert!(
        !before["latest_terminal_task_active_run_graph_status"].is_null(),
        "task close without receipt proof must preserve the terminal active-run projection: {before}"
    );
    ...
    assert!(
        diagnostics_before_blockers
            .contains(&"closed_task_active_run_projection_mismatch".to_string()),
        "diagnostics post-commit must preserve closed-run blocker when task close lacks receipt proof: {diagnostics_before}"
    );
    assert_eq!(
        diagnostics_before["taskflow_status"]["closed_task_active_run_projection_mismatch"], true,
        "diagnostics taskflow status must report the closed-run projection as blocked without receipt proof: {diagnostics_before}"
    );
    assert!(
        diagnostics_before["next_actions"]
            .as_array()
        ...
        "diagnostics post-commit should recommend manual inspection/reconcile for unproven closed-task active runs: {diagnostics_before}"
    );
    ...
    assert!(
        !after_blockers.contains(&"closed_task_active_run_projection_mismatch".to_string()),
        "explicit reconcile should clear the stale run after separate receipt checks pass: {after}"
    );
    assert!(
        after["latest_terminal_task_active_run_graph_status"].is_null(),
        "explicit reconcile should retire the active run graph after separate receipt checks pass: {after}"
    );
    let (status_after, _) =
        run_command_json_allow_failure(&["status", "--json", "--view", "full"], &state_dir);
    let status_after_blockers =
    ...
    assert!(
        !status_after_blockers.contains(&"closed_task_active_run_projection_mismatch".to_string()),
        "status should ignore reconciled terminal closure runs after explicit reconcile: {status_after}"
    );
    let (diagnostics_after, _) = run_command_json_allow_failure(
        &[
            "diagnostics",
    ...
    assert!(
        !diagnostics_after_blockers
            .contains(&"closed_task_active_run_projection_mismatch".to_string()),
        "diagnostics post-commit should clear closed-run blocker after explicit reconcile: {diagnostics_after}"
    );
    assert_eq!(
        diagnostics_after["taskflow_status"]["closed_task_active_run_projection_mismatch"],
        false
...
fn set_zombie_d_gate_enabled(project_root: &str, enabled: bool) {
    let config_path = format!("{project_root}/vida.config.yaml");
    let config = fs::read_to_string(&config_path).expect("project config should read");
    let marker = "  flows:\n";
    ...
    let replacement = format!("{gate}{marker}");
    fs::write(&config_path, config.replacen(marker, &replacement, 1))
        .expect("project config should enable ZOMBIE-D gate");
}

fn enable_zombie_d_gate(project_root: &str) {
    set_zombie_d_gate_enabled(project_root, true);
}

fn disable_zombie_d_gate(project_root: &str) {
    set_zombie_d_gate_enabled(project_root, false);
}

#[test]
fn zombie_d_gate_public_projection_and_close_contract() {
    let (project_root, state_dir) = project_bound_state_dir();
    enable_zombie_d_gate(&project_root);
    run_and_assert_success(&["boot"], &state_dir);

    let projection = run_command_json(
        &["taskflow", "consume", "agent-system", "--json"],
        &state_dir,
    );
    assert_eq!(
        projection["snapshot"]["dev_team_readiness"]["zombie_d_gate"]["enabled"],
        true
    );
    assert_eq!(
    ...
    let parent_id = "zombie-d-public-close-parent";
    let parent = run_command_json(
        &[
            "task",
            "create",
            parent_id,
            "ZOMBIE-D public contract parent",
            "--type",
            "epic",
            "--json",
        ],
    ...
    let task_id = "zombie-d-public-close-task";
    let created = run_command_json(
        &[
            "task",
            "create",
            task_id,
            "ZOMBIE-D semantic gate public contract",
            "--type",
            "task",
            "--status",
            "in_progress",
            ...
            parent_id,
            "--owned-path",
            "crates/vida/tests/zombie_d_gate.rs",
            "--json",
        ],
        &state_dir,
    );
    ...
    let (handoff, _) = run_command_json_allow_failure(
        &[
            "taskflow",
            "consume",
            "final",
    ...
    assert_eq!(
        handoff["payload"]["taskflow_handoff_plan"]["zombie_d_gate"]["status"],
        "blocked"
    );
    assert!(handoff["payload"]["closure_admission"]["blockers"]
        .as_array()
        .is_some_and(|blockers| blockers
            .iter()
            .any(|blocker| blocker == "zombie_d_matrix_missing")));

    let (blocked, close_succeeded) = run_command_json_allow_failure(
        &[
            "task",
            "close",
            task_id,
            "--reason",
            "ZOMBIE-D gate contract",
            "--json",
        ],
        &state_dir,
    );
    ...
    assert!(
        blocked["zombie_d_gate"]["blocker_codes"]
            .as_array()
            .is_some_and(|codes| codes.iter().any(|code| code == "zombie_d_matrix_missing")),
        "unexpected ZOMBIE-D blocker payload: {blocked}"
    );
    ...
    let evidence = serde_json::json!({
        ...
        "categories": {
            ...
            "M": {"status": "na", "reason": "single CLI contract fixture"},
