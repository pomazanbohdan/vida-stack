...
fn acquire_lease_expires_at(now: &str) -> Result<String, time::error::Parse> {
    OffsetDateTime::parse(now, &Rfc3339).map(|now| {
        (now + Duration::seconds(DEFAULT_ACQUIRE_LEASE_SECONDS))
            .format(&Rfc3339)
