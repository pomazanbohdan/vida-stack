...
25a. For active defect work, keep the current working cadence explicitly numbered as steps 1 through 3 and repeat that cycle as needed; one numbered step may read an unlimited number of relevant code, test, generated, runtime-state, and documentation files during investigation; step 3 in each cycle must perform the code fix for the current defect; and writing or fixing tests may ignore this cadence rule.
25b. After a defect is found and selected for work, every progress update and handoff for that defect must name the active cadence marker using this exact shape: `Крок 1/3: дослідження`, `Крок 2/3: архітектурне рішення + bounded write scope`, or `Крок 3/3: code fix`. Test authoring, proof execution, release build/install, diagnostics, TaskFlow closure, commit, and push are required post-fix gates, but they are not counted as the three repair steps.
25c. A defect cycle must not drift past the three repair steps without either applying the bounded code fix in step 3 or explicitly closing the cycle as `no-code-fix-required` with current runtime evidence that the selected defect no longer reproduces. If the defect changes shape during investigation, restate the active bounded unit, why it remains the same unit or bind a new one, then restart the numbered 1/3 through 3/3 cadence for that defect shape.
