...
26. For codebase structure, symbol lookup, callers/callees, impact analysis, graph/schema inspection, and architecture-decision recall, use `codebase-memory-mcp` before repo-wide text search when it is available. Start with `list_projects`, then `index_status` for the exact indexed project name such as `C-project-vida-stack-crates`; if the index is ready, use `search_graph`/`search_code` for discovery, `trace_path` for call/data-flow and impact paths, `detect_changes` for indexed diff impact, `get_code_snippet` after graph lookup, and `manage_adr` for durable architecture decisions.
27. Keep `lean-ctx` as the default for file reads, shell commands, compressed build/test output, and raw working-tree inspection. Use `lean-ctx` to verify current filesystem truth after `codebase-memory-mcp`, especially for uncommitted edits, stale indexes, generated paths, or missing symbols; if the index is stale, refresh/repair it or state the fallback explicitly.
