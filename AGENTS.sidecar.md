...
25a. For active defect work, keep the current working cadence explicitly numbered as steps 1 through 3 and repeat that cycle as needed; one numbered step may read an unlimited number of relevant code, test, generated, runtime-state, and documentation files during investigation; step 3 in each cycle must perform a code fix for the current defect; and writing or fixing tests may ignore this cadence rule.
