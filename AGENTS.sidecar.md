...
22f. Each open PR is a first-class TaskFlow unit, not an informal review note. Before processing a PR, create or update a dedicated TaskFlow task under the active PR batch/protocol parent with the PR number/title, source branch, status, priority, owned/read-only paths, intended role chain, acceptance target, proof target, and sequential/parallel posture. The PR task must record advisory analyst output, test-author expectations, developer/fallback implementation notes, coach/reviewer/verifier findings, proof commands, release/install evidence when required, GitHub comment/close outcome, and branch deletion status. If canonical VIDA `agent-init` can execute, route PR analysis, test authoring, implementation, coach review, verifier/prover, and closure through configured agent roles. If VIDA execution is blocked by a runtime defect, record that defect as its own TaskFlow item and enter Defective Runtime Emulation Mode only for the bounded PR: the root orchestrator must emulate the same role chain manually, keep artifacts/evidence in the PR task, and return to canonical agent execution once the blocking runtime defect is repaired. This PR protocol is one source-tier instance of the general spec-first work-item law; it must not be interpreted as limiting the same requirements to PRs only.
