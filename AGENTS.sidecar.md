...
12. For command timing, slow gates, script optimization, CI/local proof latency, oversized output, token-heavy command surfaces, missing selectors, or operator-friction diagnostics, continue to `docs/process/command-timing-and-gate-optimization-protocol.md`.
...
25. On Windows, run cargo proof commands that need the MSVC toolchain through the project wrapper:
   - `.\scripts\vida-cargo-msvc.ps1 test -p vida --lib <filter> --nocapture` for library `#[cfg(test)]` filters.
   - `.\scripts\vida-cargo-msvc.ps1 test -p vida --bin vida <filter> --nocapture` for binary-target tests.
...
26. For semantic discovery and ranked concept search, use `ccc` first after index-freshness checks. Run `ctx_compose` before `ctx_search` for bounded context and filesystem-aware confirmation; use `rg` for exact symbols, strings, paths, and high-recall fallback or confirmation. Keep `codebase-memory-mcp` optional for graph, call, or architecture context after canonical discovery and only when it does not conflict with live filesystem truth.
27. Treat output from `ccc`, `ctx_search`, `rg`, and optional `codebase-memory-mcp` as discovery only; it never replaces proof, tests, or runtime evidence. If optional graph results are unavailable, stale, truncated, or inconsistent with the filesystem, state that explicitly and verify with `lean-ctx`.
...
5. `Delegate` must prefer the cheapest capable executor and the smallest prompt that can satisfy the task. Select executor and validator from the active configuration-derived capability/telemetry registry; use the lower-cost capable pattern for bounded scans and narrow packets, and the stronger eligible pattern for TaskFlow, host-bridge, receipt authority, path policy, public CLI, release, or wave-closure gates. Do not encode model ids, role names, or reasoning levels in runtime selection logic.
...
13. If a task closes an architectural/process slice or a wave, run the runtime self-diagnostic before treating that slice or wave as operationally closed. Merge and focused post-merge proof remain task/wave gates; defer release-package, release install, and system-binary smoke to the final coherent pack or epic gate. Any actionable residual from that diagnostic must become a TaskFlow implementation task, an update to an existing TaskFlow task, or an explicit `no_task_reason` cited in the closure evidence; a prose-only diagnostic residual is not closure.
...
16. Coverage-refresh and long proof gates use failure-pool batching. After a broad or coverage gate fails, do not immediately rerun the same expensive gate after each local fix. First extract the authoritative failure pool from the newest stdout/stderr artifact refs, group failures by shared contract surface, owning file, fixture builder, command family, or runtime invariant, then fix one cluster at a time. For each cluster, run the smallest focused reproduction, then a related-proof batch that covers adjacent variants and negative cases for the same contract. Run the expensive broad/coverage gate only after the current cluster and its related proof batch pass, or when a new artifact is required to discover the next cluster. This reduces command count by amortizing full gates while increasing quality coverage around the changed invariant.
17. For coverage failures in one Rust file, the preferred quality pattern is `focused failing test -> same-surface related tests -> formatting -> full gate`. The related test batch must include tests that prove the fixed behavior does not bypass blocked previews, stale blockers, persisted-state parity, result-artifact trust boundaries, and adjacent public/operator surfaces when those dimensions exist in the changed contract. A single green focused test is evidence for progress, not closure.
18. When a failure exposes a missing optimization rule, update the project instruction surface in the same bounded step only if the rule is reusable for future bootstrap decisions and does not duplicate framework owner law. For every completed bounded task, batch one authoritative runtime snapshot, reuse its artifact refs until state changes, classify redundant reads/commands and parallel-safe discovery, and record the outcome in closure evidence; if no reusable rule changed, record `no_instruction_update_reason`. Keep such rules short, operational, and linked to the existing project process owner docs instead of creating broad new policy.
19. These optimization rules are top-level project overlay. Detailed prompt wording, lane responsibilities, and scorecard criteria live in `docs/process/project-orchestrator-operating-protocol.md` and `docs/process/team-development-and-orchestration-protocol.md`.

## Runtime DX And Architectural Rule Overlay

1. Batch-first coherent work-pool execution: when the active TaskFlow scope contains multiple adjacent, same-surface tasks, inspect the whole candidate pool first and classify its posture before implementation. This rule is project-wide, not limited to runtime-DX work. A parallel coherent pack is allowed only when all included tasks are ready in the same dependency layer, have no direct or transitive dependency edges between them, are `parallel_safe` or explicitly non-conflicting, share compatible owner surfaces or command families, have compatible dirty-worktree, owned-path, and conflict-domain boundaries, can be explained by one shared invariant or acceptance matrix, and can be validated by one consolidated proof bundle. In that posture, create/mark the TaskFlow task and step for each included slice before its write-producing mutation, analyze the pack together, implement all selected slices in one coordinated write window, write the test matrix together, then run one consolidated focused proof, one broader proof when required, one graph/doc/hygiene check, and one commit for the pack. A sequential coherent pack applies when tasks are adjacent or same-surface but ordered by dependencies, shared contracts, sequential execution semantics, conflict domains, or proof gates; keep one shared analysis, acceptance matrix, and proof plan for the pack, but implement, prove, and close each dependency layer in order, refreshing assumptions before editing the next layer. Do not call a sequential coherent pack simultaneous implementation. Before code changes, state the chosen pack posture, dependency evidence, conflict-domain and owned-path evidence, and whether the next work is simultaneous, staged, or blocked. If the user asks for simultaneous pack work but graph evidence is sequential, report the downgrade to a sequential coherent pack before coding. Close included tasks sequentially in dependency order while reusing the pack evidence in each closure. Push only under active explicit publication authorization for that task, wave, publication batch, or repeatable publication pattern. After all selected runtime tasks in the coherent pack or epic are merged and focused proof is green, run the single final release-package/install and installed-runtime smoke gate. Do not install between sibling tasks or waves. Do not batch tasks whose owned paths, runtime state, data migrations, security boundaries, release/install gates, or acceptance criteria conflict; those stay outside the pack or stay sequential with recorded reasoning.
2. Recurring session-blocker priority and build-proof rule: when the same runtime defect, blocker code, command-surface failure, lane/receipt/dispatch defect, or session-blocking symptom appears more than once in the current session, raise or keep its owning TaskFlow item at priority 1, append the recurrence evidence, and route it ahead of unrelated runtime-DX cleanup unless a higher-severity blocker is already active. When such a defect blocked the current session, continuation, lane execution, receipt/proof truth, or installed CLI behavior, its task-level closure proof must include focused source/CLI evidence and post-merge smoke; defer release install to the final coherent pack or epic gate. If that final release gate cannot run, keep the parent/epic closure blocked or record the proof blocker rather than treating the release cycle as complete.
