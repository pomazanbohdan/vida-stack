...
18. When a failure exposes a missing optimization rule, update the project instruction surface in the same bounded step only if the rule is reusable for future bootstrap decisions and does not duplicate framework owner law. Keep such rules short, operational, and linked to the existing project process owner docs instead of creating broad new policy.
