...
36. Default test-first agent chain: `analyst -> test_author -> coach_test_gate -> developer -> coach_implementation_gate -> reviewer/verifier -> prover/release_closure -> orchestrator_synthesis`. The `test_author` lane is a distinct write-producing test-first lane, middle-tier by default because regression-test quality is part of the product contract; it must not be satisfied by the QA council/verifier seat unless an explicit verifier-only packet is being routed. The `developer` lane remains the cheapest eligible write carrier by default; independent verifier/prover lanes use the configured verification carriers; architect/escalation lanes are reserved for incoherent packet boundaries, cross-scope conflicts, or repeated unresolved design blockers.
37. Every bounded work source uses the same test-first agent chain by default, with PR processing as one concrete instance. The analyst validates source intent and expected behavior against specs and local code, the test_author defines or updates regression proof, the coach checks the test contract, the developer implements, reimplements, or rejects the change in the current branch, the coach/reviewer validates implementation versus source intent and specs, the verifier/prover runs proof/build/install where required, and the orchestrator handles source-specific closure such as GitHub comment/close/branch deletion, downstream-report update, CI/release closure, operator-surface task closure, and TaskFlow closure. Parallelism is allowed for the next queued item's read-only analysis while the current item is in proof or implementation, but write scopes remain sequential unless VIDA records disjoint write ownership.
38. Role/model authority must stay config-derived. Do not hardcode concrete agent ids, carrier names, model refs, reasoning effort, or host CLI systems in runtime code, docs, prompts, or task notes except as observed evidence. Use role/task-class selectors such as analyst, test_author/autotester, developer, coach, reviewer/verifier, prover, and architect/escalation, then let the runtime resolve the current eligible carrier, model profile, cost tier, and write scope from the active configuration.
