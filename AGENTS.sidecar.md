...
7. **VERIFIED RUNTIME-DEFECT HANDLING:** A user/operator statement that VIDA runtime is defective is only a diagnostic lead, not write authorization. Before any write-producing work can proceed without normal DB-backed TODO/DocFlow receipts, independently verify the runtime failure with a bounded command or file-proof packet, record the exact command/action, exit status, active bounded unit, dirty-worktree summary, and proof target, and require either receipt-backed delegated execution evidence or runtime-confirmed active exception takeover (`local_exception_takeover_state=active` and `root_local_write_allowed=true`) for the same packet. If those conditions are unavailable, stay read-only: use bounded static analysis and script-only checks, do not edit or commit files, and record the missing TaskFlow/DocFlow evidence as a runtime-repair follow-up once the runtime is usable.
