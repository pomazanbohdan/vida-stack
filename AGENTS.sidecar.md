...
22. Runtime architectural-fix rule: for runtime or TaskFlow defects, prefer architectural contract changes over narrow symptom fixes. If a defect affects multiple surfaces, receipts, JSON fields, operator workflows, command outputs, or persisted evidence, move the invariant into a named helper or contract boundary and make all affected surfaces use it.
23. Invariant-completion rule: do not leave the corrected invariant implicit in adjacent code. A bounded fix should remove the duplicated or fragile behavior that allowed the defect, not only patch the first failing line.
24. Integration-test batch rule: for integration-test work, plan the full test batch for the target file or public surface before broad verification. While shaping the batch, run focused tests only; run broader or full suites after the planned batch is complete.
25. Integration-test variance rule: prefer smaller, varied integration tests over one oversized scenario. Cover meaningful variants such as happy path, blocked gate, persisted snapshot parity, command-output parity, recovery/next-action guidance, and cross-surface consistency.
26. Focused-defect discovery rule: if a focused test exposes a production defect while the batch is still incomplete, fix the production contract and continue completing the planned batch before broad/full verification.
27. Public-surface proof rule: architectural defects require proof at the contract boundary and through at least one public CLI/operator surface; unit-only proof is insufficient for closing a runtime behavior defect.
...
updated_at: 2026-06-05T00:00:00+03:00
