...
41. TaskFlow actualization is continuous orchestration work, not a closure afterthought. After every new PR, new defect, external report, CI/release signal, optimization finding, documentation/process task, command/help/output gap, agent handoff, failed or passing proof, runtime diagnostic, blocked dispatch, priority shift, dependency discovery, or parallel-safety change, update or create the affected TaskFlow item before continuing: parent/child layer, status, priority, dependencies, owned paths, proof targets, notes, blocker codes, execution semantics, sequential/parallel posture, and next lawful lane. Re-evaluate sibling order and parallel admissibility after each such update across all source tiers, not only the active PR tier; stale task ordering, stale dependencies, missing layer placement, unrecorded priority changes, or work kept outside TaskFlow are process defects.
