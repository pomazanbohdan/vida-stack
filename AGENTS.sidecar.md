...
34a. During batch defect recovery, the orchestrator should launch one or more read-only advisory subagents for the next defect batch when the current bounded fix can continue independently and the next-batch analysis has a disjoint write scope. During PR-first work with multiple open PRs, the same rolling rule is mandatory for the next queued PR: launch or refresh a read-only advisory analyst for PR N+1 as soon as PR N is selected, while the parent validates, reimplements, proves, comments, closes, and deletes PR N. After each individual defect or PR is proven, committed, and pushed, the orchestrator must immediately launch or refresh at least one read-only advisory subagent for the newly current defect batch or next open PR before starting the next write-producing fix, unless no parallel-safe advisory question remains; if skipped, record the concrete reason in the task notes or progress update. Advisory subagents may validate defect evidence, PR intent, likely runtime invariant, grouping, sequence/parallel posture, likely code owners, and proof targets, but they must not edit files, mutate TaskFlow/runtime state, record receipts, or decide closure. The root orchestrator must synthesize advisory findings into the batch/PR ledger, update or group TaskFlow defect or PR tasks when the task store is writable, and explicitly restate `active_bounded_unit`, `why_this_unit`, and `sequential_vs_parallel_posture` before any write-producing follow-up.
...
37. PR processing uses the same test-first agent chain by default. The analyst validates PR intent against specs and local code, the test_author defines or updates regression proof, the coach checks the test contract, the developer reimplements or rejects the PR change in the current branch, the coach/reviewer validates implementation versus PR intent and specs, the verifier/prover runs proof/build/install where required, and the orchestrator handles GitHub comment/close/branch deletion plus TaskFlow closure. Parallelism is allowed for the next PR's read-only analysis while the current PR is in proof or implementation, but PR write scopes remain sequential unless VIDA records disjoint write ownership.
