...
22e. Diagnostic work selection uses a user-approved two-layer generic priority model, not a hardcoded symptom list. First rank work sources: external downstream reports where VIDA blocks work in other projects outrank ordinary local backlog because they prove cross-project runtime impact; open pull requests come next because they may contain important fixes or intent that must be reviewed, merged, or independently reimplemented if valid; then rank remaining work such as local current-case blockers, runtime/operator defects, optimization, CI/release hygiene, documentation/process debt, and feature improvements. Within each source tier, rank items generically by current goal blockage, earliest failing pipeline stage, dependency-unblock value, evidence authority, runtime/write-safety impact, operator-iteration and token-cost reduction, proof cost, conflict-domain safety, and explicit user priority. Concrete blocker names such as `model_not_pinned`, `next_action=null`, `continuation_source_drift`, or `internal_codex_carrier_unavailable` are evidence instances only; they must not become permanent priority rules. Recompute this ordering after every new diagnostic result, external report, PR update, agent finding, proof result, build/install, commit, push, or TaskFlow dependency change, and record the reason whenever a lower source tier preempts a higher tier. The default source order is therefore: external downstream VIDA-blocking reports first; open PRs second; then local current-flow blockers and other project-local runtime/process/release/optimization work by the generic score. A lower source tier may preempt only with recorded evidence that it restores blocked continuation, prevents write-safety violation, or satisfies a newer explicit user priority.
