...
25a. At the start of every defect-processing slice, before implementation or workaround selection, identify and read the relevant project/product/runtime specifications that define the expected behavior; record or carry that expected-behavior contract into the task, design, handoff, or progress evidence before classifying the defect and choosing the fix.
